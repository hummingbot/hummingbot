...
import { baseDecode } from 'borsh';
  ...
  // information about an Hedera token
  ...
  export class NearBase {
    ...
    async getWalletFromPrivateKey(privateKey: string, accountId: string): Promise<Account> {
        if (!this._connection) {
            await this.init();
        }
    ...
    decrypt(
      ...
      password: string
        ...
        const decrpyted = Buffer.concat([decipher.update(Buffer.from(encryptedKey, 'hex')), decipher.final()]);
    ...
    async getFungibleTokenBalance(
      ...
      logger.info('Requesting balance for owner ' + contract.account.accountId + '.');
      let balance: string;
      try {
        balance = await contract.ft_balance_of({account_id: contract.account.accountId});
      } catch (_e) {
        balance = "0";
      }
      logger.info(
    ...
    async getTransaction(txHash: string, accountId: string): Promise<providers.FinalExecutionOutcome> {
  ...
  async cancelTx(
    ...
    [txHash, signedTx] = await transactions.signTransaction(
        account.accountId, nonce, [transactions.transfer(new BN(0))], baseDecode(blockHash), account.connection.signer, account.accountId, account.connection.networkId
    );
