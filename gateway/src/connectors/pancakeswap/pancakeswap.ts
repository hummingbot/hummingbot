...
export class Pancakeswap implements Uniswapish {
  ...
  public async init() {
    if (this._chain == 'bsc' && !this.bsc.ready())
      throw new Error('binance smart chain is not available');
   ...
   * @param pancakeswapRouter smart contract address
  ...
  async executeTrade(
    ...
    const contract = new Contract(pancakeswapRouter, abi, wallet);
    if (!nonce) {
