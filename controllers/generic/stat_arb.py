...
class StatArb(ControllerBase):
    ...
    def __init__(self, config: StatArbConfig, *args, **kwargs):
        ...
        self.theoretical_hedge_quote = self.config.total_amount_quote * (
            self.config.pos_hedge_ratio / (1 + self.config.pos_hedge_ratio)
        )
    ...
    def determine_executor_actions(self) -> List[ExecutorAction]:
        ...
        if (
            ...
            or self.processed_data["pair_pnl_pct"] < -self.config.sl_global
        ):
            # Close all positions
    ...
    def get_executors_to_reduce_position_on_opposite_signal(self) -> List[ExecutorAction]:
        ...
        dominant_active_executors_to_stop = self.filter_executors(
            self.executors_info,
            filter_func=lambda e: (
                e.connector_name == self.config.connector_pair_dominant.connector_name
                and e.trading_pair == self.config.connector_pair_dominant.trading_pair
                and e.side == dominant_side
            ),
        )
        hedge_active_executors_to_stop = self.filter_executors(
            self.executors_info,
            filter_func=lambda e: (
                e.connector_name == self.config.connector_pair_hedge.connector_name
                and e.trading_pair == self.config.connector_pair_hedge.trading_pair
                and e.side == hedge_side
            ),
        )
        stop_actions = [
            StopExecutorAction(controller_id=self.config.id, executor_id=executor.id, keep_position=False)
            for executor in dominant_active_executors_to_stop + hedge_active_executors_to_stop
        ]
        ...
        for position in self.positions_held:
            if (
                ...
                and position.trading_pair == self.config.connector_pair_dominant.trading_pair
                and position.side == dominant_side
            ):
                reduce_actions.extend(self.get_executors_to_reduce_position(position))
            elif (
                position.connector_name == self.config.connector_pair_hedge.connector_name
                and position.trading_pair == self.config.connector_pair_hedge.trading_pair
    ...
    def get_executors_to_quote(self) -> List[ExecutorAction]:
        ...
        if (
            self.processed_data["dominant_gap"] > Decimal("0")
        ...
        if (
            self.processed_data["hedge_gap"] > Decimal("0")
    ...
    async def update_processed_data(self):
        ...
        positions_dominant = next(
            (
                position
                for position in self.positions_held
                if position.connector_name == self.config.connector_pair_dominant.connector_name
                and position.trading_pair == self.config.connector_pair_dominant.trading_pair
                and (position.side == dominant_side or dominant_side is None)
            ),
            None,
        )
        positions_hedge = next(
            (
                position
                for position in self.positions_held
                if position.connector_name == self.config.connector_pair_hedge.connector_name
                and position.trading_pair == self.config.connector_pair_hedge.trading_pair
                and (position.side == hedge_side or hedge_side is None)
            ),
            None,
        )
        # Get position stats
        ...
        pair_pnl_pct = (
            (position_dominant_pnl_quote + position_hedge_pnl_quote) / (position_dominant_quote + position_hedge_quote)
        ...
        min_price_dominant = (
            Decimal(str(min([executor.config.entry_price for executor in executors_dominant_placed])))
            if executors_dominant_placed
            else None
        )
        max_price_dominant = (
            Decimal(str(max([executor.config.entry_price for executor in executors_dominant_placed])))
            if executors_dominant_placed
            else None
        )
        min_price_hedge = (
            Decimal(str(min([executor.config.entry_price for executor in executors_hedge_placed])))
            if executors_hedge_placed
            else None
        )
        max_price_hedge = (
            Decimal(str(max([executor.config.entry_price for executor in executors_hedge_placed])))
            if executors_hedge_placed
            else None
        )

        active_amount_dominant = Decimal(
            str(sum([executor.filled_amount_quote for executor in executors_dominant_filled]))
        )
        ...
        imbalance_scaled_pct = (
            imbalance_scaled / position_dominant_quote if position_dominant_quote != Decimal("0") else Decimal("0")
        )
        ...
        self.processed_data.update(
            {
                ...
                if min_price_dominant is not None
                else Decimal(str(dominant_price)),
                "max_price_dominant": max_price_dominant
                if max_price_dominant is not None
                else Decimal(str(dominant_price)),
    ...
    def get_spread_and_z_score(self):
        ...
        dominant_cum_returns = (
            dominant_cum_returns / dominant_cum_returns[0] if len(dominant_cum_returns) > 0 else np.array([1.0])
        )
