...
class DManV3Controller(DirectionalTradingControllerBase):
    ...
    async def update_processed_data(self):
        ...
        # Generate signal
        long_condition = (
            df[f"BBP_{self.config.bb_length}_{self.config.bb_std}_{self.config.bb_std}"] < self.config.bb_long_threshold
        )
        short_condition = (
            df[f"BBP_{self.config.bb_length}_{self.config.bb_std}_{self.config.bb_std}"]
