...
class BollinGridControllerConfig(DirectionalTradingControllerConfigBase):
    ...
    min_spread_between_orders: Decimal = Field(
        ...
        json_schema_extra={
            "prompt": "Minimum spread between grid orders (e.g., 0.005 for 0.5%): ",
            "prompt_on_new": True,
        },
    )
    ...
    max_orders_per_batch: int = Field(
        default=1, json_schema_extra={"prompt": "Maximum orders per batch: ", "prompt_on_new": True}
    )
    ...
    max_open_orders: int = Field(
        default=5, json_schema_extra={"prompt": "Maximum number of open orders: ", "prompt_on_new": True}
    )
