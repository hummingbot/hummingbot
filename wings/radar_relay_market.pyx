...
cdef class RadarRelayMarket(MarketBase):
    ...
    async def get_order(self,
                        order_hash: str,
                        account_orders: Optional[Dict[str, [Dict[str, Any]]]] = None) -> Dict[str, Any]:
        if account_orders is not None:
            order = account_orders.get(order_hash)
    ...
    async def _get_order_updates(self) -> List[Dict[str, Any]]:
        ...
        tasks = [self.get_order(o.exchange_order_id, account_orders_map)
            for o in list(self._in_flight_limit_orders.values())
