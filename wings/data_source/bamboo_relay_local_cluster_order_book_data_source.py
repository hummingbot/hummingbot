...
TRADING_PAIR_FILTER = re.compile(r"(CUSD|ETH|DAI)$")


class BambooRelayLocalClusterOrderBookDataSource(LocalClusterOrderBookDataSource):
    DIFF_TOPIC_NAME: str = "bamboorelay-order.serialized"
    SNAPSHOT_TOPIC_NAME: str = "bamboorelay-market-depth.snapshot"
    ...
    async def get_active_exchange_markets(cls) -> pd.DataFrame:
        """
        Returns all currently active WETH, CUSD trading pairs from BambooRelay, sorted by volume in descending order.
        """
        async with aiohttp.ClientSession() as client:
            async with client.get("https://rest.bamboorelay.com/main/0x/markets?include=ticker,stats") as response:
                ...
                weth_dai_price: float = 1 / float(fetch_markets.loc["DAI-WETH"]["ticker"]["price"])
    ...
    async def get_tracking_pairs(self) -> Dict[str, BambooRelayOrderBookTrackerEntry]:
        ...
        retval: Dict[str, BambooRelayOrderBookTrackerEntry] = {}
        ...
        with sql.engine.connect() as conn:
            for symbol in active_markets.index:
                ...
                if snapshot_timestamp > yesterday:
                    ...
                    order_book: BambooRelayOrderBook = BambooRelayOrderBook()
                    ...
                    retval[symbol] = BambooRelayOrderBookTrackerEntry(
                        symbol,
                        snapshot_timestamp,
                        order_book,
    ...
    async def listen_for_order_book_diffs(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        """
        Listens to real-time order book diff messages from Bamboo Relay.
        """
        while True:
            try:
    ...
    async def listen_for_order_book_snapshots(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        """
        Listens to real-time order book snapshot messages from Bamboo Relay.
        """
        while True:
            try:
