...
then
  ...
  if [ ! -d "$FOLDER" ]; then
    ...
    if [[ -d "$FOLDER/conf" && -d "$FOLDER/certs" ]]; then
      CERT_PATH=$FOLDER/certs
    else
      echo "‼️  conf & certs directory missing from path $FOLDER"
      prompt_hummingbot_data_path
    fi

    if [[ -f "$FOLDER/certs/server_cert.pem" && -f "$FOLDER/certs/server_key.pem" && -f "$FOLDER/certs/ca_cert.pem" ]]; then
      echo
    else
...
read_global_config () {
GLOBAL_CONFIG="$FOLDER/conf/conf_global.yml"
...
ENV_FILE="$FOLDER/hummingbot_conf/conf_global.yml"
...
create_instance () {
 ...
 --mount "type=bind,source=$FOLDER/conf/,destination=/usr/src/app/conf/" \
