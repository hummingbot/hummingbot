...
from kairos.cli.output import ExitCode, cell, emit, fail, json_option, render_kv, render_table
...
def _list(cm: "ClientConfigAdapter", active: Optional[Tuple[str, str, bool]], as_json: bool) -> None:
    ...
    if active is not None:
        ...
        from kairos.cli.strategy_configs import config_path, read_yaml, updatable_for
...
def _read_or_set_strategy(active: Tuple[str, str, bool], key: str, value: Optional[str], as_json: bool) -> None:
    from kairos.cli.strategy_configs import config_path, edit_config, get_value, read_yaml
    file, stype, running = active
    path = config_path(stype, file)
    if not path.exists():
