...
from kairos.cli.output import ExitCode, fail


def resolve_password(*, password_stdin: bool, confirm: bool = False) -> str:
    if password_stdin:
        line = sys.stdin.readline()
        password = line.rstrip("\n")
        if not password:
            fail("no password received on stdin", ExitCode.CONFIG_ERROR)
        return password
    ...
    if sys.stdin.isatty():
        ...
        if not password:
            fail("empty password", ExitCode.CONFIG_ERROR)
...
def unlock_keystore(password: str) -> None:
    ...
    """
    from kairos.client.config.config_crypt import ETHKeyFileSecretManger, store_password_verification
...
def login(*, password_stdin: bool = False, confirm: bool = False):
