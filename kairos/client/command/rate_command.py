...
from kairos.connector.utils import split_hb_trading_pair, validate_trading_pair
...
class RateCommand:
    def rate(self,  # type: KairosApplication
             pair: str,
             token: str
             ):
    ...
    async def show_rate(self,  # type: KairosApplication
                        pair: str,
                        ):
    ...
    async def oracle_rate_msg(self,  # type: KairosApplication
                              pair: str):
        if not validate_trading_pair(pair):
            self.notify(f"Invalid trading pair {pair}")
        else:
    ...
    async def show_token_value(self,  # type: KairosApplication
                               token: str
                               ):
        if "-" in token:
