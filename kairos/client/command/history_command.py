...
from kairos.client.settings import MAXIMUM_TRADE_FILLS_DISPLAY_OUTPUT, AllConnectorSettings
...
class HistoryCommand:
    def history(self,  # type: KairosApplication
                days: float = 0,
                verbose: bool = False,
    ...
    def get_history_trades_json(self,  # type: KairosApplication
                                days: float = 0):
        if self.strategy_file_name is None:
            return
        start_time = get_timestamp(days) if days > 0 else self.init_time
    ...
    async def history_report(self,  # type: KairosApplication
                             start_time: float,
    ...
    def report_header(self,  # type: KairosApplication
                      start_time: float):
        lines = []
    ...
    def report_performance_by_market(self,  # type: KairosApplication
                                     market: str,
                                     trading_pair: str,
    ...
    def list_trades(self,  # type: KairosApplication
                    start_time: float):
        if threading.current_thread() != threading.main_thread():
