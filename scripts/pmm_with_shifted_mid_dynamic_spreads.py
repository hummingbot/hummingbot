...
class PMMhShiftedMidPriceDynamicSpread(ScriptStrategyBase):
    ...
    def format_status(self) -> str:
        ...
        lines.extend(["\n-----------------------------------------------------------------------------------------------------------\n"])
        lines.extend(["", f"  Price Multiplier: {self.price_multiplier:.4f} | Spread Multiplier: {self.spread_multiplier:.4f}"])
        lines.extend(["", f"  Spread Base: {self.spread_base:.4f} | Spread Adjusted: {(self.spread_multiplier * self.spread_base):.4f}"])
        lines.extend(["", f"  Mid Price: {mid_price:.4f} | Price shifted: {reference_price:.4f}"])
        lines.extend(["\n-----------------------------------------------------------------------------------------------------------\n"])
        candles_df = self.get_candles_with_features()
