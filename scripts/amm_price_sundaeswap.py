...
class AmmPriceConfig(BaseClientModel):
    ...
    connector: str = Field(
        "sundaeswap/amm",
        json_schema_extra={"prompt": "Gateway connector ID (e.g. sundaeswap/amm)", "prompt_on_new": True}
    )
    chain: str = Field(
        "cardano",
    ...
    network: str = Field(
        "preview",
        json_schema_extra={"prompt": "Network (e.g. preview)", "prompt_on_new": True}
    )
    trading_pair: str = Field(
        "ADA-SBERRY",
        json_schema_extra={"prompt": "Trading pair (e.g. ADA-SBERRY)", "prompt_on_new": True}
    )
    side: str = Field(
        "SELL",
...
class AmmPriceSundaeswap(ScriptStrategyBase):
    """
    Fetches a single price quote from Sundaeswap AMM via the Gateway HTTP API.
    """
    ...
    def __init__(self, connectors: Dict[str, object], config: AmmPriceConfig):
        ...
        self.logger().info(
            f"Initialized AmmPriceSundaeswap: "
    ...
    async def async_fetch(self):
        try:
            ...
            self.log_with_clock(
                logging.ERROR,
                f"Error fetching quote from Sundaeswap AMM: {e}"
            )
