...
class SimpleDirectionalRSIConfig(StrategyV2ConfigBase):
    ...
    rsi_period: int = Field(
        ...
        client_data=ClientFieldData(
            ...
            prompt_on_new=True))
    rsi_low: Decimal = Field(
        default=Decimal("30"), gt=0,
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the RSI low (e.g. 30): ",
            prompt_on_new=True))
    rsi_high: Decimal = Field(
        default=Decimal("70"), gt=0,
        client_data=ClientFieldData(
...
class SimpleDirectionalRSI(StrategyV2Base):
    ...
    def __init__(self, connectors: Dict[str, ConnectorBase], config: SimpleDirectionalRSIConfig):
        if len(config.candles_config) == 0:
            self.max_records = config.rsi_period
        ...
        for connector_name, trading_pairs in self.config.markets.items():
            for trading_pair in trading_pairs:
                ...
                if signal is not None:
                    ...
                    if signal["signal"] == 1 and len(active_longs) == 0:
                    ...
                    elif signal["signal"] == -1 and len(active_shorts) == 0:
                        return [CreateExecutorAction(
                            executor_config=PositionExecutorConfig(
                                timestamp=self.current_timestamp,
                                connector_name=connector_name,
    ...
    def stop_actions_proposal(self) -> List[StopExecutorAction]:
        for connector_name, trading_pairs in self.config.markets.items():
            for trading_pair in trading_pairs:
                ...
                if signal is not None:
                    if signal["signal"] == -1 and len(active_longs) > 0:
                        return [StopExecutorAction(executor_id=e.id) for e in active_longs]
                    elif signal["signal"] == 1 and len(active_shorts) > 0:
                        return [StopExecutorAction(executor_id=e.id) for e in active_shorts]
    ...
    def get_active_executors_by_side(self, connector_name: str, trading_pair: str):
        active_executors_by_connector_pair = self.filter_executors(
            ...
            filter_func=lambda e: e.connector_name == connector_name and e.connector_name == trading_pair and e.is_active
        )
    ...
    def get_signal(self, connector_name: str, trading_pair: str) -> Optional[Decimal]:
        candles = self.market_data_provider.get_candles_df(connector_name, trading_pair, self.config.interval)
        candles.ta.rsi(candles["close"], length=self.config.rsi_period, append=True)
        candles["signal"] = 0
        candles.loc[candles["RSI_14"] < self.config.rsi_low, "signal"] = 1
        candles.loc[candles["RSI_14"] > self.config.rsi_high, "signal"] = -1
