...
class BacktestMM(ScriptStrategyBase):
    ...
    candle = CandlesFactory.get_candle(
        CandlesConfig(connector=exchange, trading_pair=trading_pair, interval=interval, max_records=days * 60 * 24)
    )
    candle.start()
    ...
    def on_tick(self):
        if not self.candle.ready:
            self.logger().info(
                f"Candles not ready yet for {self.trading_pair}! Missing {self.candle._candles.maxlen - len(self.candle._candles)}"
            )
            pass
        else:
            ...
            df["fees_paid"] = (
                (df["buy_amount"] * df["bid_price"] + df["sell_amount"] * df["ask_price"]) * self.fee_bps / 10000
            )
            df["base_delta"] = df["buy_amount"] - df["sell_amount"]
            df["quote_delta"] = (
                df["sell_amount"] * df["ask_price"] - df["buy_amount"] * df["bid_price"] - df["fees_paid"]
            )
    ...
    def get_assets_df(self, df):
        ...
        assets_data = [
            [
                f"{f'{self.base}':<27}",
                f"{base_bal_start:2}",
                round(base_bal_end, self.precision),
                round(base_bal_change, self.precision),
            ],
            [
                f"{f'{self.quote}':<27}",
                f"{quote_bal_start:2}",
                round(quote_bal_end, self.precision),
                round(quote_bal_change, self.precision),
            ],
            [f"{f'{self.base}-{self.quote} price':<27}", start_price, end_price, end_price - start_price],
            [
                f"{'Base asset %':<27}",
                f"{base_bal_start_pct:.2%}",
    ...
    def format_status(self) -> str:
        ...
        lines.extend(
            ["", "  Performance:"]
            + ["    " + line for line in performance_df.to_string(index=False, header=False).split("\n")]
        )
