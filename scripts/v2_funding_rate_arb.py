...
class FundingRateArbitrageConfig(StrategyV2ConfigBase):
    ...
    connectors: Set[str] = Field(
        ...
        json_schema_extra={
            "prompt": lambda mi: (
                "Enter the connectors separated by commas (e.g. hyperliquid_perpetual,binance_perpetual): "
            ),
            "prompt_on_new": True,
        },
    )
    ...
    position_size_quote: Decimal = Field(
        ...
        json_schema_extra={
            "prompt": lambda mi: (
                "Enter the position size in quote asset (e.g. order amount 100 will open 100 long on hyperliquid and 100 short on binance): "
            ),
            "prompt_on_new": True,
        },
    )
    profitability_to_take_profit: Decimal = Field(
        ...
        json_schema_extra={
            "prompt": lambda mi: (
                "Enter the profitability to take profit (including PNL of positions and fundings received): "
            ),
            "prompt_on_new": True,
        },
    )
    ...
    def update_markets(self, markets: MarketDict) -> MarketDict:
        for connector in self.connectors:
            trading_pairs = {
                FundingRateArbitrage.get_trading_pair_for_connector(token, connector) for token in self.tokens
            }
