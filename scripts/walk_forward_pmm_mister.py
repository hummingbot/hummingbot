...
def build_config(args, parameters: PMMParameters, suffix: str):
    data = {
        "id": f"walk_forward_pmm_{suffix}", "controller_type": "generic", "controller_name": "pmm_mister",
        "connector_name": args.connector, "trading_pair": args.trading_pair,
        "total_amount_quote": str(args.capital), "portfolio_allocation": str(args.portfolio_allocation),
        "target_base_pct": "0.5", "min_base_pct": "0.3", "max_base_pct": "0.7",
        "buy_spreads": str(parameters.spread), "sell_spreads": str(parameters.spread),
        "buy_amounts_pct": "1", "sell_amounts_pct": "1",
        "executor_refresh_time": parameters.refresh_seconds,
        "buy_cooldown_time": parameters.refresh_seconds, "sell_cooldown_time": parameters.refresh_seconds,
        ...
        "price_distance_tolerance": str(parameters.spread), "refresh_tolerance": str(parameters.spread),
        "leverage": args.leverage, "position_mode": "ONEWAY", "position_side": "BUY",
        "take_profit": str(parameters.take_profit), "open_order_type": "LIMIT_MAKER",
        "take_profit_order_type": "LIMIT_MAKER", "max_active_executors_by_level": 2,
        "position_profit_protection": True, "global_tp_enabled": False, "global_sl_enabled": True,
        "global_stop_loss": str(args.global_stop_loss),
    }
    return BacktestingEngineBase.get_controller_config_instance_from_dict(data, controllers_module="controllers")


def normalize_engine_results(result: Dict) -> Dict:
    raw = dict(result["results"])
    executors = result["executors"]
    filled = [executor for executor in executors if float(executor.filled_amount_quote) > 0]
    raw["net_pnl_quote"] = float(raw.get("net_pnl_quote", 0)) + float(raw.get("unrealized_pnl_quote", 0))
    raw["total_volume"] = sum(float(executor.filled_amount_quote) for executor in filled)
    raw["total_executors_with_position"] = len(filled)
    return raw


async def run_period(args, parameters: PMMParameters, start: int, end: int, suffix: str, costs: CostModel) -> Dict:
    engine = BacktestingEngineBase()
    result = await engine.run_backtesting(
        build_config(args, parameters, suffix), start, end,
        backtesting_resolution=args.interval, trade_cost=costs.fee_rate,
    )
...
async def run_walk_forward(args) -> Dict:
    ...
    windows = generate_rolling_windows(
        start, end, int(args.train_days * 86_400), int(args.test_days * 86_400), args.purge_minutes * 60,
    )
    ...
    for window in windows:
        ...
        for candidate_index, candidate in enumerate(candidates, start=1):
            try:
                metrics = await run_period(args, candidate, window.train_start, window.train_end,
        ...
        if not successful:
            folds.append({"index": window.index, "status": "failed", "window": window.to_dict(),
                          "training_runs": training_runs, "error": "所有训练参数都运行失败"})
            continue
        selected = max(successful, key=lambda run: run["metrics"]["validation_score"])
        parameters = PMMParameters(**selected["parameters"])
        try:
            metrics = await run_period(args, parameters, window.test_start, window.test_end,
                                       f"{window.index}_test", costs)
            folds.append({"index": window.index, "status": "completed", "window": window.to_dict(),
                          "selected_parameters": asdict(parameters), "training_runs": training_runs,
                          "metrics": metrics})
        except Exception as exc:
            folds.append({"index": window.index, "status": "failed", "window": window.to_dict(),
                          "selected_parameters": asdict(parameters), "training_runs": training_runs,
    ...
    return {
        ...
        "strategy": "pmm_mister", "strategy_label": "高级纯做市",
        "status": "completed" if summary["completed_folds"] == len(windows) else "partial",
        "validation_passed": summary["passed"],
        "configuration": {
            "connector": args.connector, "trading_pair": args.trading_pair, "interval": args.interval,
            "start": start, "end": end, "train_days": args.train_days, "test_days": args.test_days,
            "purge_minutes": args.purge_minutes, "capital_quote": args.capital,
            "portfolio_allocation": args.portfolio_allocation, "candidate_count": len(candidates),
        },
        "cost_model": asdict(costs), "summary": summary, "folds": folds,
    }
...
def render_markdown(report: Dict) -> str:
    ...
    lines = [
        "# 高级纯做市滚动验证报告", "", f"- 生成时间：{report['generated_at']}",
        ...
        f"- 成交次数：{summary['total_positions']}", "", "## 分折结果", "",
    ]
...
async def main():
    ...
    parser.add_argument("--json-output", default=str(ROOT / "reports" / "pmm_mister_walk_forward_latest.json"))
    parser.add_argument("--markdown-output", default=str(ROOT / "reports" / "pmm_mister_walk_forward_latest.md"))
