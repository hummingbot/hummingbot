...
class SimpleArbitrage(ScriptStrategyBase):
    ...
    markets = {exchange_A: {trading_pair}, exchange_B: {trading_pair}}
    ...
    def get_vwap_prices_for_amount(self, amount: Decimal):
        ...
        vwap_prices = {
            self.exchange_A: {"bid": bid_ex_a.result_price, "ask": ask_ex_a.result_price},
            self.exchange_B: {"bid": bid_ex_b.result_price, "ask": ask_ex_b.result_price},
        }
        return vwap_prices
    ...
    def get_fees_percentages(self, vwap_prices: Dict[str, Any]) -> Dict:
        ...
        a_fee = (
            self.connectors[self.exchange_A]
            .get_fee(
        ...
        b_fee = (
            self.connectors[self.exchange_B]
            .get_fee(
                base_currency=self.base,
                quote_currency=self.quote,
                order_type=OrderType.MARKET,
        ...
        return {self.exchange_A: a_fee, self.exchange_B: b_fee}
    ...
    def check_profitability_and_create_proposal(self, vwap_prices: Dict[str, Any]) -> Dict:
        ...
        if profitability_analysis["buy_a_sell_b"]["profitability_pct"] > self.min_profitability:
            ...
            proposal[self.exchange_A] = OrderCandidate(
            ...
            proposal[self.exchange_B] = OrderCandidate(
                trading_pair=self.trading_pair,
                is_maker=False,
                order_type=OrderType.MARKET,
    ...
    def format_status(self) -> str:
        ...
        vwap_prices = self.get_vwap_prices_for_amount(self.order_amount)
        lines.extend(
            ["", "  VWAP Prices for amount"]
            + ["     " + line for line in pd.DataFrame(vwap_prices).to_string().split("\n")]
        )
        profitability_analysis = self.get_profitability_analysis(vwap_prices)
        lines.extend(
            ["", "  Profitability (%)"]
