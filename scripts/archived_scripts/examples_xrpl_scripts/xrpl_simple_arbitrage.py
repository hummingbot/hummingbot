...
class SimpleArbitrage(ScriptStrategyBase):
    ...
    """
    order_amount = Decimal("1")  # in base asset
    min_profitability = Decimal("0.002")  # in percentage
    base_a = "XRP"
    quote_a = "USD"
    base_b = "XRP"
    quote_b = "USDT"
    trading_pair_a = f"{base_a}-{quote_a}"
    trading_pair_b = f"{base_b}-{quote_b}"
    exchange_A = "xrpl_xrpl_mainnet"
    exchange_B = "gate_io_paper_trade"
    ...
    def on_tick(self):
        if self.last_arbitrage_ts < (self.current_timestamp - self.arbitrage_delay):
            self.last_arbitrage_ts = self.current_timestamp
    ...
    def get_vwap_prices_for_amount(self, amount: Decimal):
        bid_ex_a = self.connectors[self.exchange_A].get_vwap_for_volume(self.trading_pair_a, False, amount)
        ask_ex_a = self.connectors[self.exchange_A].get_vwap_for_volume(self.trading_pair_a, True, amount)
        bid_ex_b = self.connectors[self.exchange_B].get_vwap_for_volume(self.trading_pair_b, False, amount)
        ask_ex_b = self.connectors[self.exchange_B].get_vwap_for_volume(self.trading_pair_b, True, amount)
        vwap_prices = {
            self.exchange_A: {
    ...
    def get_fees_percentages(self, vwap_prices: Dict[str, Any]) -> Dict:
        ...
        a_fee = self.connectors[self.exchange_A].get_fee(
            base_currency=self.base_a,
            quote_currency=self.quote_a,
        ...
        b_fee = self.connectors[self.exchange_B].get_fee(
            base_currency=self.base_b,
            quote_currency=self.quote_b,
            order_type=OrderType.MARKET,
            order_side=TradeType.BUY,
            amount=self.order_amount,
    ...
    def check_profitability_and_create_proposal(self, vwap_prices: Dict[str, Any]) -> Dict:
        ...
        if profitability_analysis["buy_a_sell_b"]["profitability_pct"] > self.min_profitability:
            ...
            proposal[self.exchange_A] = OrderCandidate(trading_pair=self.trading_pair_a, is_maker=False,
                                                       order_type=OrderType.MARKET,
                                                       order_side=TradeType.BUY, amount=Decimal(self.order_amount),
                                                       price=vwap_prices[self.exchange_A]["ask"])
            proposal[self.exchange_B] = OrderCandidate(trading_pair=self.trading_pair_b, is_maker=False,
                                                       order_type=OrderType.MARKET,
