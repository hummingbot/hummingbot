...
class DexOneWay(ScriptStrategyBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger().info("=== INIT Dex One Way ===")
        ...
        # Bybit (for price reference only)
        self.bybit = ccxt.bybit({
            "apiKey": os.getenv("BYBIT_KEY"),
            "secret": os.getenv("BYBIT_SEC"),
        })
        self.logger().info("Bybit connector initialized (price reference only)")
    ...
    def on_tick(self):
        try:
            ...
            # 1️⃣  CEX fair-value (for reference only)
