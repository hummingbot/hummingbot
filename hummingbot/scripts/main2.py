...
class PrecisionTradingStrategyHummingbot(ScriptStrategyBase):
    ...
    def _calculate_trap_indicators(self):
        ...
        if candle_range > 1e-9: # Avoid division by zero
            ...
            # Original logic used ratio to body, switching to ratio to range for stability when body is small
            # self.trap_indicators['upper_wick_ratio'] = upper_wick / candle_body if candle_body > 1e-9 else (5.0 if upper_wick > 0 else 0) # Large ratio if body is zero
            # self.trap_indicators['lower_wick_ratio'] = lower_wick / candle_body if candle_body > 1e-9 else (5.0 if lower_wick > 0 else 0)
    ...
    def calculate_market_regime(self):
        ...
        # Annualized estimate (depends heavily on timeframe) - Use simpler relative measure
        # volatility = np.std(returns) * np.sqrt(252) # This depends on timeframe, less reliable
        ...
        if adx > 25: # Trending market
             ...
             # Check volatility within trend
             # if volatility_std > np.percentile(pd.Series(returns).rolling(window=20).std().dropna(), 75): # High volatility?
    ...
    def calculate_indicator_scores(self):
        ...
        if current_price > ema_short > ema_long: self.indicator_scores['ema'] = 80.0 + ema_bonus # Strong Bullish
        elif current_price > ema_short and current_price > ema_long: self.indicator_scores['ema'] = 70.0 + ema_bonus # Bullish
        elif current_price > ema_long: self.indicator_scores['ema'] = 60.0 # Weak Bullish
        elif current_price < ema_short < ema_long: self.indicator_scores['ema'] = 20.0 - ema_bonus # Strong Bearish
        elif current_price < ema_short and current_price < ema_long: self.indicator_scores['ema'] = 30.0 - ema_bonus # Bearish
        elif current_price < ema_long: self.indicator_scores['ema'] = 40.0 # Weak Bearish
        else: self.indicator_scores['ema'] = 50.0 # Neutral / Crossing
        ...
        if current_price <= lower_band * 1.005: # Price touching or below lower band
            ...
            self.indicator_scores['bbands'] = 20.0 + (position * 60.0) # Scale 20-80 within bands
    ...
    def _check_rsi_divergence(self, lookback=14, bearish=False) -> bool:
        ...
        try:
            ...
            if not bearish: # Bullish divergence: price lower low, RSI higher low
                ...
                # Optional stricter check: rsi_makes_higher_low = rsi_series.iloc[-1] > window_rsi.min()

                return price_makes_lower_low and rsi_makes_higher_low and rsi_series.iloc[-1] < 50 # Only if RSI is not already high
                ...
                # Optional stricter check: rsi_makes_lower_high = rsi_series.iloc[-1] < window_rsi.max()

                return price_makes_higher_high and rsi_makes_lower_high and rsi_series.iloc[-1] > 50 # Only if RSI is not already low
    ...
    def _calculate_sr_score(self, current_price: float) -> float:
        ...
        if closest_support is None and closest_resistance is None: return 50.0
        if closest_support is None: closest_support = 0 # Treat price floor as support
        if closest_resistance is None: closest_resistance = float('inf') # No ceiling
        ...
        if support_dist <= resistance_dist: # Closer to support
            if support_dist < proximity_threshold_strong: return 85.0 # Very close to support
            elif support_dist < proximity_threshold_weak: return 70.0 # Near support
            else: return 60.0 # Moving towards support
        else: # Closer to resistance
            if resistance_dist < proximity_threshold_strong: return 15.0 # Very close to resistance
            elif resistance_dist < proximity_threshold_weak: return 30.0 # Near resistance
            else: return 40.0 # Moving towards resistance
    ...
    def detect_bull_trap(self, price: float, resistance_level: Optional[float] = None) -> Tuple[bool, float]:
        ...
        if resistance_level is None:
            ...
            if not resistances_above: return False, 0.0
        ...
        if is_breaking or is_rejecting_near:
            ...
            if is_trap: logger.warning(f"Potential Bull Trap Detected! Score: {trap_score:.1f}")
    ...
    def detect_bear_trap(self, price: float, support_level: Optional[float] = None) -> Tuple[bool, float]:
        ...
        if support_level is None:
            ...
            if not supports_below: return False, 0.0
        ...
        if is_breaking or is_rejecting_near:
            ...
            is_trap = trap_score >= 65 # Adjusted threshold
            if is_trap: logger.warning(f"Potential Bear Trap Detected! Score: {trap_score:.1f}")
            return is_trap, trap_score
    ...
    def generate_final_signal(self) -> Optional[Dict]:
        ...
        if regime == "trending":
            ...
            if trend_dir == 1 and base_score < 50: confidence_modifier -= 0.1 # Penalize shorts in uptrend
            if trend_dir == -1 and base_score > 50: confidence_modifier -= 0.1 # Penalize longs in downtrend
        elif regime == "ranging":
        ...
        # elif regime == "volatile": # Could add specific logic for high volatility
    ...
    def _calculate_score_for_tf(self, tf_name: str) -> float:
        ...
        # Or just check alignment of key indicators like RSI, MACD, EMA position
        ...
        # RSI check
        if ind['rsi'] > 60: score += 20 * weight
        elif ind['rsi'] < 40: score -= 20 * weight

        # MACD check
        if ind['macd_line'] > ind['signal_line']: score += 20 * weight
        elif ind['macd_line'] < ind['signal_line']: score -= 20 * weight

        # EMA check
        if current_price > ind['ema_short']: score += 20 * weight
        elif current_price < ind['ema_short']: score -= 20 * weight
        ...
        try:
            ...
            for tp_key, tp_info in take_profit_levels.items():
                if tp_info['active']:
                    ...
                    if tp_triggered:
                        ...
                        # Update position amount if tracking partial closes needed, or rely on fill events
                        # self.active_position['amount'] = remaining_amount # Simple update

            # Check if all TPs are hit (or handle full closure differently)
            if all(not tp['active'] for tp in take_profit_levels.values()):
                 logger.info("All Take Profit levels hit.")
                 # Optional: Close any tiny remaining amount or let trailing SL handle it
                 # if remaining_amount > self.connector.trading_rules[self.trading_pair].min_order_size:
                 #      self.close_position("All TPs hit")
            ...
            if trailing_start_price is not None:
                ...
                if should_trail:
                    ...
                    if side == 'buy':
                        # Trail below the highest price reached since trailing started
                        new_sl = highest_profit_price - sl_distance
                        # Ensure new SL is above initial SL and ideally above entry
                        new_sl = max(new_sl, initial_sl, entry_price * 1.001) # Ensure SL moves up
                    elif side == 'sell':
                        # Trail above the lowest price reached
                        new_sl = highest_profit_price + sl_distance
                        # Ensure new SL is below initial SL and ideally below entry
                    ...
                    if (side == 'buy' and new_sl > current_sl) or \
                       (side == 'sell' and new_sl < current_sl):
                         ...
                         current_sl = new_sl # Update for immediate check if needed
    ...
    def calculate_position_size(self) -> Optional[Decimal]:
        ...
        try:
            ...
            # Amount of quote to risk
            risk_amount_quote = quote_balance * Decimal(str(self.position_size_pct))

            # Get current price to estimate position size in base asset
            ...
            # Calculate base amount without leverage
            ...
            # Adjust for trading rules (min size, step size)
            ...
            if quantized_amount < trading_rules.min_order_size:
                ...
                return None
            # Check if affordable (though risk % should handle this)
            # cost = quantized_amount * current_price / Decimal(self.leverage)
    ...
    def place_order(self, side: str, price: float, amount: Decimal, order_type: OrderType = OrderType.LIMIT, position_action: PositionAction = PositionAction.OPEN):
        ...
        try:
            ...
            if order_id:
                ...
                # Store order details if needed for tracking pending entries/exits
            else:
    ...
    def close_position(self, reason: str):
        ...
        amount_to_close = self.active_position['amount'] # Close the full tracked amount
        ...
        # Reset active position immediately - rely on fill event for confirmation if needed
    ...
    def on_tick(self):
        ...
        # --- Rate Limiting Update Frequency ---
        if now - self.last_update_time < self.update_interval:
            # Only manage active position on faster ticks if needed
            if self.active_position:
                self.manage_active_position() # Check SL/TP/Trailing frequently
            return
        ...
        # Use asyncio.create_task if fetching takes too long and blocks the tick
        # For simplicity here, using await directly
        # success = await self.fetch_market_data()
        # if not success:
        #     logger.warning("Skipping tick due to data fetch failure.")
        #     return
        # self.calculate_indicators() # Calculate for all fetched TFs
        # self.calculate_market_regime()
        # self.calculate_indicator_scores() # Calculate primary score first

        # --- Signal Generation ---
        # final_signal_info = self.generate_final_signal()

        # --- Position Management (Check SL/TP/Trailing *before* new entry logic) ---
        if self.active_position:
            self.manage_active_position()
            # If position was closed by manage_active_position, self.active_position will be None now
            if self.active_position is None:
                 return # Don't try to enter a new position immediately after closing one
        ...
        #         tp_config = self.risk_params['take_profit']
        #         trailing_start_pct = self.risk_params.get('trailing_start')

        #         entry_price = price # Use signal price as estimated entry
        #         initial_sl_price = 0.0
        #         tp_levels = {}
        #         trailing_start_price = None

        #         if signal == 'buy':
        #             initial_sl_price = entry_price * (1 - sl_pct)
        #             if trailing_start_pct:
        #                  trailing_start_price = entry_price * (1 + trailing_start_pct)
        #             for tp_key, tp_info in tp_config.items():
        #                 tp_levels[tp_key] = {
        #                     'price': entry_price * (1 + tp_info['pct']),
        #                     'size_pct': tp_info['size'],
        #                     'active': True
        #                 }
        #         elif signal == 'sell':
        #              initial_sl_price = entry_price * (1 + sl_pct)
        ...
        #                  }
                ...
                # --- Place Entry Order ---
                # Using MARKET order for entry simplicity, can change to LIMIT
                # self.place_order(
                #     side=signal,
                #     price=entry_price, # Use current price for market order
                #     amount=position_size,
                #     order_type=OrderType.MARKET,
                #     position_action=PositionAction.OPEN
                # )

                # --- Store Active Position Info (tentatively) ---
                # This should ideally be confirmed/updated by the fill event
                # self.active_position = {
                #     'side': signal,
                #     'entry_price': entry_price, # Store estimate, update on fill
                #     'amount': position_size,    # Store estimate, update on fill
                #     'initial_sl': initial_sl_price,
                #     'current_sl': initial_sl_price, # Start SL at initial
                #     'tp_levels': tp_levels,
                #     'trailing_start_price': trailing_start_price,
                #     'highest_profit_price': entry_price, # Initialize peak price tracker
                #     'entry_time': now,
                #     'status': 'pending_entry' # Mark as pending until filled
                # }
                # logger.info(f"Attempting to open {signal.upper()} position. Est. Entry: {entry_price:.4f}, Size: {position_size:.6f}, SL: {initial_sl_price:.4f}")
    ...
    def did_fill_order(self, event: OrderFilledEvent):
        ...
        amount = event.amount
        price = event.price
        position_action = event.position # Hummingbot uses 'position' for PositionAction
        ...
        if self.active_position and self.active_position.get('status') == 'pending_entry':
             # Update entry details with actual fill info
             if (self.active_position['side'] == 'buy' and trade_type == TradeType.BUY) or \
                (self.active_position['side'] == 'sell' and trade_type == TradeType.SELL):
                 # TODO: Handle partial fills if entry order fills in multiple parts
                 self.active_position['entry_price'] = float(price)
                 self.active_position['amount'] = float(amount) # Use actual filled amount
                 self.active_position['status'] = 'active'
                 # Recalculate SL/TP based on actual entry price if significantly different?
                 logger.info(f"Active position entry confirmed/updated: Side={self.active_position['side']}, "
                             f"Entry={self.active_position['entry_price']:.4f}, Amount={self.active_position['amount']:.6f}")

        elif position_action == PositionAction.CLOSE:
             # Logic to confirm position closure or partial TP fills
             logger.info(f"Position close/TP fill event received for order {order_id}.")
             # If tracking partial TPs accurately, reduce active_position['amount'] here
             # If it was a full close (SL/manual/final TP), self.active_position might already be None
             # Add checks if necessary based on order IDs placed for SL/TP.
    ...
    def format_status(self) -> str:
        ...
        # lines.append(f"Scores: R:{self.indicator_scores['rsi']:.0f} M:{self.indicator_scores['macd']:.0f} E:{self.indicator_scores['ema']:.0f} "
        #             f"B:{self.indicator_scores['bbands']:.0f} V:{self.indicator_scores['volume']:.0f} SR:{self.indicator_scores['support_resistance']:.0f}")

        lines.append(f"S/R Levels: Supports={self.support_levels}, Resistances={self.resistance_levels}")
        # lines.append(f"Traps: VolΔ={self.trap_indicators['volume_delta']:.2f}, OrdImb={self.trap_indicators['order_imbalance']:.2f}, "
        #             f"SpreadΔ={self.trap_indicators['bid_ask_spread']:.2f}, WickRej={self.trap_indicators['wick_rejection']:.2f}")
    ...
    def on_stop(self):
        ...
        # self.cancel_all_orders() # Consider if you want to leave SL/TP orders open or close position
        if self.active_position:
             logger.warning("Strategy stopped with an active position. Manual intervention might be required.")
             # Optionally close the position on stop:
