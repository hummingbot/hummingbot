...
class PureMarketMakingStrategyAugmentedPurePython(StrategyPyBase):
    ...
    def get_mid_price(self):
        delegate = self._asset_price_delegate
    ...
    def inventory_skew_stats_data_frame(self) -> Optional[pd.DataFrame]:
        ...
        base_asset_amount, quote_asset_amount = self.get_adjusted_available_balance(self.active_orders)
    ...
    #    return self.execute_orders_proposal(proposal)

    def cancel_order(self, order_id: str):
        return StrategyBase.cancel_order(self, market_trading_pair_tuple=self._market_info, order_id=order_id)
    ...
    def start(self, clock: Clock, timestamp: float):
        ...
        if self._hanging_orders_enabled:
            ...
            restored_order_ids = self.track_restored_orders(self.market_info)
    ...
    @cython.locals(current_tick=cython.double, last_tick=cython.double, should_report_warnings=cython.bint)
    ...
    def get_adjusted_available_balance(self, orders: List) -> Tuple[Any, Any]:
        """
    ...
    def apply_order_levels_modifiers(self, proposal):
        self.apply_price_band(proposal)
        if self.moving_price_band_enabled:
            self.apply_moving_price_band(proposal)
        if self._ping_pong_enabled:
            self.apply_ping_pong(proposal)

    def apply_price_band(self, proposal):
    ...
    def apply_moving_price_band(self, proposal):
        price = self.get_price()
    ...
    def apply_ping_pong(self, proposal):
        self._ping_pong_warning_lines = []
    ...
    def apply_order_price_modifiers(self, proposal):
        if self._order_optimization_enabled:
            self.apply_order_optimization(proposal)

        if self._add_transaction_costs_to_orders:
            self.apply_add_transaction_costs(proposal)

    def apply_order_size_modifiers(self, proposal):
        if self._inventory_skew_enabled:
            self.apply_inventory_skew(proposal)

    def apply_inventory_skew(self, proposal):
        market = self._market_info.market

        base_balance, quote_balance = self.get_adjusted_available_balance(self.active_orders)
    ...
    def adjusted_available_balance_for_orders_budget_constraint(self):
        ...
        return self.get_adjusted_available_balance(all_non_hanging_orders)
    ...
    def apply_budget_constraint(self, proposal):
        ...
        for buy in proposal.buys:
            buy_fee = market.get_fee(self.base_asset, self.quote_asset, OrderType.LIMIT, TradeType.BUY,
                                     buy.size, buy.price)
    ...
    def filter_out_takers(self, proposal):
        market = self._market_info.market
    ...
    def apply_order_optimization(self, proposal):
        market = self._market_info.market
        ...
        if len(proposal.buys) > 0:
            ...
            for i, proposed in enumerate(proposal.buys):
                if self._split_order_levels_enabled:
                    proposal.buys[i].price = (market.quantize_order_price(self.trading_pair, lower_buy_price)
        ...
        if len(proposal.sells) > 0:
            ...
            for i, proposed in enumerate(proposal.sells):
                if self._split_order_levels_enabled:
                    proposal.sells[i].price = (market.quantize_order_price(self.trading_pair, higher_sell_price)
    ...
    def apply_add_transaction_costs(self, proposal):
        market = self._market_info.market
        for buy in proposal.buys:
            ...
            buy.price = market.quantize_order_price(self.trading_pair, price)
        for sell in proposal.sells:
            fee = market.get_fee(self.base_asset, self.quote_asset,
                                 self._limit_order_type, TradeType.SELL, sell.size, sell.price)
            price = sell.price * (Decimal(1) + fee.percent)
            sell.price = market.quantize_order_price(self.trading_pair, price)
    ...
    def is_within_tolerance(self, current_prices, proposal_prices):
    ...
    def cancel_active_orders(self, proposal):
        """
    ...
    def to_create_orders(self, proposal):
        non_hanging_orders_non_cancelled = [o for o in self.active_non_hanging_orders if
                                            not self._hanging_orders_tracker.is_potential_hanging_order(o)]
    ...
    def execute_orders_proposal(self, proposal):
        ...
        if len(proposal.buys) > 0:
            ...
            for idx, buy in enumerate(proposal.buys):
                ...
                if idx < number_of_pairs:
                    order = next(iter(o for o in self.active_orders if o.client_order_id == bid_order_id))
                    if order:
        ...
        if len(proposal.sells) > 0:
            ...
            for idx, sell in enumerate(proposal.sells):
                ...
                orders_created = True
                if idx < number_of_pairs:
                    order = next(iter(o for o in self.active_orders if o.client_order_id == ask_order_id))
                    if order:
