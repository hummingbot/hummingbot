...
class TwapTradeStrategy(StrategyPyBase):
    ...
    def process_market(self, market_info):
        ...
        if self._quantity_remaining > 0:
            ...
            elif (self.current_timestamp > self._previous_timestamp + self._order_delay_time) and (
                self._first_order is False
            ):
                self.logger().info(
                    "Current time: "
            ...
            if self.has_enough_balance(market_info, quantized_amount):
                if self._is_buy:
                    order_id = self.buy_with_specific_market(
                        market_info, amount=quantized_amount, order_type=OrderType.LIMIT, price=quantized_price
                    )
