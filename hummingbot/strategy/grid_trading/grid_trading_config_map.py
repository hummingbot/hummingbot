...
def maker_trading_pair_prompt():
    derivative = grid_trading_config_map.get("derivative").value
...
def order_amount_prompt() -> str:
    trading_pair = grid_trading_config_map["market"].value
...
def on_validate_price_source(value: str):
    if value != "external_market":
        grid_trading_config_map["price_source_derivative"].value = None
        grid_trading_config_map["price_source_market"].value = None
    if value != "custom_api":
        grid_trading_config_map["price_source_custom_api"].value = None
    else:
        grid_trading_config_map["price_type"].value = "custom"


def validate_price_type(value: str) -> Optional[str]:
    error = None
    price_source = grid_trading_config_map.get("price_source").value
...
def price_source_market_prompt() -> str:
    external_market = grid_trading_config_map.get("price_source_derivative").value
    return f'Enter the token trading pair on {external_market} >>> '


def validate_price_source_derivative(value: str) -> Optional[str]:
    if value == grid_trading_config_map.get("derivative").value:
...
def validate_price_source_market(value: str) -> Optional[str]:
    market = grid_trading_config_map.get("price_source_derivative").value
...
grid_trading_config_map = {
    "strategy":
        ConfigVar(key="strategy",
                  prompt=None,
                  default="grid_trading"),
    "derivative":
    ...
    "position_mode":
        ConfigVar(key="position_mode",
                  ...
                  prompt_on_new=True),
    "lower_bound":
        ConfigVar(key="lower_bound",
                  prompt="lower_bound of price",
                  type_str="decimal",
                  prompt_on_new=True,
                  validator=validate_price_floor_ceiling),
    "upper_bound":
        ConfigVar(key="upper_bound",
                  prompt="upper_bound of price",
                  type_str="decimal",
                  prompt_on_new=True,
                  validator=validate_price_floor_ceiling),

    "trading_direction":
        ConfigVar(key="trading_direction",
                  prompt="long, short or neutrual",
                  type_str="str",
                  default="long",
                  prompt_on_new=True),
    "max_amount":
        ConfigVar(key="max_amount",
                  prompt="total exposure",
                  type_str="decimal",
                  prompt_on_new=True,
                  validator=validate_price_floor_ceiling),
    "price_interval":
        ConfigVar(key="orderprice_interval_level_spread",
                  prompt="price interval of each grid",
                  type_str="decimal",
                  validator=validate_price_floor_ceiling,
                  prompt_on_new=True,
                  default=Decimal("1")),
    "size_of_order":
        ConfigVar(key="size_of_order",
                  prompt="size of each order",
                  type_str="decimal",
                  validator=validate_price_floor_ceiling,
                  #prompt_on_new=True,
                  default=Decimal("1")),
    "trigger_price":
        ConfigVar(key="trigger_price",
                  prompt="price to trigger strategy",
                  type_str="decimal",
                  default=Decimal(0),
                  validator=validate_price_floor_ceiling),
    #### alternatives unused.
    ...
    "order_level_spread":
        ConfigVar(key="order_level_spread",
                  ...
                  required_if=lambda: grid_trading_config_map.get("order_levels").value > 1,
                  type_str="decimal",
    ...
    "ask_order_optimization_depth":
        ConfigVar(key="ask_order_optimization_depth",
                  ...
                  required_if=lambda: grid_trading_config_map.get("order_optimization_enabled").value,
                  type_str="decimal",
    ...
    "price_type":
        ConfigVar(key="price_type",
                  ...
                  type_str="str",
                  required_if=lambda: grid_trading_config_map.get("price_source").value != "custom_api",
                  default="mid_price",
    ...
    "price_source_derivative":
        ConfigVar(key="price_source_derivative",
                  ...
                  required_if=lambda: grid_trading_config_map.get("price_source").value == "external_market",
                  type_str="str",
