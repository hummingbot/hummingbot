...
class DirectionalExchangeStrategyBase(ScriptStrategyBase):
    """
    Base class to create directional strategies using the PositionExchangeExecutor.
    ...
    Attributes:
        ...
        active_executors (List[PositionExchangeExecutor]): List of currently active position executors.
        stored_executors (List[PositionExchangeExecutor]): List of closed position executors that have been stored.
    ...
    active_executors: List[PositionExchangeExecutor] = []
    stored_executors: List[PositionExchangeExecutor] = []
    ...
    def get_active_executors(self) -> List[PositionExchangeExecutor]:
        return [signal_executor for signal_executor in self.active_executors
                if not signal_executor.is_closed]

    def get_closed_executors(self) -> List[PositionExchangeExecutor]:
        return [signal_executor for signal_executor in self.active_executors
    ...
    def on_tick(self):
        ...
        if self.max_active_executors_condition and self.all_candles_ready and self.time_between_signals_condition:
            ...
            if position_config:
                signal_executor = PositionExchangeExecutor(
                    strategy=self,
    ...
    def get_position_config(self):
        ...
        if signal == 0:
            ...
            position_config = PositionExchangeExecutorConfig(
