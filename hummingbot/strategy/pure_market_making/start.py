...
def start(self):
    ...
    try:
        ...
        inventory_target_base_pct = (
            0
            if c_map.get("inventory_target_base_pct").value is None
            else c_map.get("inventory_target_base_pct").value / Decimal("100")
        )
        ...
        bid_order_level_spreads = convert_decimal_string_to_list(c_map.get("bid_order_level_spreads").value)
        ask_order_level_spreads = convert_decimal_string_to_list(c_map.get("ask_order_level_spreads").value)
        bid_order_level_amounts = convert_decimal_string_to_list(c_map.get("bid_order_level_amounts").value)
        ask_order_level_amounts = convert_decimal_string_to_list(c_map.get("ask_order_level_amounts").value)
        if split_order_levels_enabled:
            buy_list = [
                ["buy", spread, amount] for spread, amount in zip(bid_order_level_spreads, bid_order_level_amounts)
            ]
            sell_list = [
                ["sell", spread, amount] for spread, amount in zip(ask_order_level_spreads, ask_order_level_amounts)
            ]
        ...
        if price_source == "external_market":
            ...
            asset_price_delegate = APIAssetPriceDelegate(
                self.markets[exchange], price_source_custom_api, custom_api_update_interval
            )
