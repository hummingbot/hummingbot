...
LIGHTER_LIMIT_ID = "LIGHTER_PERP_CANDLES_LIMIT"
LIGHTER_LIMIT = 24000
...
class LighterPerpetualCandles(CandlesBase):
    ...
    async def initialize_exchange_data(self):
        ...
        for ob in response.get("order_books", []):
            if ob.get("market_type") == "perp" and ob.get("symbol") == base_asset:
                self._market_id = ob["market_id"]
                return
        raise ValueError(
            f"Perpetual market '{base_asset}' not found in Lighter order books."
        )
