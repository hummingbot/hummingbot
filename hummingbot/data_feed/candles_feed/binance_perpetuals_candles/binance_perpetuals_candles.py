...
class BinancePerpetualsCandles(NetworkBase):
    _bcf_logger: Optional[HummingbotLogger] = None
    _binance_candles_shared_instance: "BinancePerpetualsCandles" = None
    ...
    async def fill_historical_candles(self):
        while not self.is_ready:
            ...
            try:
                ...
                missing_records = self._candles.maxlen - len(self._candles)
                if len(candles) == 1 and missing_records > len(candles):
                    raise Exception(f"There is no enough historical information to fill this candle {self.name}. Missing records: {missing_records}")
                self._candles.extendleft(candles[-(missing_records + 1):-1][::-1])
            except asyncio.CancelledError:
                raise
            except Exception as e:
                self.logger().exception(
                    f"Unexpected error occurred when getting historical klines. {e}. Retrying in 1 seconds...",
                )
