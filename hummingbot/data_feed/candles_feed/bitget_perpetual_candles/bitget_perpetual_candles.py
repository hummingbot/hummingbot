...
class BitgetPerpetualCandles(CandlesBase):
    ...
    @property
    def name(self):
        return f"bitget_perpetual_{self._trading_pair}"
    ...
    def health_check_url(self):
        self.rest_url + CONSTANTS.HEALTH_CHECK_ENDPOINT
    ...
    def __normalize_interval(self, interval: str) -> str:
    ...
    def _get_rest_candles_params(self,
        ...
        params = {
            ...
            "granularity": self.__normalize_interval(self.interval),
            "limit": limit,
            #coin-futures   usdc-futures    susdt-futures   scoin-futures   susdc-futures
    ...
    def _parse_rest_candles(self, data: dict, end_time: Optional[int] = None) -> List[List[float]]:
        return [
            [self.ensure_timestamp_in_seconds(row[0]), row[1], row[2], row[3], row[4], row[5], row[6],
             #TODO
             1000, 1000, 1000]
    ...
    def ws_subscription_payload(self):
        candle_params = [f"{self._ex_trading_pair.lower()}@kline_{self.__normalize_interval(self.interval)}"]
        payload = {
            "method": "SUBSCRIBE",
