"""``hbot order-book`` — bid/ask depth for a pair on a connector.

Read-only public market data, fetched ad-hoc (no running bot): builds a connector, waits for the
order book, reads the top-N levels, tears down. The pair is fuzzy-matched against the connector's
...
async def _run(ccm, connector: str, pair: str, lines: int, timeout: float) -> Tuple[List[dict], str, List[str]]:
...
def order_book(
    ...
    """Show the order-book depth (top levels of bids and asks) for a pair on a connector."""
    ...
    try:
        rows, matched, alts = asyncio.run(_run(ccm, connector, pair, lines, timeout))
    except asyncio.TimeoutError:
        fail("timed out waiting for the order book", ExitCode.TIMEOUT)

    title = f"book {matched} ({connector})"
