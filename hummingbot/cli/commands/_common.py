...
def one_type(v1: bool, v2: bool, controller: bool, json_output: bool, required: bool) -> Optional[str]:
    ...
    if len(chosen) > 1:
        fail(f"use only one of {names}", ExitCode.CONFIG_ERROR, json_output=json_output)
    if required and not chosen:
        fail(f"specify one of {names}", ExitCode.CONFIG_ERROR, json_output=json_output)
    return chosen[0] if chosen else None


def read_json_object_from_stdin(json_output: bool) -> dict:
...
def resolve_db_for_command(name: Optional[str], json_output: bool) -> Tuple[Path, Optional[str], bool]:
    ...
    if not bot.exists():
        fail("no bot has been started (pass a name to view a past bot)", ExitCode.NOT_FOUND, json_output=json_output)
