"""``hbot rules`` — show a connector's trading rules for a pair.
...
async def _run(ccm, connector: str, pair: str, timeout: float) -> Tuple[dict, str, List[str]]:
    ...
    _symbol_map, rules, _conn = await load_universe(ccm, connector, timeout)
    if not rules:
        fail(f"{connector} returned no trading rules", ExitCode.ERROR)
...
def rules(
    ...
    """Show a connector's trading rules for a pair (min size, min notional, tick/step sizes)."""
    ...
    try:
        rule, matched, alts = asyncio.run(_run(ccm, connector, pair, timeout))
    except asyncio.TimeoutError:
        fail("network timeout fetching trading rules", ExitCode.TIMEOUT)

    title = f"rules {matched} ({connector})"
