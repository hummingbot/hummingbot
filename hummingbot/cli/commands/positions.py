"""``hbot positions`` — open positions on a perpetual connector.
...
async def _fetch(ccm, connector: str, timeout: float) -> Tuple[Optional[List[dict]], Optional[str]]:
    from hummingbot.client.config.security import Security
    from hummingbot.user.user_balances import UserBalances
    keys = Security.api_keys(connector)
    if not keys:
        return None, f"no keys for '{connector}' — run `hbot connect {connector}` first"
    market = UserBalances.connect_market(connector, ccm, **keys)
    if market is None or not hasattr(market, "account_positions"):
        return None, f"'{connector}' is not a perpetual connector (no positions)"
...
def positions(
    connector: str = typer.Argument(..., help="Perpetual connector, e.g. hyperliquid_perpetual."),
    password_stdin: bool = typer.Option(
        False, "--password-stdin", help="Read the keystore password from stdin (else $HBOT_PASSWORD or a prompt)."),
) -> None:
    """Show your open positions on a perpetual connector."""
    ...
    try:
        rows, err = asyncio.run(_fetch(ccm, connector, timeout))
    ...
    if not rows:
        echo(f"## positions {connector}\n\n_(no open positions)_")
    else:
        echo(render_table(rows, title=f"positions {connector}",
