...
class HashkeyPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        for rule in trading_pair_rules:
            if not rule["symbol"].endswith("PERPETUAL"):
                continue
    ...
    async def trading_pair_associated_to_exchange_symbol(self, symbol: str):
        return symbol.rstrip("-PERPETUAL")

    async def exchange_symbol_associated_to_pair(self, trading_pair: str):
        trading_pair = "".join(trading_pair.split("-"))
        return f"{trading_pair}-PERPETUAL"
    ...
    async def _fetch_last_fee_payment(self, trading_pair: str) -> Tuple[int, Decimal, Decimal]:
        ...
        if not result:
            ...
            timestamp: int = int(pd.Timestamp(result["nextSettleTime"], tz="UTC").timestamp())
