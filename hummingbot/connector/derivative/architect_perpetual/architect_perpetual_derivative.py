...
class ArchitectPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict) -> List[TradingRule]:
        # Expected format (mocked in tests):
        # {"symbols": [{"symbol": "BTC-USDT", "base": "BTC", "quote": "USDT",
        #               "min_qty": "0.001", "min_price": "0.1", "min_notional": "5"}]}
        rules = []
        for symbol_info in exchange_info_dict.get("symbols", []):
            trading_pair = symbol_info.get("trading_pair") or symbol_info.get("symbol")
            if trading_pair is None:
                continue
            min_qty = Decimal(str(symbol_info.get("min_qty", "0")))
            min_price = Decimal(str(symbol_info.get("min_price", "0")))
            min_notional = Decimal(str(symbol_info.get("min_notional", "0")))
            base = symbol_info.get("base") or trading_pair.split("-")[0]
            quote = symbol_info.get("quote") or trading_pair.split("-")[-1]
            rules.append(
                TradingRule(
    ...
    async def _update_time_synchronizer(self, *args, **kwargs):
        # Best-effort no-op. In production this should query CONSTANTS.SERVER_TIME_URL.
        return
