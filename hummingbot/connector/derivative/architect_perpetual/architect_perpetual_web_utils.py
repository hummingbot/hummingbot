...
def public_rest_url(path_url: str, domain: str = CONSTANTS.DOMAIN) -> str:
    base_url = CONSTANTS.TESTNET_REST_BASE_URL if domain == CONSTANTS.TESTNET_DOMAIN else CONSTANTS.REST_BASE_URL
...
def wss_url(domain: str = CONSTANTS.DOMAIN) -> str:
    return CONSTANTS.TESTNET_WS_BASE_URL if domain == CONSTANTS.TESTNET_DOMAIN else CONSTANTS.WS_BASE_URL
...
def build_api_factory(
    ...
    api_factory = WebAssistantsFactory(
        ...
        ] if time_synchronizer else None,
    )
    return api_factory
