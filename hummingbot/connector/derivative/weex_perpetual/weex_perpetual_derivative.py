...
import hummingbot.connector.derivative.weex_perpetual.weex_perpetual_utils as weex_utils
from hummingbot.connector.derivative.weex_perpetual import weex_perpetual_web_utils as web_utils
from hummingbot.connector.derivative.weex_perpetual.weex_perpetual_api_order_book_data_source import (
    WeexPerpetualAPIOrderBookDataSource,
)
from hummingbot.connector.derivative.weex_perpetual.weex_perpetual_auth import WeexPerpetualAuth
from hummingbot.connector.derivative.weex_perpetual.weex_perpetual_user_stream_data_source import (
    WeexPerpetualUserStreamDataSource,
)
...
class WeexPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def __init__(
        ...
        weex_perpetual_api_key: str = None,
        weex_perpetual_secret_key: str = None,
        trading_pairs: Optional[List[str]] = None,
        trading_required: bool = True,
        domain: str = CONSTANTS.DEFAULT_DOMAIN,
    ):

        self.weex_perpetual_api_key = weex_perpetual_api_key
        self.weex_perpetual_secret_key = weex_perpetual_secret_key
        self._trading_required = trading_required
        self._trading_pairs = trading_pairs
        self._domain = domain
        self._last_trade_history_timestamp = None
        self._real_time_balance_update = False  # Remove this once weex enables available balance again through ws
    ...
    @property
    def authenticator(self) -> WeexPerpetualAuth:
        return WeexPerpetualAuth(self.weex_perpetual_api_key, self.weex_perpetual_secret_key, self._time_synchronizer)
    ...
    def supported_position_modes(self) -> List[PositionMode]:
        if all(weex_utils.is_linear_perpetual(tp) for tp in self._trading_pairs):
            return [PositionMode.ONEWAY, PositionMode.HEDGE]
        elif all(not weex_utils.is_linear_perpetual(tp) for tp in self._trading_pairs):
            # As of Weex API v5, we only support ONEWAY mode for non-linear perpetuals
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        data = {
            "category": "linear" if weex_utils.is_linear_perpetual(tracked_order.trading_pair) else "inverse",
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        data = {
            "category": "linear" if weex_utils.is_linear_perpetual(trading_pair) else "inverse",
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return WeexPerpetualAPIOrderBookDataSource(
            self.trading_pairs,
            connector=self,
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return WeexPerpetualUserStreamDataSource(
            auth=self._auth,
            api_factory=self._web_assistants_factory,
            domain=self._domain,
        )
    ...
    async def _request_order_fills(self, order: InFlightOrder) -> Dict[str, Any]:
        ...
        body_params = {
            "category": "linear" if weex_utils.is_linear_perpetual(order.trading_pair) else "inverse",
    ...
    async def _process_account_position_event(self, position_msg: Dict[str, Any]):
        ...
        # Trigger balance update because Weex doesn't have balance updates through the websocket
        ...
        for instrument in instrument_info_dict:
            try:
                ...
                if exchange_symbol in symbol_map:
                    ...
                    is_linear = weex_utils.is_linear_perpetual(trading_pair)
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(weex_utils.is_exchange_information_valid, exchange_info):
    ...
    async def _fetch_last_fee_payment(self, trading_pair: str) -> Tuple[int, Decimal, Decimal]:
        ...
        if weex_utils.is_linear_perpetual(trading_pair):
