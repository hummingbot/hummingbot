...
class PhemexPerpetualAPIOrderBookDataSource(PerpetualAPIOrderBookDataSource):
    ...
    def __init__(
        ...
        self._diff_messages_queue_key = "book"
        self._funding_info_messages_queue_key = "market24h"
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot_msg: OrderBookMessage = OrderBookMessage(OrderBookMessageType.SNAPSHOT, {
            ...
            "bids": snapshot_response["book"]["bids"],
            "asks": snapshot_response["book"]["asks"]
    ...
    def _channel_originating_message(self, event_message: Dict[str, Any]) -> str:
        ...
        elif self._funding_info_messages_queue_key in event_message:
            channel = self._funding_info_messages_queue_key
        return channel

    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        timestamp: float = time.time()
        raw_message["symbol"] = await self._connector.trading_pair_associated_to_exchange_symbol(
            raw_message["symbol"])
        if raw_message["type"] == "incremental":
            book = raw_message["book"]
    ...
    async def _parse_funding_info_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        ...
        data: Dict[str, Any] = raw_message["market24h"]
        ...
        funding_info = FundingInfoUpdate(
            ...
            next_funding_utc_timestamp=self._next_funding_time(),
            rate=Decimal(data["predFundingRate"]),
        )
