...
class RubinPerpetualRESTPreProcessor(RESTPreProcessorBase):
...
def public_rest_url(path_url: str, domain: str = CONSTANTS.DEFAULT_DOMAIN) -> str:
    ...
    :param domain: the rubin domain to connect to ("exchange" or "us"). The default value is "exchange"
    :return: the full URL to the endpoint
    """
    return CONSTANTS.RUBIN_REST_URL + path_url
...
def build_api_factory(
    ...
    api_factory = WebAssistantsFactory(
        throttler=throttler,
        rest_pre_processors=[
            RubinPerpetualRESTPreProcessor(),
        ],
    )
    return api_factory
