...
from hummingbot.connector.derivative.rubin_perpetual.data_sources.tx import SigningCfg, Transaction


class RubinPerpetualV4Client:
    ...
    def __init__(
        ...
        self.stubTendermint = tendermint_query_grpc.ServiceStub(
            grpc_client
        )
    ...
    async def latest_block(self) -> tendermint_query.GetLatestBlockResponse:
        ...
        '''
        return await self.stubTendermint.GetLatestBlock(
    ...
    async def cancel_order(
        ...
        subaccount_id = SubaccountId(owner=self._rubin_chain_address, number=self._subaccount_num)
        order_id = OrderId(
    ...
    async def query_account(self):
        request = QueryAccountRequest(address=self._rubin_chain_address)
