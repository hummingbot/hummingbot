...
ORDER_CANCELED_EVENT = MarketEvent.OrderCanceled
...
class DydxPerpetualDerivative(ExchangeBase, PerpetualTrading):
    ...
    async def cancel_order(self, client_order_id: str):
        ...
        if in_flight_order is None:
            ...
            self.trigger_event(ORDER_CANCELED_EVENT, cancelation_event)
            return False
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancelationResult]:
        ...
        order_status = {o.client_order_id: o.is_done for o in cancelation_queue.values()}
        ...
        for order_id, in_flight in cancelation_queue.items():
            try:
    ...
    def _issue_order_events(self, tracked_order: DydxPerpetualInFlightOrder):
        ...
        for (market_event, new_amount, new_price, new_fee) in issuable_events:
            if market_event == MarketEvent.OrderCanceled:
                ...
                self.trigger_event(
                    ORDER_CANCELED_EVENT, OrderCanceledEvent(self.current_timestamp, tracked_order.client_order_id)
                )
    ...
    async def _update_order_status(self):
        ...
        for client_order_id, tracked_order in tracked_orders.items():
            ...
            try:
                ...
                # is reasonably old, mark the orde as canceled
