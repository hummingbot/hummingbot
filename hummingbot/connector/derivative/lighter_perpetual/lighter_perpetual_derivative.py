...
class LighterPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def __init__(
        ...
        lighter_perpetual_api_secret: Optional[str] = None,
        lighter_perpetual_api_key_private_key: Optional[str] = None,
        use_vault: bool = False,
        lighter_perpetual_api_key: Optional[str] = None,
        ...
        lighter_perpetual_additional_api_private_keys: Optional[Dict[int, str]] = None,
        lighter_perpetual_eth_private_key: Optional[str] = None,
    ):
        self.lighter_perpetual_api_key = lighter_perpetual_api_key
        self.lighter_perpetual_secret_key = (
            lighter_perpetual_api_key_private_key or lighter_perpetual_api_secret
        )
        ...
        additional_keys = dict(lighter_perpetual_additional_api_private_keys or {})
        inferred_max = (
            max([lighter_perpetual_api_key_index] + list(additional_keys.keys()))
            if additional_keys
            else lighter_perpetual_api_key_index
        )
        self._max_api_key_index = lighter_perpetual_max_api_key_index or inferred_max
        self._additional_private_keys = additional_keys
        self._eth_private_key = lighter_perpetual_eth_private_key
    ...
    def authenticator(self) -> Optional[LighterPerpetualAuth]:
        ...
        return LighterPerpetualAuth(
            domain=self._domain,
            api_key_private_key=self.lighter_perpetual_secret_key,
            account_index=self._account_index,
            api_key_index=self._api_key_index,
            additional_api_private_keys=self._additional_private_keys,
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        if (
            ...
            self.logger().debug(
                f"The order {order_id} does not exist on Lighter Perpetuals. "
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        coin = symbol.split("-")[0]
        market_index = self.coin_to_asset[coin]

        signed_payload, signing_error = await self._auth.sign_create_order(
            market_index=market_index,
            client_order_index=self._client_order_index(order_id),
            base_amount=str(amount),
            price=str(price),
            is_ask=trade_type is TradeType.SELL,
            order_type=self._map_order_type(order_type),
            time_in_force=self._map_time_in_force(order_type),
            reduce_only=position_action == PositionAction.CLOSE,
            trigger_price=0,
        )
        if signing_error:
            raise IOError(f"Error signing order {order_id}: {signing_error}")

        request_payload = self._decorate_signed_payload(
            signed_payload,
            order_id=order_id,
    ...
    @staticmethod
    def _client_order_index(order_id: str) -> int:
        try:
            return abs(int(order_id, 16)) & 0x7FFFFFFFFFFFFFFF
        except ValueError:
            return abs(hash(order_id)) & 0x7FFFFFFFFFFFFFFF
    ...
    @staticmethod
    def _map_time_in_force(order_type: OrderType) -> int:
        if order_type is OrderType.MARKET:
            return 0
    ...
    def _decorate_signed_payload(
        ...
        action = payload_dict.setdefault("action", {})
        orders = action.setdefault("orders", [{}])
        if not orders:
            orders.append({})
        order_entry = orders[0]
        order_entry["asset"] = market_index
        order_entry["c"] = order_id
        order_entry["b"] = is_buy
        order_entry["s"] = str(amount)
        order_entry["p"] = str(price)
            ...
            try:
                all_fills_response = await self._api_post(
                    ...
                    data={
                        ...
                        "user": self.lighter_perpetual_api_key,
                    },
                )
