...
class BackpackDerivative(PerpetualDerivativePyBase):
    web_utils = web_utils
    ...
    def __init__(
        ...
        self.backpack_passphrase = backpack_passphrase
    ...
    def authenticator(self) -> BackpackAuth:
        return BackpackAuth(self.backpack_api_key,
                                   self.backpack_passphrase,
                                   self.backpack_secret_key,
    ...
    def supported_position_modes(self):
        # Backpack only supports ONEWAY mode for all perpetuals, no hedge mode
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        ...
        for symbol_data in filter(backpack_utils.is_exchange_information_valid, exchange_info):
            try:
                mapping[symbol_data["symbol"]] = combine_to_hb_trading_pair(base=symbol_data["baseCurrency"],
                                                                            quote=symbol_data["quoteCurrency"])
            except ValueDuplicationError:
                # We can safely ignore this, Backpack API returns a duplicate entry for XBT-USDT
                pass
    ...
    async def _trading_pair_position_mode_set(self, mode: PositionMode, trading_pair: str) -> Tuple[bool, str]:
        ...
        if mode == PositionMode.HEDGE:
            msg = "Backpack Perpetuals don't allow for a position mode change."
            success = False
        else:
            msg = "Success"
            success = True
