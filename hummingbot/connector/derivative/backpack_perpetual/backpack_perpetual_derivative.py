...
from hummingbot.connector.derivative.backpack_perpetual.backpack_perpetual_constants import MarginMode
...
class BackpackPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    async def _update_trading_fees(self):
        ...
        for symbol_details in symbol_data:
            if backpack_perpetual_utils.is_exchange_information_valid(exchange_info=symbol_details):
    ...
    async def _process_account_position_event(self, position_entries: List[Dict[str, Any]]):
        ...
        # Backpack sends position events as snapshots.
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(
        ...
        for symbol_data in exchange_info:
            if backpack_perpetual_utils.is_exchange_information_valid(exchange_info=symbol_data):
                try:
    ...
    async def _format_trading_rules(
        ...
        for rule in exchange_info_dict:
            if backpack_perpetual_utils.is_exchange_information_valid(exchange_info=rule):
                try:
                    trading_pair = await self.trading_pair_associated_to_exchange_symbol(
                        symbol=rule["symbol"]
                    )
