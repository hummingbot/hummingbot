...
async def get_current_server_time(
    ...
    server_time = float(response.get("timestamp", response.get("server_time", 0)))
    return server_time


def is_exchange_information_valid(rule: Dict[str, Any]) -> bool:
    return rule.get("is_active", True) and rule.get("instrument_type") == "PERPETUAL"
