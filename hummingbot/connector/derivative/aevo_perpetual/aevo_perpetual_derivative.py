...
from hummingbot.connector.derivative.aevo_perpetual.aevo_perpetual_constants import MarginMode
...
class AevoPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def __init__(
        ...
        aevo_perpetual_secret_key: str = None,
        aevo_perpetual_passphrase: str = None,
        trading_pairs: Optional[List[str]] = None,
        trading_required: bool = True,
    ) -> None:

        self.aevo_perpetual_api_key = aevo_perpetual_api_key
        self.aevo_perpetual_secret_key = aevo_perpetual_secret_key
        self.aevo_perpetual_passphrase = aevo_perpetual_passphrase
    ...
    def authenticator(self) -> AevoPerpetualAuth:
        return AevoPerpetualAuth(
            api_key=self.aevo_perpetual_api_key,
            secret_key=self.aevo_perpetual_secret_key,
            passphrase=self.aevo_perpetual_passphrase,
    ...
    async def _update_trading_fees(self):
        ...
        for symbol_details in symbol_data:
            if aevo_perpetual_utils.is_exchange_information_valid(exchange_info=symbol_details):
    ...
    async def _process_account_position_event(self, position_entries: List[Dict[str, Any]]):
        ...
        # Aevo sends position events as snapshots.
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(
        ...
        for symbol_data in exchange_info:
            if aevo_perpetual_utils.is_exchange_information_valid(exchange_info=symbol_data):
                try:
    ...
    async def _format_trading_rules(
        ...
        for rule in exchange_info_dict:
            if aevo_perpetual_utils.is_exchange_information_valid(exchange_info=rule):
                try:
                    trading_pair = await self.trading_pair_associated_to_exchange_symbol(
                        symbol=rule["symbol"]
                    )
