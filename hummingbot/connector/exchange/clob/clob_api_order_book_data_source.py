...
class CLOBAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_all_mid_prices(domain: str = CONSTANTS.DEFAULT_DOMAIN) -> Dict[str, Decimal]:
        ...
        for record in resp_json:
            try:
                pair = await CLOBAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
    ...
    async def exchange_symbol_associated_to_pair(
        ...
        """
        symbol_map = await CLOBAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=domain,
            api_factory=api_factory,
            throttler=throttler,
            time_synchronizer=time_synchronizer)
    ...
    async def fetch_trading_pairs(
        ...
        """
        mapping = await CLOBAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=domain,
            throttler=throttler,
            api_factory=api_factory,
            time_synchronizer=time_synchronizer,
        ...
        while True:
            try:
                ...
                if "result" in json_msg:
                    continue
                trading_pair = await CLOBAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
                    symbol=json_msg["s"],
                    domain=self._domain,
