...
class CoinsxyzAPIUserStreamDataSource:
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        """
        Create and return a connected WebSocket assistant for user data.
        """
        async with self._connection_lock:
            try:
                if self._ws_assistant:
                    try:
                        await asyncio.wait_for(self._ws_assistant.disconnect(), timeout=5.0)
                    except Exception:
                        pass
                    finally:
    ...
    async def _process_websocket_messages(self, websocket_assistant: WSAssistant, message_queue: asyncio.Queue):
        ...
        try:
            ...
            self.logger().error(f"Error in WebSocket message iteration: {e}")
            raise
        ...
        while not self._shutdown_event.is_set():
            ...
            try:
                ...
                if "1006" in error_msg:
                    ...
                    self.logger().warning(f"Connection timeout (attempt {self._reconnect_attempts}): {e}")
                else:
