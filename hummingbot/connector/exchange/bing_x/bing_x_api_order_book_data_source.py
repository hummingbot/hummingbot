...
class BingXAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _subscribe_channels(self, ws: WSAssistant):
        ...
        try:
            for trading_pair in self._trading_pairs:
                trade_payload = {"id": "trade", "dataType": trading_pair + "@trade"}
                subscribe_trade_request: WSJSONRequest = WSJSONRequest(payload=trade_payload)

                depth_payload = {"id": "depth", "dataType": trading_pair + "@depth"}
    ...
    async def subscribe_to_trading_pair(self, trading_pair: str) -> bool:
        ...
        try:
            ...
            trade_payload = {"id": f"trade_{subscribe_id}", "dataType": trading_pair + "@trade"}
            subscribe_trade_request: WSJSONRequest = WSJSONRequest(payload=trade_payload)

            depth_payload = {"id": f"depth_{subscribe_id}", "dataType": trading_pair + "@depth"}
            subscribe_orderbook_request: WSJSONRequest = WSJSONRequest(payload=depth_payload)
    ...
    async def unsubscribe_from_trading_pair(self, trading_pair: str) -> bool:
        ...
        try:
            ...
            trade_payload = {"id": f"unsub_trade_{subscribe_id}", "dataType": trading_pair + "@trade", "event": "unsub"}
            unsubscribe_trade_request: WSJSONRequest = WSJSONRequest(payload=trade_payload)

            depth_payload = {"id": f"unsub_depth_{subscribe_id}", "dataType": trading_pair + "@depth", "event": "unsub"}
