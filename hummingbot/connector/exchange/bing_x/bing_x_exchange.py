...
class BingXExchange(ExchangePyBase):
    ...
    def quantize_order_amount(self, trading_pair: str, amount: Decimal) -> Decimal:
        ...
        step_size = Decimal(trading_rule.min_base_amount_increment)

        if amount % step_size != 0:
            return ((amount // step_size) + 1) * step_size
        else:
            return (amount // step_size) * step_size
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        self.logger().warning(f"Failed to cancel order {order_id}. Response: {cancel_result}")
        return False
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        for rule in trading_pair_rules:
            try:
                ...
                last_traded_price = await self._get_last_traded_price(trading_pair)
