...
class PolkadexDataSource:
    ...
    async def cancel_order(self, order: InFlightOrder, market_symbol: str, timestamp: float) -> OrderState:
        ...
        # TODO (dmitry): do we need such logic in PolkadexExchange.cancel_expired_orders
        try:
    ...
    # TODO (dmitry): functionality of this function is the same as `_place_order_cancel`
    async def place_expired_order_cancel(self, order_id: str, market_symbol: str):
        cancel_request = self._runtime_config.create_scale_object("H256").encode(order_id)
