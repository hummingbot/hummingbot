...
from hummingbot.connector.exchange.asterdex import (
    asterdex_constants as CONSTANTS,
    asterdex_utils as utils,
...
class AsterdexExchange(ExchangePyBase):
    ...
    def authenticator(self):
        return AsterdexAuth(self.asterdex_api_key, self.asterdex_secret_key)
    ...
    async def get_all_pairs_prices(self) -> Dict[str, Any]:
        ...
        It returns the response of the exchange (expected to be used by the AsterDex RateSource for the RateOracle)
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return AsterdexAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://asterdex.github.io/asterdex-pro-api/#channel-order-and-balance
                ...
                # We need to use the offline balance estimation for AsterDex
    ...
    async def _all_trade_updates_for_order(self, order: InFlightOrder) -> List[TradeUpdate]:
        # AsterDex does not have an endpoint to retrieve trades for a particular order
