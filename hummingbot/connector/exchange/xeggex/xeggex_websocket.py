...
from hummingbot.connector.exchange.xeggex.xeggex_utils import RequestId, XeggexAPIError
...
class XeggexWebsocket(RequestId):
    ...
    def __init__(self,
                 auth: Optional[XeggexAuth] = None):
        self._auth: Optional[XeggexAuth] = auth
    ...
    async def connect(self):
        ...
        if self._isPrivate:
            ...
            if json_msg.get("result") is not True:
                ...
                raise XeggexAPIError({"error": f"Failed to authenticate to websocket - {err_msg}."})
