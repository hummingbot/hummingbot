...
class BybitAPIUserStreamDataSource(UserStreamTrackerDataSource):
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        await asyncio.wait_for(self._process_ws_messages(ws=ws, output=output),
    ...
    async def _authenticate_connection(self, ws):
        """
    ...
    async def _subscribe_channels(self, ws):
        """
        Subscribes to the balance, trades and fill events through the provided websocket connection.
        :param ws: the websocket assistant used to connect to the exchange
        """
        payload = {
            "op": "subscribe",
            "args": ["outboundAccountInfo", "executionReport", "ticketInfo"]
        }
        subscribe_message: WSRequest = WSRequest(payload=payload)
