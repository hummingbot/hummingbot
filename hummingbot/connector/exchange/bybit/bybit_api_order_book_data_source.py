...
class BybitAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    def __init__(self, trading_pairs: List[str] = None, domain: Optional[str] = None):
    ...
    async def get_last_traded_prices(cls, trading_pairs: List[str], domain: Optional[str] = None) -> Dict[str, float]:
        ...
        async with aiohttp.ClientSession() as client:
            async with client.get(f"{endpoint_url}") as response:
                if response.status == 200:
                    resp_json = await response.json()
                    if "result" in resp_json:
                        for token_pair_info in resp_json["result"]:
                            pass
                            # token_pair = f"{token_pair_info[:]}"
                            # if token_pair_info["market_id"] in trading_pairs:
