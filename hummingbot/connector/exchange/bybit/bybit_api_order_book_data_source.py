...
class BybitAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_all_mid_prices(domain: str = CONSTANTS.DEFAULT_DOMAIN) -> Dict[str, Decimal]:
        ...
        for record in resp_json:
            try:
                pair = await BybitAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
    ...
    async def _init_trading_pair_symbols(
        ...
        try:
            ...
            # for symbol_data in filter(bybit_utils.is_exchange_information_valid, data["symbols"]):
