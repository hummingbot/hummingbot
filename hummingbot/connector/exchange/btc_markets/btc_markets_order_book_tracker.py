...
class BtcMarketsOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self, trading_pairs: Optional[List[str]] = None,):
        super().__init__(BtcMarketsAPIOrderBookDataSource(trading_pairs), trading_pairs)
        ...
        self._order_books: Dict[str, BtcMarketsOrderBook] = {}
        self._saved_message_queues: Dict[str, Deque[BtcMarketsOrderBookMessage]] = \
            defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, BtcMarketsActiveOrderTracker] = defaultdict(BtcMarketsActiveOrderTracker)
    ...
    async def _track_single_book(self, trading_pair: str):
        ...
        """
        past_diffs_window: Deque[BtcMarketsOrderBookMessage] = deque()
        self._past_diffs_windows[trading_pair] = past_diffs_window

        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: BtcMarketsOrderBook = self._order_books[trading_pair]
        active_order_tracker: BtcMarketsActiveOrderTracker = self._active_order_trackers[trading_pair]
        ...
        while True:
            try:
                message: BtcMarketsOrderBookMessage = None
                saved_messages: Deque[BtcMarketsOrderBookMessage] = self._saved_message_queues[trading_pair]
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    past_diffs: List[BtcMarketsOrderBookMessage] = list(past_diffs_window)
