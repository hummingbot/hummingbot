...
class FmfwOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
                 ...
                 auth: FmfwAuth = None):
        super().__init__(FmfwAPIOrderBookDataSource(trading_pairs, auth), trading_pairs)
        self._auth = auth
        self._order_book_diff_stream: asyncio.Queue = asyncio.Queue()
        self._order_book_snapshot_stream: asyncio.Queue = asyncio.Queue()
        self._ev_loop: asyncio.BaseEventLoop = asyncio.get_event_loop()
        self._saved_message_queues: Dict[str, Deque[FmfwOrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, FmfwActiveOrderTracker] = defaultdict(FmfwActiveOrderTracker)
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ob_message: FmfwOrderBookMessage = await self._order_book_diff_stream.get()
    ...
    async def _track_single_book(self, trading_pair: str):
        past_diffs_window: Deque[FmfwOrderBookMessage] = deque()
        self._past_diffs_windows[trading_pair] = past_diffs_window

        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: OrderBook = self._order_books[trading_pair]
        active_order_tracker: FmfwActiveOrderTracker = self._active_order_trackers[trading_pair]
        ...
        while True:
            try:
                message: FmfwOrderBookMessage = None
                saved_messages: Deque[FmfwOrderBookMessage] = self._saved_message_queues[trading_pair]
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    past_diffs: List[FmfwOrderBookMessage] = list(past_diffs_window)
