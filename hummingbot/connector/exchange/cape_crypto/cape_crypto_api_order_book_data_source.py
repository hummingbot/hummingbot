...
from hummingbot.connector.exchange.cape_crypto.cape_crypto_order_book import CapeCryptoOrderBookBook
...
if TYPE_CHECKING:
    from hummingbot.connector.exchange.cape_crypto.cape_crypto_exchange import CapeCryptoExchange


class CapeCryptoAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot['sequence'] = 1
        snapshot_msg: OrderBookMessage = CapeCryptoOrderBookBook.snapshot_message_from_exchange(
            snapshot,
            snapshot_timestamp,
    ...
    async def _parse_trade_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        ...
        trade_message = CapeCryptoOrderBookBook.trade_message_from_exchange(
    ...
    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        ...
        diff_msg: OrderBookMessage = CapeCryptoOrderBookBook.diff_message_from_exchange(
            raw_message[next(iter(raw_message))],
            diff_timestamp,
            metadata={"trading_pair": trading_pair}
        )
