...
from typing import Any, AsyncGenerator, Awaitable, Callable, Coroutine, Dict, Generator, Protocol, Tuple
...
class StreamDataSource(AutoStreamBlock[WSResponse, Dict[str, Any]]):
    """
    ...
    def _filter_empty_data(self, message: WSResponse) -> Generator[Dict[str, Any], None, None]:
        if message.data:
            self._last_recv_time_s = self._time()
            yield message.data
    ...
    async def open(self) -> None:
        ...
        if self._stream_state == StreamState.CLOSED:
            ...
            await self._ws_assistant.connect(ws_url=constants.WSS_URL, ping_timeout=30.0)
