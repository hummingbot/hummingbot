...
from . import cape_crypto_staging_constants as CONSTANTS
...
class CapeCryptoStagingConfigMap(BaseConnectorConfigMap):
    connector: str = Field(default="cape_crypto_staging", client_data=None)
    cape_crypto_staging_api_key: SecretStr = Field(
        default=...,
    ...
    cape_crypto_staging_api_secret: SecretStr = Field(
        default=...,
        client_data=ClientFieldData(
    ...
    class Config:
        title = "cape_crypto_staging"


KEYS = CapeCryptoStagingConfigMap.construct()
...
def convert_exchange_timestamp_to_ms(timestamp: str) -> int:
    ...
    return int(timestamp_formatted.timestamp())
