...
class BloxrouteOpenbookExchange(ExchangePyBase):
    ...
    async def _place_order(self,
        ...
        blxr_token_pair = TradingPair.fromString(trading_pair)
        if blxr_token_pair.baseToken not in self._trading_pairs_to_payer_address:
            raise Exception(f"payer address for token ${blxr_token_pair.baseToken} was not provided on initialization")
        await self._provider.submit_order(
            owner_address=self.owner_address,
            payer_address=self._trading_pairs_to_payer_address[blxr_token_pair.baseToken],
            market=f"{blxr_token_pair.baseToken}{blxr_token_pair.quoteToken}",
            side=TradeTypeToSide(trade_type),
