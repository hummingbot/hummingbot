...
class ZigzagExchange(ExchangePyBase):
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return ZigzagAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return ZigzagAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _place_order(self,
        ...
        price_str = f"{price:f}"
        type_str = ZigzagExchange.zigzag_order_type(order_type)
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(zigzag_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Zigzag's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Zigzag's get order endpoint does not return trade IDs.
    ...
    async def _update_order_status(self):
        # This is intended to be a backup measure to close straggler orders, in case Zigzag's user stream events
        # are not working.
        # The minimum poll interval for order status is 10 seconds.
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(zigzag_utils.is_exchange_information_valid, exchange_info["symbols"]):
    ...
    async def _get_last_traded_price(self, trading_pair: str) -> float:
        ...
        return float(resp_json[trading_pair]["lastPrice"])
