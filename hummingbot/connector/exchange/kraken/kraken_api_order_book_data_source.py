...
class KrakenAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    @classmethod
    async def _get_last_traded_price(cls, trading_pair: str, client: RESTAssistant, throttler: AsyncThrottler) -> float:
        url = (
        ...
        async with throttler.execute_task(CONSTANTS.TICKER_PATH_URL):
            response = RESTRequest(method=RESTMethod["GET"],
                                   url=url
                                   )

            resp = await client.call(request=response, timeout=100)
    ...
    async def fetch_trading_pairs(cls, client: RESTAssistant, throttler: Optional[AsyncThrottler] = None) -> List[str]:
        ...
        try:
            async with throttler.execute_task(CONSTANTS.ASSET_PAIRS_PATH_URL):
                ...
                response = await client.call(request=response, timeout=5)
