...
from hummingbot.connector.exchange.openware.openware_utils import RequestId
...
class OpenwareWebsocket(RequestId):
    ...
    def __init__(self,
                 auth: Optional[OpenwareAuth] = None,
                 throttler: Optional[AsyncThrottler] = None):
        self._auth: Optional[OpenwareAuth] = auth
    ...
    async def subscribe(self, streams: Optional[Dict[str, List]] = {}) -> int:
        return await self.request(Constants.WS_EVENT_SUBSCRIBE, {"streams": streams})

    # unsubscribe to a method
    async def unsubscribe(self, streams: Optional[Dict[str, List]] = {}) -> int:
