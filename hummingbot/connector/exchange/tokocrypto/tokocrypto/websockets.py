...
from tokocrypto.client import Client


class TokocryptoClientProtocol(WebSocketClientProtocol):
...
class TokocryptoReconnectingClientFactory(ReconnectingClientFactory):
...
class TokocryptoClientFactory(WebSocketClientFactory, TokocryptoReconnectingClientFactory):

    protocol = TokocryptoClientProtocol
...
class TokocryptoSocketManager(threading.Thread):
    ...
    def __init__(self, client, user_timeout=DEFAULT_USER_TIMEOUT):
        """Initialise the TokocryptoSocketManager

        :param client: Tokocrypto API client
    ...
    def _start_socket(self, path, callback, prefix='ws/'):
        ...
        factory = TokocryptoClientFactory(factory_url)
        factory.protocol = TokocryptoClientProtocol
    ...
    def start_miniticker_socket(self, callback, update_time=1000):
        ...
        This is not in the official Tokocrypto api docs, but this is what
        feeds the right column on a ticker page on Tokocrypto.
    ...
    def start_multiplex_socket(self, streams, callback):
        ...
        Message Format - see Tokocrypto API docs for all types
