...
class TokocryptoTime:
    """
    Used to monkey patch tokocrypto client's time module to adjust request timestamp when needed
    """
    TOKOCRYPTO_TIME_API = "https://www.tokocrypto.com/open/v1/common/time"
    NaN = float("nan")
    _bt_logger = None
    ...
    @classmethod
    def get_instance(cls) -> "TokocryptoTime":
        if cls._bt_shared_instance is None:
            cls._bt_shared_instance = TokocryptoTime()
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.TOKOCRYPTO_TIME_API) as resp:
                    ...
                    tokocrypto_server_time_ms: float = float(resp_data["timestamp"])
                    local_after_ms: float = time.perf_counter() * 1e3
            local_server_time_pre_image_ms: float = (local_before_ms + local_after_ms) / 2.0
            time_offset_ms: float = tokocrypto_server_time_ms - local_server_time_pre_image_ms
            self.add_time_offset_ms_sample(time_offset_ms)
            self._last_update_local_time = time.perf_counter()
        except asyncio.CancelledError:
            raise
        except Exception:
            self.logger().network("Error getting Tokocrypto server time.", exc_info=True,
                                  app_warning_msg="Could not refresh Tokocrypto server time. "
