...
class BitfinexAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_all_pairs(cls) -> pd.DataFrame:
        async with aiohttp.ClientSession() as client:
            exchange_conf_response = await safe_gather(
            ...
            exchange_confs = list(
                convert_from_exchange_trading_pair(item[0])
                for item in filter(
    ...
    async def get_trading_pairs(self) -> List[str]:
        ...
        if not self._trading_pairs:
            try:
                self._trading_pairs = await self.get_all_pairs()
            except Exception:
