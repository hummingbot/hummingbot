...
class SouthxchangeOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self, trading_pairs: Optional[List[str]] = None, markets_enabled: Dict[int , str] = {}):
        super().__init__(SouthxchangeAPIOrderBookDataSource(trading_pairs, markets_enabled), trading_pairs)
        ...
        self._order_books: Dict[str, SouthXchangeOrderBook] = {}
        self._saved_message_queues: Dict[str, Deque[SouthXchangeOrderBookMessage]] = \
            defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, SouthXchangeActiveOrderTracker] = defaultdict(SouthXchangeActiveOrderTracker)
    ...
    async def _track_single_book(self, trading_pair: str):
        ...
        """
        past_diffs_window: Deque[SouthXchangeOrderBookMessage] = deque()
        self._past_diffs_windows[trading_pair] = past_diffs_window

        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: SouthXchangeOrderBook = self._order_books[trading_pair]
        ...
        while True:
            try:
                message: SouthXchangeOrderBookMessage = None
                saved_messages: Deque[SouthXchangeOrderBookMessage] = self._saved_message_queues[trading_pair]
