...
import hummingbot.connector.exchange.bitzlato.bitzlato_http_utils as http_utils
...
from hummingbot.connector.exchange.bitzlato.bitzlato_in_flight_order import BitzlatoInFlightOrder
from hummingbot.connector.exchange.bitzlato.bitzlato_order_book_tracker import BitzlatoOrderBookTracker
from hummingbot.connector.exchange.bitzlato.bitzlato_user_stream_tracker import BitzlatoUserStreamTracker
from hummingbot.connector.exchange.bitzlato.bitzlato_utils import (
    BitzlatoAPIError,
...
class BitzlatoExchange(ExchangeBase):
    """
    BitzlatoExchange connects with Bitzlato.com exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
                 ...
                 bitzlato_secret_key: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True
                 ):
        """
        :param bitzlato_api_key: The API key to connect to private Bitzlato.com APIs.
        :param bitzlato_secret_key: The API secret.
        ...
        self._bitzlato_auth = BitzlatoAuth(bitzlato_api_key, bitzlato_secret_key)
        self._set_order_book_tracker(BitzlatoOrderBookTracker(
            throttler=self._throttler,
            trading_pairs=trading_pairs))
        self._user_stream_tracker = BitzlatoUserStreamTracker(
            throttler=self._throttler,
            bitzlato_auth=self._bitzlato_auth,
            trading_pairs=trading_pairs)
    ...
    async def _api_request(self,
        ...
        parsed_response = await http_utils.api_call_with_retries(
            method=method,
            endpoint=endpoint,
            auth_headers=self._bitzlato_auth.get_headers if is_auth_required else None,
            params=params,
        ...
        if "errors" in parsed_response or "error" in parsed_response:
            ...
            raise BitzlatoAPIError(parsed_response)
    ...
    async def _create_order(self,
        ...
        try:
            ...
            if isinstance(e, BitzlatoAPIError):
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> CancellationResult:
        ...
        :param trading_pair: The market trading pair (Unused during cancel on Bitzlato.com)
    ...
    def stop_tracking_order_exceed_not_found_limit(self, tracked_order: BitzlatoInFlightOrder):
        """
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for response, tracked_order in zip(responses, tracked_orders):
                if isinstance(response, BitzlatoAPIError):
    ...
    def _process_order_message(self, order_msg: Dict[str, Any]):
        ...
        Example Order:
        {
            ...
            'executed_volume': '0.0',
            'at': 1659603641,
            'created_at': 1659603641,
            'updated_at': 1659603645,
            'trades_count': 0,
            'uuid': 'b700c45a-ffff-ffff-aaaa-ffff94a4fd8d'
        }
        """
    ...
    async def _trigger_order_fill(self,
                                  tracked_order: BitzlatoInFlightOrder,
    ...
    async def _user_stream_event_listener(self):
        ...
        BitzlatoAPIUserStreamDataSource.
        """
    ...
    async def all_trading_pairs(self) -> List[str]:
        ...
        return await BitzlatoAPIOrderBookDataSource.fetch_trading_pairs()

    async def get_last_traded_prices(self, trading_pairs: List[str]) -> Dict[str, float]:
        # This method should be removed and instead we should implement _get_last_traded_price
        return await BitzlatoAPIOrderBookDataSource.get_last_traded_prices(
            trading_pairs=trading_pairs,
            throttler=self._throttler
        )
