...
from .bitzlato_active_order_tracker import BitzlatoActiveOrderTracker
from .bitzlato_constants import Constants
from .bitzlato_order_book import BitzlatoOrderBook
from .bitzlato_utils import BitzlatoAPIError, convert_from_exchange_trading_pair, convert_to_exchange_trading_pair
...
class BitzlatoAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        ...
        active_order_tracker: BitzlatoActiveOrderTracker = BitzlatoActiveOrderTracker()
    ...
    async def listen_for_trades(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        ...
        while True:
            try:
                ws = BitzlatoWebsocket(throttler=self._throttler)
                    ...
                    if response is not None:
                        for msg_key in list(response.keys()):
                            ...
                            if len(split_key) != 2:
                                ...
                                self.logger().info(f"Unrecognized message received from Bitzlato websocket: {response}")
                                continue
                            ...
                            for trade in response[msg_key]["trades"]:
                                ...
                                trade_msg: OrderBookMessage = BitzlatoOrderBook.trade_message_from_exchange(
                                    trade,
                                    trade_timestamp,
                                    metadata={"trading_pair": trading_pair})
                    ...
                    if response is not None:
                        for msg_key in list(response.keys()):
                            ...
                            if Constants.WS_METHODS['ORDERS_UPDATE'] in msg_key:
                                order_book_msg_cls = BitzlatoOrderBook.diff_message_from_exchange
                                split_key = msg_key.split(Constants.WS_METHODS['ORDERS_UPDATE'], 1)
                            elif Constants.WS_METHODS['ORDERS_SNAPSHOT'] in msg_key:
                                order_book_msg_cls = BitzlatoOrderBook.snapshot_message_from_exchange
