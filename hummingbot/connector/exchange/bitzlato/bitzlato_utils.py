...
class BitzlatoAPIError(IOError):
...
def convert_from_exchange_trading_pair(ex_trading_pair: str) -> Optional[str]:
    # Bitzlato.io uses lowercase (btc_usdt)
...
class BitzlatoConfigMap(BaseConnectorConfigMap):
    connector: str = Field(default="bitzlato", client_data=None)
    bitzlato_api_key: SecretStr = Field(
        default=...,
    ...
    bitzlato_secret_key: SecretStr = Field(
        default=...,
        client_data=ClientFieldData(
...
KEYS = BitzlatoConfigMap.construct()
