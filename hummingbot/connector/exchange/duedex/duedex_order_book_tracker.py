...
class DuedexOrderBookTracker(OrderBookTracker):
    _orderbook_tracker_logger: Optional[HummingbotLogger] = None

    @classmethod
    def logger(cls) -> HummingbotLogger:
        if cls._orderbook_tracker_logger is None:
            cls._orderbook_tracker_logger = logging.getLogger(__name__)
        return cls._orderbook_tracker_logger

    def __init__(self,
                 trading_pairs: Optional[List[str]] = None):
        super().__init__(DuedexAPIOrderBookDataSource(trading_pairs), trading_pairs)
    ...
    def data_source(self) -> OrderBookTrackerDataSource:
        if not self._data_source:
            self._data_source = DuedexAPIOrderBookDataSource(trading_pairs=self._trading_pairs)
