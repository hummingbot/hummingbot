...
class NobitexAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        connection_info = await rest_assistant.execute_request(
            url=web_utils.private_rest_url(
                path_url=CONSTANTS.PRIVATE_WS_DATA_PATH_URL, domain=self._domain
            ),
        ...
        self._ping_interval = (
            int(connection_info["data"]["instanceServers"][0]["pingInterval"])
            * 0.8
            * 1e-3
        )
        ...
        await ws.connect(
            ws_url=f"{ws_url}?token={token}", message_timeout=self._ping_interval
        )
        return ws
    ...
    async def _process_event_message(
        ...
        if (
            ...
            and event_message.get("subject")
            in [CONSTANTS.ORDER_CHANGE_EVENT_TYPE, CONSTANTS.BALANCE_EVENT_TYPE]
        ):
