...
from hummingbot.connector.exchange.nobitex import (
    ...
    nobitex_utils as utils,
    nobitex_web_utils as web_utils,
)
from hummingbot.connector.exchange.nobitex.nobitex_api_order_book_data_source import (
    NobitexAPIOrderBookDataSource,
)
from hummingbot.connector.exchange.nobitex.nobitex_api_user_stream_data_source import (
    NobitexAPIUserStreamDataSource,
)
...
class NobitexExchange(ExchangePyBase):
    ...
    def __init__(
        ...
        self.nobitex_passphrase = nobitex_passphrase
        self.nobitex_secret_key = nobitex_secret_key
        self._domain = domain
    ...
    def authenticator(self):
        return NobitexAuth(
            ...
            passphrase=self.nobitex_passphrase,
            secret_key=self.nobitex_secret_key,
    ...
    def orders_path_url(self):
        return (
            ...
            if self._domain == "hft"
            else CONSTANTS.ORDERS_PATH_URL
        )
    ...
    def _is_order_not_found_during_status_update_error(
        ...
        return str(CONSTANTS.RET_CODE_RESOURCE_NOT_FOUND) in str(
            status_update_exception
        ) and str(CONSTANTS.RET_MSG_RESOURCE_NOT_FOUND) in str(status_update_exception)

    def _is_order_not_found_during_cancelation_error(
        self, cancelation_exception: Exception
    ) -> bool:
        return str(CONSTANTS.RET_CODE_ORDER_NOT_EXIST_OR_NOT_ALLOW_TO_CANCEL) in str(
            cancelation_exception
        ) and str(CONSTANTS.RET_MSG_ORDER_NOT_EXIST_OR_NOT_ALLOW_TO_CANCEL) in str(
            cancelation_exception
        )
    ...
    def _get_fee(
        ...
        if trading_pair in self._trading_fees:
            ...
            fee_value = (
                Decimal(fees_data["makerFeeRate"])
                if is_maker
                else Decimal(fees_data["takerFeeRate"])
            )
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(
        ...
        for symbol_data in filter(
            utils.is_pair_information_valid, exchange_info.get("data", [])
        ):
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        if exchange_order_id.get("data") is None:
            raise IOError(f"Error placing order on nobitex: {exchange_order_id}")
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        params = (
            {"symbol": tracked_order.trading_pair} if self.domain == "hft" else None
        )
        ...
        if cancel_result.get("data") is not None:
            return tracked_order.exchange_order_id in cancel_result["data"].get(
                response_param, []
            )
        else:
            raise IOError(f"Error cancelling order on nobitex: {cancel_result}")
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://docs.nobitex.com/#private-order-change-events
                if (
                    event_type == "message"
                    and event_subject == CONSTANTS.ORDER_CHANGE_EVENT_TYPE
                ):
                    ...
                    event_type == "message"
                    and event_subject == CONSTANTS.BALANCE_EVENT_TYPE
                ):
                    ...
                    self._account_available_balances.update(
    ...
    async def _format_trading_rules(
        self, raw_trading_pair_info: Dict[str, Any]
    ) -> List[TradingRule]:
        trading_rules = []
    ...
    async def _update_trading_fees(self):
        ...
        for idx in range(0, len(trading_symbols), CONSTANTS.TRADING_FEES_SYMBOL_LIMIT):
            sub_trading_symbols = trading_symbols[
                idx : idx + CONSTANTS.TRADING_FEES_SYMBOL_LIMIT
            ]
    ...
    async def _update_orders_fills(self, orders: List[InFlightOrder]):
        ...
        # An alternative for nobitex would be to use the limit/fills that returns 24hr updates, which should
    ...
    async def _all_trades_updates(
        ...
        if len(orders) > 0:
            ...
            self._last_order_fill_ts_s = int(
                max(
                    self._last_order_fill_ts_s,
                    min([o.creation_timestamp for o in orders]),
                )
            )

            # From nobitex https://docs.nobitex.com/#list-fills:
            ...
            for trade in all_fills_response.get("items", []):
                if str(trade["orderId"]) in exchange_to_client:
                    fee = TradeFeeBase.new_spot_fee(
                        ...
                        flat_fees=[
                            TokenAmount(
                                amount=Decimal(trade["fee"]), token=trade["feeCurrency"]
                            )
                        ],
                    )
                    ...
                    self._last_order_fill_ts_s = max(
                        self._last_order_fill_ts_s, trade["createdAt"] * 1e-3
                    )
    ...
    async def _all_trade_updates_for_order(
        ...
        raise Exception(
            "Developer: This method should not be called, it is obsoleted for nobitex"
        )
    ...
    async def _request_order_status(self, tracked_order: InFlightOrder) -> OrderUpdate:
        ...
        is_active = (
            updated_order_data["data"]["active"]
            if self.domain == "hft"
            else updated_order_data["data"]["isActive"]
        )
