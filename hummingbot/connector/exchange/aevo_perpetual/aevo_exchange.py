...
import hummingbot.connector.exchange.aevo.aevo_constants as CONSTANTS
from hummingbot.connector.exchange.aevo import aevo_utils, aevo_web_utils as web_utils
from hummingbot.connector.exchange.aevo.aevo_api_order_book_data_source import AevoAPIOrderBookDataSource
from hummingbot.connector.exchange.aevo.aevo_api_user_stream_data_source import AevoAPIUserStreamDataSource
from hummingbot.connector.exchange.aevo.aevo_auth import AevoAuth
...
class AevoExchange(ExchangePyBase):

    web_utils = web_utils

    def __init__(
        self,
        aevo_api_key: str = None,
        aevo_secret_key: str = None,
        aevo_passphrase: str = None,
        balance_asset_limit: Optional[Dict[str, Dict[str, Decimal]]] = None,
        rate_limits_share_pct: Decimal = Decimal("100"),
        trading_pairs: Optional[List[str]] = None,
        trading_required: bool = True,
    ) -> None:
        self._api_key = aevo_api_key
        self._secret_key = aevo_secret_key
        self._passphrase = aevo_passphrase
    ...
    @property
    def authenticator(self) -> AevoAuth:
        return AevoAuth(
    ...
    async def _update_trading_fees(self) -> None:
        ...
        for symbol_details in symbol_data:
            if aevo_utils.is_exchange_information_valid(exchange_info=symbol_details):
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return AevoAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return AevoAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(
        ...
        for symbol_data in exchange_info["data"]:
            if aevo_utils.is_exchange_information_valid(exchange_info=symbol_data):
                try:
    ...
    async def _format_trading_rules(
        ...
        for rule in exchange_info_dict["data"]:
            if aevo_utils.is_exchange_information_valid(exchange_info=rule):
                try:
                    trading_pair = await self.trading_pair_associated_to_exchange_symbol(
                        symbol=rule["symbol"]
                    )
