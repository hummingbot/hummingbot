...
from hummingbot.connector.exchange.backpack import backpack_utils, backpack_web_utils as web_utils
...
class BackpackExchange(ExchangePyBase):
    ...
    def __init__(
        ...
        self._passphrase = backpack_passphrase
    ...
    async def _update_trading_fees(self) -> None:
        ...
        for symbol_details in symbol_data:
            if backpack_utils.is_exchange_information_valid(exchange_info=symbol_details):
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(
        ...
        for symbol_data in exchange_info["data"]:
            if backpack_utils.is_exchange_information_valid(exchange_info=symbol_data):
                try:
    ...
    async def _format_trading_rules(
        ...
        for rule in exchange_info_dict["data"]:
            if backpack_utils.is_exchange_information_valid(exchange_info=rule):
                try:
                    trading_pair = await self.trading_pair_associated_to_exchange_symbol(
                        symbol=rule["symbol"]
                    )
