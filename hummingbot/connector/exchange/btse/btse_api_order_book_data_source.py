...
from hummingbot.connector.exchange.btse import btse_utils, btse_web_utils as web_utils
...
class BtseAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_last_traded_prices(cls,
        ...
        :param domain: which Btse domain we are connecting to (the default value is 'com')
    ...
    async def get_all_mid_prices(domain: str = CONSTANTS.DEFAULT_DOMAIN) -> Dict[str, Decimal]:
        ...
        for record in resp_json:
            try:
                pair = await BtseAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
    ...
    async def exchange_symbol_associated_to_pair(
        ...
        """
        symbol_map = await BtseAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=domain,
            api_factory=api_factory,
            throttler=throttler,
            time_synchronizer=time_synchronizer)
    ...
    async def fetch_trading_pairs(
        ...
        """
        mapping = await BtseAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=domain,
            throttler=throttler,
            api_factory=api_factory,
            time_synchronizer=time_synchronizer,
        ...
        while True:
            try:
                ...
                if "result" in json_msg:
                    continue
                trading_pair = await BtseAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
                    symbol=json_msg["s"],
                    domain=self._domain,
    ...
    async def _init_trading_pair_symbols(
        ...
        try:
            ...
            for symbol_data in filter(btse_utils.is_exchange_information_valid, data["symbols"]):
