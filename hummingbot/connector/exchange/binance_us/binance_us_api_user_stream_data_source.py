...
BINANCE_US_API_ENDPOINT = "https://api.binance.us/api/v1/"
BINANCE_US_USER_STREAM_ENDPOINT = "userDataStream"
...
class BinanceUsAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    async def get_listen_key(self):
        async with aiohttp.ClientSession() as client:
            async with client.post(f"{BINANCE_US_API_ENDPOINT}{BINANCE_US_USER_STREAM_ENDPOINT}",
                                   headers={"X-MBX-APIKEY": self._binance_client.API_KEY}) as response:
                response: aiohttp.ClientResponse = response
                if response.status != 200:
                    raise IOError(f"Error fetching Binance.US user stream listen key. HTTP status is {response.status}.")
                data: Dict[str, str] = await response.json()
                return data["listenKey"]

    async def ping_listen_key(self, listen_key: str) -> bool:
        async with aiohttp.ClientSession() as client:
            async with client.put(f"{BINANCE_US_API_ENDPOINT}{BINANCE_US_USER_STREAM_ENDPOINT}",
    ...
    async def get_ws_connection(self) -> websockets.WebSocketClientProtocol:
        stream_url: str = f"wss://stream.binance.us:9443/ws/{self._current_listen_key}"
