...
class BinanceUsTime:
    """
    Used to monkey patch Binance.US client's time module to adjust request timestamp when needed
    """
    BINANCE_US_TIME_API = "https://api.binance.us/api/v1/time"
    NaN = float("nan")
    _bt_logger = None
    ...
    @classmethod
    def get_instance(cls) -> "BinanceUsTime":
        if cls._bt_shared_instance is None:
            cls._bt_shared_instance = BinanceUsTime()
    ...
    async def update_server_time_offset(self):
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.BINANCE_US_TIME_API) as resp:
            ...
            self.logger().network("Error getting Binance.US server time.", exc_info=True,
                                  app_warning_msg="Could not refresh Binance.US server time. "
