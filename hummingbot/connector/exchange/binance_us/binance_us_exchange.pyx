...
from hummingbot.connector.exchange.binance_us.binance_us_api_order_book_data_source import BinanceUsAPIOrderBookDataSource
...
from .binance_us_order_book_tracker import BinanceUsOrderBookTracker
from .binance_us_user_stream_tracker import BinanceUsUserStreamTracker
from .binance_us_time import BinanceUsTime
from .binance_us_in_flight_order import BinanceUsInFlightOrder
from .binance_us_utils import (
...
cdef class BinanceUsExchange(ExchangeBase):
    ...
    @property
    def in_flight_orders(self) -> Dict[str, BinanceUsInFlightOrder]:
    ...
    @property
    def order_book_tracker(self) -> BinanceUsOrderBookTracker:
        return self._order_book_tracker

    @property
    def user_stream_tracker(self) -> BinanceUsUserStreamTracker:
        return self._user_stream_tracker

    def restore_tracking_states(self, saved_states: Dict[str, any]):
        self._in_flight_orders.update({
            key: BinanceUsInFlightOrder.from_json(value)
    ...
    async def schedule_async_call(
            ...
            app_warning_msg: str = "Binance.US API call failed. Check API key and network connection.") -> any:
    ...
    async def query_api(
            ...
            func,
            *args,
            app_warning_msg: str = "Binance.US API call failed. Check API key and network connection.",
        ...
        async with self._throttler.weighted_task(request_weight=request_weight):
            try:
                ...
                if "Timestamp for this request" in str(ex):
                    self.logger().warning("Got Binance.US timestamp error. "
                                          "Going to force update Binance.US server time offset...")
                    binance_time = BinanceUsTime.get_instance()
    ...
    async def _update_trade_fees(self):
        ...
        if current_timestamp - self._last_update_trade_fees_timestamp > 60.0 * 60.0 or len(self._trade_fees) < 1:
            try:
                ...
                self.logger().network("Error fetching Binance.US trade fees.", exc_info=True,
                                      app_warning_msg=f"Could not fetch Binance.US trading fees. "
    ...
    cdef object c_get_fee(self,
        ...
        return estimate_fee("binance_us", is_maker)
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from Binance.US. Check API key and network connection."
                )
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.", exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Binance.US. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.", exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Binance.US. "
    ...
    async def create_order(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting {side_str} {type_str} order to Binance.US for "
                f"{amount} {trading_pair} "
                f"{price}.",
                exc_info=True,
    ...
    async def execute_cancel(self, trading_pair: str, order_id: str):
        try:
            ...
            if "Unknown order sent" in e.message or e.code == 2011:
                ...
                self.logger().debug(f"The order {order_id} does not exist on Binance.US. No cancellation needed.")
                self.c_stop_tracking_order(order_id)
                self.c_trigger_event(self.MARKET_ORDER_CANCELLED_EVENT_TAG,
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel order with Binance.US. Check API key and network connection."
            )
    ...
    cdef c_start_tracking_order(self,
                                ...
                                object amount,
                                object order_type):
        self._in_flight_orders[order_id] = BinanceUsInFlightOrder(
