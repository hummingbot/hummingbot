...
class K2APIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def fetch_trading_pairs() -> List[str]:
        async with aiohttp.ClientSession() as client:
            async with client.get(f"{constants.REST_URL}/Public/GetPairs", timeout=10) as response:
    ...
    async def get_order_book_data(trading_pair: str) -> Dict[str, any]:
        ...
        async with aiohttp.ClientSession() as client:
            ...
            async with client.get(url=f"{constants.REST_URL}/public/getorderbook", params=params) as resp:
                if resp.status != 200:
                    raise IOError(
