...
class BitrueAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        url = f"{CONSTANTS.WSS_PRIVATE_URL}/stream?listenKey={self._current_listen_key}"
        await ws.connect(ws_url=url, ping_timeout=CONSTANTS.WS_HEARTBEAT_TIME_INTERVAL)
        return ws

    async def _subscribe_channels(self, websocket_assistant: WSAssistant):
        """
        Subscribes to the balance and order update events through the provided websocket connection.
        ...
        try:
            ...
            subscribe_balance_request: WSJSONRequest = WSJSONRequest(payload=balance_update_payload)
            subscribe_order_request: WSJSONRequest = WSJSONRequest(payload=order_update_payload)

            await websocket_assistant.send(subscribe_balance_request)
            await websocket_assistant.send(subscribe_order_request)

            self.logger().info("Subscribed to private balance and order update channels.")
        except asyncio.CancelledError:
            raise
        except Exception:
            self.logger().error(
                "Unexpected error occurred subscribing to balance and order update streams...",
                exc_info=True
            )
            raise
    ...
    async def _get_listen_key(self):
        ...
        try:
            data = await rest_assistant.execute_request(
                url=f"{CONSTANTS.LISTEN_KEY_URL}/{CONSTANTS.PRIVATE_API_VERSION}/{CONSTANTS.USER_STREAM_PATH_URL}",
    ...
    async def _ping_listen_key(self) -> bool:
        ...
        try:
            data = await rest_assistant.execute_request(
                url=f"{CONSTANTS.LISTEN_KEY_URL}/{CONSTANTS.PRIVATE_API_VERSION}/{CONSTANTS.USER_STREAM_PATH_URL}/{self._current_listen_key}",
