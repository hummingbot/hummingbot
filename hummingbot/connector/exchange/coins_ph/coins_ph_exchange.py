...
from hummingbot.connector.exchange.coins_ph import (
    coins_ph_constants as CONSTANTS,
    coins_ph_utils,
    coins_ph_web_utils as web_utils,
)
from hummingbot.connector.exchange.coins_ph.coins_ph_api_order_book_data_source import CoinsPhAPIOrderBookDataSource
from hummingbot.connector.exchange.coins_ph.coins_ph_api_user_stream_data_source import CoinsPhAPIUserStreamDataSource
from hummingbot.connector.exchange.coins_ph.coins_ph_auth import CoinsPhAuth
...
class CoinsPhExchange(ExchangePyBase):
    ...
    def __init__(self,
                 ...
                 coins_ph_api_key: str,
                 coins_ph_api_secret: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True,
                 domain: str = CONSTANTS.DEFAULT_DOMAIN,
                 ):
        self.api_key = coins_ph_api_key
        self.secret_key = coins_ph_api_secret
        self._domain = domain
        self._trading_required = trading_required
        self._trading_pairs = trading_pairs
        self._last_trades_poll_coins_ph_timestamp = 1.0
        super().__init__(client_config_map)

    @staticmethod
    def coins_ph_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(coins_ph_type: str) -> OrderType:
        return OrderType[coins_ph_type]
    ...
    async def get_all_pairs_prices(self) -> List[Dict[str, str]]:
        ...
        pairs_prices = await rest_assistant.execute_request(
            url=CONSTANTS.REST_URL + 'quote/' + CONSTANTS.PUBLIC_API_VERSION + CONSTANTS.TICKER_BOOK_PATH_URL,
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return CoinsPhAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return CoinsPhAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _place_order(self,
        ...
        type_str = CoinsPhExchange.coins_ph_order_type(order_type)
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(coins_ph_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://coins-docs.github.io/user-data-stream/
                # As per the order update section in Coins.ph the ID of the order being canceled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Coins.ph's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Coins.ph's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_coins_ph_timestamp * 1e3)
            self._last_trades_poll_coins_ph_timestamp = self._time_synchronizer.time()
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(coins_ph_utils.is_exchange_information_valid, exchange_info["symbols"]):
    ...
    async def _get_last_traded_price(self, trading_pair: str) -> float:
        ...
        resp_json = await rest_assistant.execute_request(
            url=CONSTANTS.REST_URL + 'quote/' + CONSTANTS.PUBLIC_API_VERSION + CONSTANTS.TICKER_PRICE_CHANGE_PATH_URL,
            params=params,
            method=RESTMethod.GET,
