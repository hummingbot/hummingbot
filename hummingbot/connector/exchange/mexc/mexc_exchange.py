...
class MexcExchange(ExchangePyBase):
    ...
    async def get_all_pairs_prices(self) -> List[Dict[str, str]]:
        pairs_prices = await self._api_get(path_url=CONSTANTS.TICKER_BOOK_PATH_URL, content_type="application/json")
        return pairs_prices
    ...
    async def _make_network_check_request(self):
        await self._api_get(path_url=self.check_network_request_path, content_type="application/json")

    async def _make_trading_rules_request(self) -> Any:
        exchange_info = await self._api_get(path_url=self.trading_rules_request_path, content_type="application/json")
        return exchange_info

    async def _make_trading_pairs_request(self) -> Any:
        exchange_info = await self._api_get(path_url=self.trading_pairs_request_path, content_type="application/json")
        return exchange_info
