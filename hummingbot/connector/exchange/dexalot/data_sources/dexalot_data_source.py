...
class DexalotClient:
    ...
    def __init__(
        ...
        self.provider = (
            CONSTANTS.DEXALOT_SUBNET_RPC_URL if self._domain == "dexalot" else CONSTANTS.TESTNET_DEXALOT_SUBNET_RPC_URL
        )
        ...
        self.account: LocalAccount = (
            Account.from_key(dexalot_api_secret) if self.trading_required or self.trading_capability else None
        )  # See the above comment for details
        ...
        TRADEPAIRS_ADDRESS = (
            CONSTANTS.DEXALOT_TRADEPAIRS_ADDRESS
            if self._domain == "dexalot"
            else CONSTANTS.TESTNET_DEXALOT_TRADEPAIRS_ADDRESS
        )
        PORTFOLIOSUB_ADDRESS = (
            CONSTANTS.DEXALOT_PORTFOLIOSUB_ADDRESS
            if self._domain == "dexalot"
            else CONSTANTS.TESTNET_DEXALOT_PORTFOLIOSUB_ADDRESS
        )

        self.trade_pairs_manager = self.async_w3.eth.contract(address=TRADEPAIRS_ADDRESS, abi=DEXALOT_TRADEPAIRS_ABI)

        self.portfolio_sub_manager = self.async_w3.eth.contract(
            address=PORTFOLIOSUB_ADDRESS, abi=DEXALOT_PORTFOLIOSUB_ABI
        )
    ...
    async def _get_token_info(self):
        ...
        for token_info in token_raw_info_list:
            self.balance_evm_params[token_info["subnet_symbol"]] = {"token_evmdecimals": token_info["evmdecimals"]}
        ...
        async with self.transaction_lock:
            ...
            for retry_attempt in range(CONSTANTS.TRANSACTION_REQUEST_ATTEMPTS):
                ...
                try:
                    ...
                    signed_txn = self.async_w3.eth.account.sign_transaction(transaction, private_key=self._private_key)
