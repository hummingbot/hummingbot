...
from hummingbot.connector.exchange.bitget_unified import bitget_unified_utils, bitget_unified_web_utils as web_utils
from hummingbot.connector.exchange.bitget_unified.bitget_unified_api_order_book_data_source import (
    BitgetUnifiedAPIOrderBookDataSource,
)
from hummingbot.connector.exchange.bitget_unified.bitget_unified_api_user_stream_data_source import (
    BitgetUnifiedAPIUserStreamDataSource,
)
from hummingbot.connector.exchange.bitget_unified.bitget_unified_auth import BitgetUnifiedAuth
...
class BitgetUnifiedExchange(ExchangePyBase):

    web_utils = web_utils

    def __init__(
        self,
        bitget_unified_api_key: str = None,
        bitget_unified_secret_key: str = None,
        bitget_unified_passphrase: str = None,
        balance_asset_limit: Optional[Dict[str, Dict[str, Decimal]]] = None,
        rate_limits_share_pct: Decimal = Decimal("100"),
        trading_pairs: Optional[List[str]] = None,
        trading_required: bool = True,
    ) -> None:
        self._api_key = bitget_unified_api_key
        self._secret_key = bitget_unified_secret_key
        self._passphrase = bitget_unified_passphrase
    ...
    async def _update_trading_fees(self) -> None:
        ...
        for symbol_details in symbol_data:
            ...
            if (
                bitget_unified_utils.is_exchange_information_valid(exchange_info=symbol_details)
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return BitgetUnifiedAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return BitgetUnifiedAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    def _parse_trade_update(
        ...
        # type); BitgetUnified V3 already reports feeDetail.fee as a positive value, abs() normalises the
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(
        ...
        for symbol_data in exchange_info["data"]:
            if bitget_unified_utils.is_exchange_information_valid(exchange_info=symbol_data):
                try:
