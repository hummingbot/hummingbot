...
SNAPSHOT_REST_URL = "https://trade.mandala.exchange/open/v1/market/depth"
BINANCE_STREAM_URL = "wss://stream.binance.cc/ws"
MANDALA_STREAM_URL = "wss://trade.mandala.exchange/ws"
BINANCE_TICKER_PRICE_CHANGE_URL = "https://api.binance.cc/api/v1/ticker/24hr"
TICKER_PRICE_CHANGE_URL = "https://trade.mandala.exchange/v1/market/klines"
EXCHANGE_INFO_URL = "https://trade.mandala.exchange/open/v1/common/symbols"


class MandalaAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def fetch_trading_pairs(domain="com") -> List[str]:
        try:
            from hummingbot.connector.exchange.mandala.mandala_utils import convert_from_exchange_trading_pair
            async with aiohttp.ClientSession() as client:
    ...
    async def get_snapshot(client: aiohttp.ClientSession, trading_pair: str, limit: int = 1000,
        ...
        params: Dict = {"limit": str(limit), "symbol": convert_to_mandala_exchange_trading_pair(trading_pair)} if limit != 0 \
            else {"symbol": convert_to_mandala_exchange_trading_pair(trading_pair)}
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        async with aiohttp.ClientSession() as client:
            ...
            snapshot_msg: OrderBookMessage = MandalaOrderBook.snapshot_message_from_exchange(
                snapshot,
                snapshot_timestamp,
            ...
            try:
                ...
                async with websockets.connect(stream_url) as ws:
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        ...
                        order_book_message: OrderBookMessage = MandalaOrderBook.diff_message_from_exchange(
                            msg, time.time())
