...
class MandalaTime:
    ...
    @classmethod
    def get_instance(cls) -> "MandalaTime":
        if cls._bt_shared_instance is None:
            cls._bt_shared_instance = MandalaTime()
    ...
    async def update_server_time_offset(self):
        try:
            ...
            time_offset_ms: float = mandala_server_time_ms - local_server_time_pre_image_ms
            self.add_time_offset_ms_sample(time_offset_ms)
            self._last_update_local_time = time.perf_counter()
        except asyncio.CancelledError:
            raise
        except Exception:
            self.logger().network("Error getting Mandala server time.", exc_info=True,
                                  app_warning_msg="Could not refresh Mandala server time. "
