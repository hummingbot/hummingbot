...
from hummingbot.connector.exchange.mandala.mandala import client as mandala_client_module
from hummingbot.connector.exchange.mandala.mandala.exceptions import MandalaAPIException
...
from hummingbot.connector.exchange.mandala.mandala_api_order_book_data_source import MandalaAPIOrderBookDataSource
...
from .mandala_order_book_tracker import MandalaOrderBookTracker
from .mandala_time import MandalaTime
...
cdef class MandalaExchangeTransactionTracker(TransactionTracker):
    cdef:
        MandalaExchange _owner

    def __init__(self, owner: MandalaExchange):
        super().__init__()
        self._owner = owner
...
cdef class MandalaExchange(ExchangeBase):
    ...
    def __init__(self,
                 mandala_api_key: str,
                 mandala_api_secret: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True,
                 domain="com"
                 ):

        self.monkey_patch_mandala_time()
        super().__init__()
        self._trading_required = trading_required
        self._order_book_tracker = MandalaOrderBookTracker(trading_pairs=trading_pairs, domain=domain)
        self._domain = domain
        self._mandala_client = MandalaClient(mandala_api_key, mandala_api_secret, tld=domain)
        ...
        self._tx_tracker = MandalaExchangeTransactionTracker(self)
    ...
    def name(self) -> str:
        if self._domain == "com":
            ...
            return f"mandala_{self._domain}"
    ...
    @property
    def mandala_client(self) -> MandalaClient:
        return self._mandala_client
    ...
    @property
    def in_flight_orders(self) -> Dict[str, MandalaInFlightOrder]:
    ...
    @property
    def order_book_tracker(self) -> MandalaOrderBookTracker:
        return self._order_book_tracker

    def restore_tracking_states(self, saved_states: Dict[str, any]):
        self._in_flight_orders.update({
            key: MandalaInFlightOrder.from_json(value)
            for key, value in saved_states.items()
        })

    async def get_active_exchange_markets(self) -> pd.DataFrame:
        return await MandalaAPIOrderBookDataSource.get_active_exchange_markets()

    def monkey_patch_mandala_time(self):
        if mandala_client_module.time != MandalaTime.get_instance():
            mandala_client_module.time = MandalaTime.get_instance()
            MandalaTime.get_instance().start()

    async def schedule_async_call(
            self,
            coro: Coroutine,
            timeout_seconds: float,
            app_warning_msg: str = "Mandala API call failed. Check API key and network connection.") -> any:
    ...
    async def query_api(
            ...
            func,
            *args,
            app_warning_msg: str = "Mandala API call failed. Check API key and network connection.",
        ...
        async with self._throttler.weighted_task(request_weight=request_weight):
            try:
                ...
                if "Timestamp for this request" in str(ex):
                    self.logger().warning("Got Mandala timestamp error. "
                                          "Going to force update Mandala server time offset...")
                    mandala_time = MandalaTime.get_instance()
                    mandala_time.clear_time_offset_ms_samples()
                    await mandala_time.schedule_update_server_time_offset()
                raise ex
    ...
    async def _update_balances(self):
        ...
        account_info = await self.query_api(self._mandala_client.get_account)
    ...
    async def _update_trade_fees(self):
        ...
        if current_timestamp - self._last_update_trade_fees_timestamp > 60.0 * 60.0 or len(self._trade_fees) < 1:
            try:
                res = await self.query_api(self._mandala_client.get_trade_fee)
                ...
                self.logger().network("Error fetching Mandala trade fees.", exc_info=True,
                                      app_warning_msg=f"Could not fetch Mandala trading fees. "
    ...
    cdef object c_get_fee(self,
        ...
        if trading_pair not in self._trade_fees:
            # https://trade.mandala.exchange/en/fee/schedule
    ...
    async def _update_trading_rules(self):
        ...
        if current_tick > last_tick or len(self._trading_rules) < 1:
            exchange_info = await self.query_api(self._mandala_client.get_exchange_info)
    ...
    async def _update_order_fills_from_trades(self):
        cdef:
            ...
            # in case Mandala's user stream events are not working.
            # This is separated from _update_order_status which only updates the order status without producing filled
            # events, since Mandala's get order endpoint does not return trade IDs.
    ...
    async def _update_order_status(self):
        cdef:
            # This is intended to be a backup measure to close straggler orders, in case Mandala's user stream events
            # are not working.
            # The minimum poll interval for order status is 10 seconds.
            int64_t last_tick = <int64_t>(self._last_poll_timestamp / self.UPDATE_ORDER_STATUS_MIN_INTERVAL)
            int64_t current_tick = <int64_t>(self._current_timestamp / self.UPDATE_ORDER_STATUS_MIN_INTERVAL)
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.", exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Mandala. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.", exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Mandala. "
    ...
    async def server_time(self) -> int:
        ...
        """
        result = await self.query_api(self._mandala_client.get_server_time)
    ...
    async def check_network(self) -> NetworkStatus:
        try:
            await self.query_api(self._mandala_client.ping)
        except asyncio.CancelledError:
            raise
        except Exception:
    ...
    @staticmethod
    def mandala_order_type(order_type: OrderType) -> str:
        if order_type is OrderType.MARKET:
            return MandalaClient.ORDER_TYPE_MARKET
        else:
            return MandalaClient.ORDER_TYPE_LIMIT

    @staticmethod
    def to_hb_order_type(mandala_type: str) -> OrderType:
        return OrderType[mandala_type]

    @staticmethod
    def to_hb_trade_type(mandala_trade_type: str) -> TradeType:
        return TradeType[mandala_trade_type]
    ...
    async def execute_cancel(self, trading_pair: str, order_id: str):
        try:
            ...
            if "Unknown order sent" in e.message or e.code == 2011:
                ...
                self.logger().debug(f"The order {order_id} does not exist on Mandala. No cancellation needed.")
                self.c_stop_tracking_order(order_id)
                self.c_trigger_event(self.MARKET_ORDER_CANCELLED_EVENT_TAG,
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel order with Mandala. Check API key and network connection."
            )
    ...
    cdef c_start_tracking_order(self,
                                ...
                                object amount,
                                object order_type):
        self._in_flight_orders[order_id] = MandalaInFlightOrder(
