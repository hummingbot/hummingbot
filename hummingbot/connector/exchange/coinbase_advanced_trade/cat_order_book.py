...
class CoinbaseAdvancedTradeOrderBook(OrderBook):
    ...
    @classmethod
    def level2_or_trade_message_from_exchange(cls,
                                              msg: Dict[str, any],
                                              timestamp: float,
        ...
        :param trading_pair: Hummingbot trading pair associated with the message
        ...
        if channel == "market_trades":
            return cls.market_trades_order_book_message(msg, trading_pair)

        elif channel == "l2_data":
            return cls.level2_order_book_message(msg, timestamp, trading_pair)

        raise ValueError(f"Unexpected channel: {channel}")

    @classmethod
    def level2_order_book_message(cls,
                                  msg: Dict[str, any],
                                  timestamp: float,
                                  trading_pair: str) -> OrderBookMessage:
        """
        ...
        :return: a snapshot message with the snapshot information received from the exchange
        """
        event: Dict[str, Any] = msg["events"][0]
    ...
    @classmethod
    def market_trades_order_book_message(cls, msg: Dict[str, Any], trading_pair: str) -> OrderBookMessage:
        """
