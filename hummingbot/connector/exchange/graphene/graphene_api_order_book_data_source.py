...
from hummingbot.connector.exchange.graphene.graphene_constants import \
    GrapheneConstants
# HUMMINGBOT MODULES
from hummingbot.connector.exchange.graphene.graphene_order_book import \
    GrapheneOrderBook
...
class GrapheneAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    @classmethod
    def trading_pair_symbol_map_ready(cls, domain: str):
        """
        Checks if the mapping from exchange to client trading pairs has been initialized
        :param domain: the name of the graphene blockchain
        :return: True if the mapping has been initialized, False otherwise
        """
        return (
            domain in cls._trading_pair_symbol_map
            and len(cls._trading_pair_symbol_map[domain]) > 0
        )
    ...
    async def trading_pair_symbol_map(
        ...
        **__,
        ...
        """
        internal map used to translate trading pairs from and to the exchange notation
        In general this should not be used, instead call the methods
            ~ `exchange_symbol_associated_to_pair`
            ~ `trading_pair_associated_to_exchange_symbol`
        :param domain: the name of the graphene blockchain
        :return: bidirectional mapping for pair exchange notation and client notation
        """
        if not cls.trading_pair_symbol_map_ready(domain=domain):
            async with cls._mapping_initialization_lock:
                # Check condition again
                # (could have been initialized waiting for the lock to be released)
                if not cls.trading_pair_symbol_map_ready(domain=domain):
                    await cls._init_trading_pair_symbols(domain)
    ...
    @staticmethod
    async def fetch_trading_pairs(domain: str = "peerplays") -> List[str]:
        """
        Returns a list of all known trading pairs enabled to operate with
        :param domain: the name of the graphene blockchain
        :return: list of trading pairs in client notation
        """
        # probably overkill... self.constants.chain.PAIRS would suffice
        mapping = await GrapheneAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=domain
        )
    ...
    async def _init_trading_pair_symbols(
        ...
        **__,
        ...
        """
        bidirectional mapping of trade symbols in exchange and client notation
        in our case they map 1:1 as BASE-QUOTE format
        """
        mapping = bidict()
        constants = GrapheneConstants(domain)
        for trading_pair in constants.chain.PAIRS:
