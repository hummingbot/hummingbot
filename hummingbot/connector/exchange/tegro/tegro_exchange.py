...
class TegroExchange(ExchangePyBase):
    ...
    def authenticator(self):
        return TegroAuth(api_key=self.api_key, api_secret=self.secret_key)
    ...
    async def get_chain_list(self):
        account_info = await self._api_request(
            method=RESTMethod.GET, path_url=CONSTANTS.CHAIN_LIST, limit_id=CONSTANTS.CHAIN_LIST, is_auth_required=False
        )
    ...
    async def _generate_cancel_order_typed_data(self, order_id: str, ids: list) -> Dict[str, Any]:
        try:
            params = {"order_ids": ids, "user_address": self.api_key.lower()}
            data = await self._api_request(
            ...
            if is_not_active:
                self.logger().debug(f"The order {order_id} does not exist on tegro." f"No cancelation needed.")
    ...
    async def _request_order_status(self, tracked_order: InFlightOrder) -> OrderUpdate:
        ...
        updated_order_data = await self._api_get(
            ...
            params={"chain_id": self.chain, "order_id": o_id},
    ...
    async def approve_allowance(self, token=None, fail_silently: bool = True):
        ...
        for token in token_list:
            ...
            try:
                ...
                approval_contract = contract.functions.approve(exchange_con_addr, MAX_UINT256).build_transaction(
                    tx_params
                )
