...
from hummingbot.connector.gateway.clob_spot.data_sources.oraidex.oraidex_helprs import (
    AsyncLock,
    automatic_retry_with_timeout,
    generate_hash,
)
from hummingbot.connector.gateway.clob_spot.data_sources.oraidex.oraidex_types import OrderStatus as OraiOrderStatus
                ...
                try:
                    if len(response['orders']) and response["orders"][0] is not None and response["orders"][0]["status"].upper() != order.current_state:
                        ...
                        message = {
                            "trading_pair": order.trading_pair,
                            "update_timestamp": time(),
                            "new_state": updated_order["status"].upper(),
                        }

                        if updated_order["status"].upper() in {
                        ...
                        elif updated_order["status"] == OrderState.FILLED.name:
                            message = {
                                "timestamp": time(),
                        ...
                        elif updated_order["status"].upper() == OrderState.CANCELED.name:
