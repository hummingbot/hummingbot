...
class GatewayStatusMonitor:
    ...
    async def _monitor_loop(self):
        while True:
            try:
                ...
                the try block. They wouldn't be as a result of http related error because they're expected to be caught.
                """
                pass
            finally:
    ...
    async def _fetch_gateway_configs(self) -> Dict[str, Any]:
        try:
            return await self._get_gateway_instance().get_configuration()
        except Exception:
            # Return empty dict if fetching configs fails
            return {}
