...
class HistoryCommand:
    ...
    def report_performance_by_market(
        ...
        assets_data = [
            (
                [f"{base:<17}", "-", "-", "-"]
                if market
                in AllConnectorSettings.get_derivative_names()  # No base asset for derivatives because they are margined
                else [
                    f"{base:<17}",
    ...
    def list_trades(
        ...
        if len(df) > 0:
            ...
            lines.extend(["", "  Recent trades:"] + ["    " + line for line in df_lines])
        else:
