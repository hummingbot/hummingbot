...
class StatusCommand:
            ...
            while len(warning_lines) < self.APP_WARNING_STATUS_LIMIT:
                ...
                for key in logger_keys:
                    ...
                    warning_lines.append(
                        f"    * {pd.Timestamp(warning_item.timestamp, unit='s')} - ({key}) - {warning_item.warning_msg}"
                    )
    ...
    async def strategy_status(self, live: bool = False):
        active_paper_exchanges = [
            exchange for exchange in self.trading_core.markets.keys() if exchange.endswith("paper_trade")
        ]
    ...
    def missing_configurations_legacy(
        ...
        if not isinstance(config_map, ClientConfigAdapter):
            missing_configs = missing_required_configs_legacy(get_strategy_config_map(self.trading_core.strategy_name))
