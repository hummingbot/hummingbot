...
class LPHistoryCommand:
    ...
    async def _report_pair_performance(
        ...
        total_volume_base = sum(
            Decimal(str(o.base_amount or 0)) + Decimal(str(c.base_amount or 0)) for o, c in zip(opens, closes)
        )
        total_volume_quote = sum(
            Decimal(str(o.quote_amount or 0)) + Decimal(str(c.quote_amount or 0)) for o, c in zip(opens, closes)
        )
        ...
        lines.extend(
            ["", "  Closed Positions:"]
            + ["    " + line for line in pos_df.to_string(index=False, header=False).split("\n")]
        )
