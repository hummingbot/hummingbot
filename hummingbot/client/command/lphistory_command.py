...
def get_timestamp(days_ago: float = 0.0) -> float:
...
class LPHistoryCommand:
    ...
    def _list_lp_updates(
        ...
        if len(updates) > 0:
            ...
            lines.extend(
                ["", "  LP Position Updates:"] + ["    " + line for line in df.to_string(index=False).split("\n")]
            )
        else:
    ...
    async def _report_pair_performance(
        ...
        closed_positions = {addr: pos for addr, pos in positions.items() if "ADD" in pos and "REMOVE" in pos}
        ...
        lines.append(
            f"Positions Opened: {open_position_count + closed_position_count}  |  Positions Closed: {closed_position_count}"
        )
        ...
        both_positions = [
            (o, c) for o, c in zip(opens, closes) if (o, c) not in buy_positions and (o, c) not in sell_positions
        ]
        ...
        pos_data = [
            ...
            [
                ...
                smart_round(
                    sum(Decimal(str(o.base_amount or 0)) + Decimal(str(c.base_amount or 0)) for o, c in buy_positions),
                    precision,
                ),
                smart_round(
                    sum(Decimal(str(o.base_amount or 0)) + Decimal(str(c.base_amount or 0)) for o, c in sell_positions),
                    precision,
                ),
            ],
            [
                ...
                smart_round(
                    sum(
                        Decimal(str(o.quote_amount or 0)) + Decimal(str(c.quote_amount or 0)) for o, c in both_positions
                    ),
                    precision,
                ),
                smart_round(
                    sum(
                        Decimal(str(o.quote_amount or 0)) + Decimal(str(c.quote_amount or 0)) for o, c in buy_positions
                    ),
                    precision,
                ),
                smart_round(
                    sum(
                        Decimal(str(o.quote_amount or 0)) + Decimal(str(c.quote_amount or 0)) for o, c in sell_positions
                    ),
                    precision,
                ),
            ],
        ...
        lines.extend(
            ["", "  Closed Positions:"] + ["    " + line for line in pos_df.to_string(index=False).split("\n")]
        )
