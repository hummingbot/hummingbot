...
class OrderBookCommand:
    def order_book(self,  # type: HummingbotApplication
                   lines: int = 5):
        safe_ensure_future(self.show_order_book(lines))

    async def show_order_book(self, lines: int = 5):
        if len(self.markets.keys()) == 0:
            self._notify("There is currently no active market.")
            return
        for market in self.markets.values():
            for trading_pair, order_book in market.order_books.items():
                ...
                self._notify(f"  market: {market.name} {trading_pair}\n")
                self._notify("\n".join(text_lines))
