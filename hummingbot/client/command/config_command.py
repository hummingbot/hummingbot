...
no_restart_pmm_keys = [
    ...
    "price_ceiling_pct",
    "price_floor_pct",
    "price_band_refresh_time" "order_optimization_enabled",
    "bid_order_optimization_depth",
    "ask_order_optimization_depth",
]
client_configs_to_display = [
    "autofill_import",
    "kill_switch_mode",
    "kill_switch_rate",
    "mqtt_bridge",
...
color_settings_to_display = ["top_pane", "bottom_pane", "output_pane", "input_pane", "logs_pane", "terminal_primary"]
columns = ["Key", "Value"]
...
class ConfigCommand:
    ...
    def list_client_configs(
        ...
        lines = [
            "    " + line
            for line in format_df_for_printout(
                df, table_format=self.client_config_map.tables_format, max_col_width=50
            ).split("\n")
        ]
        self.notify("\n".join(lines))
    ...
    def build_model_df_data(
        ...
        for traversal_item in config_map.traverse():
            if to_print is not None and traversal_item.attr not in to_print:
                continue
            attr_printout = (
                ("  " * (traversal_item.depth - 1) + ("\u221f " if not is_windows() else "  ") + traversal_item.attr)
                if traversal_item.depth
                else traversal_item.attr
            )
