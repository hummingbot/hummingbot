...
def format_performance_by_market(market: str, trading_pair: str, perf: "PerformanceMetrics",
                                 precision: Optional[int] = None) -> List[str]:
    """Build the per-market Trades / Assets / Performance report lines for ``market``/``trading_pair``.

    Shared by the interactive ``history`` command and the ``hbot history`` CLI so the layout stays in
    one place. Returns lines (the caller joins/notifies/echoes them). Derivatives omit base-asset rows.
    """
    import pandas as pd
    ...
    trades_df = pd.DataFrame(
        data=[[f"{'Number of trades':<27}", perf.num_buys, perf.num_sells, perf.num_trades],
              [f"{f'Total trade volume ({base})':<27}",
               sr(perf.b_vol_base, precision), sr(perf.s_vol_base, precision), sr(perf.tot_vol_base, precision)],
              [f"{f'Total trade volume ({quote})':<27}",
               sr(perf.b_vol_quote, precision), sr(perf.s_vol_quote, precision), sr(perf.tot_vol_quote, precision)],
              [f"{'Avg price':<27}",
               sr(perf.avg_b_price, precision), sr(perf.avg_s_price, precision), sr(perf.avg_tot_price, precision)]],
        columns=["", "buy", "sell", "total"])
    lines += ["", "  Trades:"] + ["    " + ln for ln in trades_df.to_string(index=False).split("\n")]

    assets_df = pd.DataFrame(
        data=[[f"{base:<17}", "-", "-", "-"] if is_deriv else
              [f"{base:<17}", sr(perf.start_base_bal, precision), sr(perf.cur_base_bal, precision),
               sr(perf.tot_vol_base, precision)],
              [f"{quote:<17}", sr(perf.start_quote_bal, precision), sr(perf.cur_quote_bal, precision),
               sr(perf.tot_vol_quote, precision)],
    ...
    perf_data = [["Hold portfolio value    ", f"{sr(perf.hold_value, precision)} {quote}"],
                 ["Current portfolio value ", f"{sr(perf.cur_value, precision)} {quote}"],
                 ["Trade P&L               ", f"{sr(perf.trade_pnl, precision)} {quote}"]]
    perf_data += [["Fees paid               ", f"{sr(amt, precision)} {tok}"] for tok, amt in perf.fees.items()]
    perf_data += [["Total P&L               ", f"{sr(perf.total_pnl, precision)} {quote}"],
