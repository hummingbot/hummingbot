...
def load_parser(hummingbot: "HummingbotApplication", command_tabs) -> ThrowingArgumentParser:
    ...
    create_parser.add_argument(
        "--v2-config",
        dest="script_to_config",
        nargs="?",
        default=None,
        help="Name of the v2 strategy (from conf/scripts/)",
    )
    create_parser.add_argument(
        "--controller-config", dest="controller_name", nargs="?", default=None, help="Name of the controller"
    )
    ...
    start_parser.add_argument(
        "--v2", type=str, dest="v2_conf", help="V2 strategy config file name (from conf/scripts/)"
    )
    ...
    lphistory_parser.add_argument(
        "-d", "--days", type=float, default=0, dest="days", help="How many days in the past (can be decimal value)"
    )
    lphistory_parser.add_argument(
        "-v", "--verbose", action="store_true", default=False, dest="verbose", help="List all LP position updates"
    )
    lphistory_parser.add_argument(
        "-p", "--precision", default=None, type=int, dest="precision", help="Level of precision for values displayed"
    )
    ...
    gateway_allowance_parser = gateway_subparsers.add_parser(
        "allowance", help="Check token allowances for ethereum connectors"
    )
    gateway_allowance_parser.add_argument(
        "connector", nargs="?", default=None, help="Ethereum connector name/type (e.g., uniswap/amm)"
    )
    gateway_allowance_parser.set_defaults(func=hummingbot.gateway_allowance)

    gateway_approve_parser = gateway_subparsers.add_parser(
        "approve", help="Approve token for use with ethereum connectors"
    )
    gateway_approve_parser.add_argument(
        "connector", nargs="?", default=None, help="Connector name/type (e.g., jupiter/router)"
    )
    ...
    gateway_balance_parser.add_argument(
        "tokens", nargs="?", default=None, help="Comma-separated list of tokens to check (optional)"
    )
    ...
    gateway_config_parser.add_argument(
        "namespace", nargs="?", default=None, help="Namespace (e.g., ethereum-mainnet, uniswap)"
    )
    ...
    gateway_connect_parser.add_argument(
        "chain", nargs="?", default=None, help="Blockchain chain (e.g., ethereum, solana)"
    )
    ...
    gateway_pool_parser.add_argument(
        "connector", nargs="?", default=None, help="Connector name/type (e.g., uniswap/amm)"
    )
    ...
    gateway_swap_parser = gateway_subparsers.add_parser("swap", help="Swap tokens")
    gateway_swap_parser.add_argument(
        "connector", nargs="?", default=None, help="Network (e.g., solana-mainnet-beta, ethereum-mainnet)"
    )
    gateway_swap_parser.add_argument(
        "args",
        nargs="*",
