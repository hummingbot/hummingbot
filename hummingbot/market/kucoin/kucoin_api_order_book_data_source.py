            ...
            for row in all_markets.itertuples():
                ...
                if product_name.startswith("USDT"):
                    ...
                    if mul in all_markets.index:
                        all_markets.loc[product_name, "USDVolume"] = float(row.volValue) * float(
                            all_markets.loc[mul, "last"])
                    elif div in all_markets.index:
                        all_markets.loc[product_name, "USDVolume"] = float(row.volValue) / float(
                            all_markets.loc[div, "last"])
        ...
        async with aiohttp.ClientSession() as client:
            ...
            for index, trading_pair in enumerate(trading_pairs):
                try:
                    ...
                    retval[trading_pair] = KucoinOrderBookTrackerEntry(trading_pair, snapshot_timestamp, order_book,
