...
cdef class VeridexTransactionTracker(TransactionTracker):
    cdef:
        VeridexMarket _owner

    def __init__(self, owner: VeridexMarket):
        super().__init__()
        self._owner = owner
...
cdef class VeridexMarket(MarketBase):
    ...
    def __init__(self,
        ...
        self._tx_tracker = VeridexTransactionTracker(self)
    ...
    @property
    def in_flight_limit_orders(self) -> Dict[str, VeridexInFlightOrder]:
        return self._in_flight_limit_orders

    @property
    def in_flight_market_orders(self) -> Dict[str, VeridexInFlightOrder]:
    ...
    def limit_orders(self) -> List[LimitOrder]:
        cdef:
            list retval = []
            VeridexInFlightOrder typed_in_flight_order
            str base_currency
            str quote_currency
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        self._in_flight_market_orders.update({
            key: VeridexInFlightOrder.from_json(value)
    ...
    async def get_active_exchange_markets(self):
        return await VeridexAPIOrderBookDataSource.get_active_exchange_markets()
    ...
    cdef c_start_tracking_limit_order(self,
        ...
        self._in_flight_limit_orders[order_id] = VeridexInFlightOrder(
    ...
    cdef c_start_tracking_market_order(self,
                                       ...
                                       object amount,
                                       str tx_hash):
        self._in_flight_market_orders[tx_hash] = VeridexInFlightOrder(
            client_order_id=order_id,
