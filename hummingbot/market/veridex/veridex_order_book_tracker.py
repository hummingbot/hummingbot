...
from hummingbot.core.data_type.order_book_message import (
    ...
    VeridexOrderBookMessage,
    OrderBookMessage,
)
from hummingbot.core.data_type.order_book_tracker_entry import VeridexOrderBookTrackerEntry
...
class VeridexOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
        ...
        self._order_books: Dict[str, VeridexOrderBook] = {}
        self._saved_message_queues: Dict[str, Deque[VeridexOrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, VeridexActiveOrderTracker] = defaultdict(VeridexActiveOrderTracker)
    ...
    def data_source(self) -> OrderBookTrackerDataSource:
        if not self._data_source:
            if self._data_source_type is OrderBookTrackerDataSourceType.EXCHANGE_API:
                self._data_source = VeridexAPIOrderBookDataSource(symbols=self._symbols)
            else:
    ...
    async def _refresh_tracking_tasks(self):
        ...
        available_pairs: Dict[str, VeridexOrderBookTrackerEntry] = await self.data_source.get_tracking_pairs()
        ...
        for symbol in new_symbols:
            order_book_tracker_entry: VeridexOrderBookTrackerEntry = available_pairs[symbol]
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ob_message: VeridexOrderBookMessage = await self._order_book_diff_stream.get()
                ...
                order_book: VeridexOrderBook = self._order_books[trading_pair_symbol]
    ...
    async def _track_single_book(self, symbol: str):
        past_diffs_window: Deque[VeridexOrderBookMessage] = deque()
        self._past_diffs_windows[symbol] = past_diffs_window

        message_queue: asyncio.Queue = self._tracking_message_queues[symbol]
        order_book: VeridexOrderBook = self._order_books[symbol]
        active_order_tracker: VeridexActiveOrderTracker = self._active_order_trackers[symbol]
        ...
        while True:
            try:
                message: VeridexOrderBookMessage = None
                saved_messages: Deque[VeridexOrderBookMessage] = self._saved_message_queues[symbol]
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    past_diffs: List[VeridexOrderBookMessage] = list(past_diffs_window)
