...
class KucoinOrderBookTracker(OrderBookTracker):
    _kuobt_logger: Optional[HummingbotLogger] = None

    @classmethod
    def logger(cls) -> HummingbotLogger:
        if cls._kuobt_logger is None:
            cls._kuobt_logger = logging.getLogger(__name__)
        return cls._kuobt_logger
    ...
    def data_source(self) -> OrderBookTrackerDataSource:
        if not self._data_source:
            if self._data_source_type is OrderBookTrackerDataSourceType.EXCHANGE_API:
                self._data_source = KucoinAPIOrderBookDataSource(symbols=self._symbols)
            else:
        ...
        while True:
            try:
                ...
                if message.type is OrderBookMessageType.DIFF:
                    # Kucoin websocket messages contain the entire order book state so they should be treated as snapshots
