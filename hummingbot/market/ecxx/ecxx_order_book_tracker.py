...
from hummingbot.market.ecxx.ecxx_api_order_book_data_source import EcxxAPIOrderBookDataSource


class EcxxOrderBookTracker(OrderBookTracker):
    _ecobt_logger: Optional[HummingbotLogger] = None

    @classmethod
    def logger(cls) -> HummingbotLogger:
        if cls._ecobt_logger is None:
            cls._ecobt_logger = logging.getLogger(__name__)
        return cls._ecobt_logger
    ...
    def data_source(self) -> OrderBookTrackerDataSource:
        if not self._data_source:
            if self._data_source_type is OrderBookTrackerDataSourceType.EXCHANGE_API:
                self._data_source = EcxxAPIOrderBookDataSource(symbols=self._symbols)
            else:
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ...
                # TODO: Check if it need to make sure to handle the trade stream
