...
class KyberOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
        ...
        self._order_books: Dict[str, KyberOrderBook] = {}
        self._saved_message_queues: Dict[str, Deque[KyberOrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._order_book_diff_stream: asyncio.Queue = asyncio.Queue()
        self._order_book_snapshot_stream: asyncio.Queue = asyncio.Queue()
        self._ev_loop: asyncio.BaseEventLoop = asyncio.get_event_loop()
        self._data_source: Optional[OrderBookTrackerDataSource] = None
        self._active_order_trackers: Dict[str, KyberActiveOrderTracker] = defaultdict(KyberActiveOrderTracker)
    ...
    def data_source(self) -> OrderBookTrackerDataSource:
        if not self._data_source:
            if self._data_source_type is OrderBookTrackerDataSourceType.REMOTE_API:
                ...
                self._data_source = KyberAPIOrderBookDataSource(trading_pairs=self._trading_pairs, chain=self._chain)
            else:
    ...
    async def _refresh_tracking_tasks(self):
        ...
        available_pairs: Dict[str, KyberOrderBookTrackerEntry] = await self.data_source.get_tracking_pairs()
        ...
        for trading_pair in new_trading_pairs:
            order_book_tracker_entry: KyberOrderBookTrackerEntry = available_pairs[trading_pair]
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ob_message: KyberOrderBookMessage = await self._order_book_diff_stream.get()
                ...
                order_book: KyberOrderBook = self._order_books[trading_pair]
    ...
    async def _track_single_book(self, trading_pair: str):
        past_diffs_window: Deque[KyberOrderBookMessage] = deque()
        self._past_diffs_windows[trading_pair] = past_diffs_window

        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: KyberOrderBook = self._order_books[trading_pair]
        active_order_tracker: KyberActiveOrderTracker = self._active_order_trackers[trading_pair]
        ...
        while True:
            try:
                message: KyberOrderBookMessage = None
                saved_messages: Deque[KyberOrderBookMessage] = self._saved_message_queues[trading_pair]
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    past_diffs: List[KyberOrderBookMessage] = list(past_diffs_window)
