...
from hummingbot.smart_components.meta_strategies.data_types import MetaStrategyMode, MetaStrategyStatus
...
class MetaExecutorBase:
    def __init__(self, strategy: ScriptStrategyBase, meta_strategy: MetaStrategyBase, update_interval: float = 1.0):
        ...
        self.status = MetaStrategyStatus.NOT_STARTED
    ...
    async def control_loop(self):
        self.on_start()
        self.status = MetaStrategyStatus.ACTIVE
        while not self.terminated.is_set():
            await self.control_task()
            await asyncio.sleep(self.update_interval)
        self.status = MetaStrategyStatus.TERMINATED
        self.on_stop()
