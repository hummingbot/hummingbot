    ...
    def simulate_execution(self, df, initial_portfolio_usd, trade_cost):
        ...
        for order_level in self.controller.config.order_levels:
            df = self.apply_triple_barrier_method(df,
                                                  tp=float(order_level.triple_barrier_config.take_profit),
                                                  sl=float(order_level.triple_barrier_config.stop_loss),
                                                  tl=int(order_level.triple_barrier_config.time_limit),
