...
class DManV6(GenericController):
    ...
    def create_actions_proposal(self) -> List[CreateDCAExecutorAction]:
        ...
        if dca_executors_df.empty:
            ...
            if n_long_dcas == 0:
                create_long_dca_flag = True
            elif n_long_dcas < self.config.max_dca_per_side and float(close_price) < float(min_long_dca_average_price) * (1 - self.config.min_distance_between_dca):
                create_long_dca_flag = True
            if n_short_dcas == 0:
                create_short_dca_flag = True
            elif n_short_dcas < self.config.max_dca_per_side and float(close_price) > float(max_short_dca_average_price) * (1 + self.config.min_distance_between_dca):
                create_short_dca_flag = True
