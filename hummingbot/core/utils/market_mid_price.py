...
BEAXY_PRICE_URL = "https://services.beaxy.com/api/v2/symbols/rates"
...
def get_mid_price(exchange: str, trading_pair: str) -> Optional[Decimal]:
    if exchange == "binance":
        ...
        return beaxy_mid_price(trading_pair)
    else:
        return binance_mid_price(trading_pair)
...
@cachetools.func.ttl_cache(ttl=10)
def beaxy_mid_price(trading_pair: str) -> Optional[Decimal]:
    resp = requests.get(url=BEAXY_PRICE_URL)
    records = resp.json()
    result = None
    for key, value in records.items():
        pair = BeaxyMarket.convert_from_exchange_trading_pair(key)
        if trading_pair == pair and value["bid"] is not None and value["ask"] is not None:
            result = (Decimal(str(value["bid"])) + Decimal(str(value["ask"]))) / Decimal("2")
            break
    return result
