...
class EvedexPerpetualRateSource(RateSourceBase):
    ...
    async def get_prices(self, quote_token: Optional[str] = None) -> Dict[str, Decimal]:
        ...
        try:
            ...
            self.logger().exception(
                msg="Unexpected error while retrieving rates from EvedEx Perpetual. Check the log file for more info.",
            )
        return results
    ...
    def _build_evedex_perpetual_connector_without_private_keys() -> 'EvedexPerpetualDerivative':
        ...
        return EvedexPerpetualDerivative(
            evedex_perpetual_api_key="",
            evedex_perpetual_private_key="",
            trading_pairs=[],
