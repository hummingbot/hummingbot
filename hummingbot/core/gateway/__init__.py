...
class GatewayPaths:
    ...
    @classmethod
    def get_default_paths(cls) -> "GatewayPaths":
        """
        """
        ...
        external_logs_path: Optional[Path] = getenv("LOGS_FOLDER") and Path(getenv("LOGS_FOLDER"))

        if inside_docker and not (external_certs_path and external_conf_path and external_logs_path):
            raise EnvironmentError("CERTS_FOLDER, GATEWAY_CONF_FOLDER and LOGS_FOLDER must be defined when running "
                                   "as container.")
