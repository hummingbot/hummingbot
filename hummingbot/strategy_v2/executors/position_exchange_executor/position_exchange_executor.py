...
class PositionExchangeExecutor(ExecutorBase):
    _logger = None
    ...
    def __init__(self, strategy: ScriptStrategyBase, config: PositionExchangeExecutorConfig,
                 update_interval: float = 1.0, max_retries: int = 10):
        """
        Initialize the PositionExchangeExecutor instance.

        :param strategy: The strategy to be used by the PositionExchangeExecutor.
        :param config: The configuration for the PositionExchangeExecutor, subclass of PositionExecutoConfig.
        :param update_interval: The interval at which the PositionExchangeExecutor should be updated, defaults to 1.0.
        :param max_retries: The maximum number of retries for the PositionExchangeExecutor, defaults to 5.
        """
        ...
        self.config: PositionExchangeExecutorConfig = config
    ...
    def close_price(self) -> Decimal:
        ...
        """
        if self._stop_loss_order and self._stop_loss_order.is_done:
            return self._stop_loss_order.average_executed_price
        if self._trailing_stop_order and self._trailing_stop_order.is_done:
            return self._trailing_stop_order.average_executed_price
        if self._take_profit_order and self._take_profit_order.is_done:
            return self._take_profit_order.average_executed_price
    ...
    def get_cum_fees_quote(self) -> Decimal:
        ...
        """
        orders = [self._open_order, self._close_order, self._take_profit_order, self._stop_loss_order, self._trailing_stop_order]
    ...
    def open_orders_completed(self):
        ...
        """
        return not self._open_order or self._open_order.is_done

    def open_and_close_volume_match(self):
        if self.open_filled_amount == Decimal("0"):
            return True
        return any((getattr(self, f"_{order}_order") and getattr(self, f"_{order}_order").is_filled for order in ["close", "take_profit", "stop_loss", "trailing_stop"]))
    ...
    def control_open_order(self):
        ...
        if self._open_order:
            if (
                    self._open_order.order and
                    not self._open_order.is_filled and
                    not self._is_within_activation_bounds(
                ...
                self.cancel_trailing_stop_order()

        elif self._is_within_activation_bounds(
                self.config.entry_price,
                self.config.side,
                self.config.triple_barrier_config.open_order_type
        ):
    ...
    def control_barriers(self):
        ...
        """
        if (
                self._open_order and
                self._open_order.is_filled and
                self.open_filled_amount >= self.trading_rules.min_order_size and
    ...
    def control_stop_loss(self):
        """
        Stop-loss is on the exchange. As a control, one could cancel/replace the stop-loss order if the price has moved

        :return: None
        """
        if self.config.triple_barrier_config.stop_loss and (not self._stop_loss_order or not self._trailing_stop_order):
    ...
    def control_trailing_stop(self):
        if self.config.triple_barrier_config.trailing_stop:
            ...
            if self._trailing_stop_trigger_pct and not self._trailing_stop_order:
                if net_pnl_pct - self.config.triple_barrier_config.trailing_stop.trailing_delta > self._trailing_stop_trigger_pct:
                    ...
                    if self._stop_loss_order and self._stop_loss_order.order_id:
                        self.cancel_stop_loss_order()
                    self.place_trailing_stop_order()

            elif net_pnl_pct > self.config.triple_barrier_config.trailing_stop.activation_price:
    ...
    async def control_close_order(self):
        ...
        # Close order can be close_order, take_profit_order, stop_loss_order or trailing_stop_order
        for name in ["_close_order", "_take_profit_order", "_stop_loss_order", "_trailing_stop_order"]:
            if (order := getattr(self, name)) and order.order and order.order.is_open:
                in_flight_order = self.get_in_flight_order(self.config.connector_name, order.order_id)
    ...
    def place_stop_loss_order(self):
        ...
        order_id = self.place_order(
            ...
            amount=self.config.amount,
            price=self.config.triple_barrier_config.stop_loss,
    ...
    def place_take_profit_order(self):
        ...
        order_id = self.place_order(
            ...
            amount=self.config.amount,
            price=self.config.triple_barrier_config.take_profit,
            side=self.close_order_side,
            position_action=PositionAction.OPEN,
            kwargs={"price_in_percent": True}
        )
    ...
    def place_trailing_stop_order(self):
        ...
        self._trailing_stop_order = TrackedOrder(order_id=order_id)
        self.logger().debug(f"Executor ID: {self.config.id} - Placing open order {order_id}")
    ...
    def cancel_stop_loss_order(self):
        """
        This method is responsible for canceling the stop loss order.
    ...
    def cancel_trailing_stop_order(self):
        """
        This method is responsible for canceling the stop loss order.

        :return: None
        """
        if self._trailing_stop_order and self._trailing_stop_order.order_id:
            self._strategy.cancel(
                connector_name=self.config.connector_name,
                trading_pair=self.config.trading_pair,
                order_id=self._trailing_stop_order.order_id
            )
            self.logger().debug("Removing trailing stop")
    ...
    def update_tracked_orders_with_order_id(self, order_id: str):
        ...
        elif self._take_profit_order and self._take_profit_order.order_id == order_id:
            self._take_profit_order.order = in_flight_order
        elif self._stop_loss_order and self._stop_loss_order.order_id == order_id:
    ...
    def process_order_completed_event(self, _, market, event: Union[BuyOrderCompletedEvent, SellOrderCompletedEvent]):
        ...
        if self._take_profit_order and self._take_profit_order.order_id == event.order_id:
            ...
            self._status = RunnableStatus.SHUTTING_DOWN
        elif self._stop_loss_order and self._stop_loss_order.order_id == event.order_id:
            self.close_type = CloseType.STOP_LOSS
            self._close_order = self._stop_loss_order
            self._stop_loss_order = None
            self._status = RunnableStatus.SHUTTING_DOWN
        elif self._trailing_stop_order and self._trailing_stop_order.order_id == event.order_id:
            self.close_type = CloseType.TRAILING_STOP
            self._close_order = self._trailing_stop_order
    ...
    def process_order_canceled_event(self, _, market: ConnectorBase, event: OrderCancelledEvent):
        ...
        if self._stop_loss_order and event.order_id == self._stop_loss_order.order_id:
            ...
            self._stop_loss_order = None
        if self._trailing_stop_order and event.order_id == self._trailing_stop_order.order_id:
            self._failed_orders.append(self._trailing_stop_order)
            self._trailing_stop_order = None
    ...
    def process_order_failed_event(self, _, market, event: MarketOrderFailureEvent):
        ...
        if self._open_order and event.order_id == self._open_order.order_id:
            ...
            self._current_retries += 1
        elif self._take_profit_order and event.order_id == self._take_profit_order.order_id:
            self._failed_orders.append(self._take_profit_order)
            self._take_profit_order = None
            self.logger().error(f"Take profit order failed {event.order_id}. Retrying {self._current_retries}/{self._max_retries}")
            self._current_retries += 1
        elif self._stop_loss_order and event.order_id == self._stop_loss_order.order_id:
            self._failed_orders.append(self._stop_loss_order)
            self._stop_loss_order = None
            self.logger().error(f"Take profit order failed {event.order_id}. Retrying {self._current_retries}/{self._max_retries}")
            self._current_retries += 1
