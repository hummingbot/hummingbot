...
from hummingbot.strategy.twap.twap_config_map import target_asset_amount_prompt
from hummingbot.strategy_v2.executors.executor_base import ExecutorBase
from hummingbot.strategy_v2.executors.scalping_executor.data_types import ScalpingExecutorConfig
from hummingbot.strategy_v2.models.base import RunnableStatus
from hummingbot.strategy_v2.models.executors import CloseType, TrackedOrder


class ScalpingExecutor(ExecutorBase):
    _logger = None
    profit = False
    ...
    def __init__(self, strategy: ScriptStrategyBase, config: ScalpingExecutorConfig,
        ...
        if config.triple_barrier_config.open_order_type != OrderType.LIMIT:
            error = "Only limit orders are supported for time_limit and stop_loss"
            self.logger().error(error)
            raise ValueError(error)
        super().__init__(strategy=strategy, config=config, connectors=[config.connector_name],
                         update_interval=update_interval)
        if not config.triple_barrier_config.open_order_price:
            error = "Only limit orders are supported for entry"
            self.logger().error(error)
            raise ValueError(error)
    ...
    def entry_price(self) -> Decimal:
        ...
        if self._open_order and self._open_order.is_done:
            ...
            if self.config.side == TradeType.BUY:
                ...
                return min(self.config.triple_barrier_config.open_order_price, best_bid)
            else:
                best_ask = self.get_price(self.config.connector_name, self.config.trading_pair, PriceType.BestAsk)
                return max(self.config.triple_barrier_config.open_order_price, best_ask)
        else:
            return self.config.triple_barrier_config.open_order_price
    ...
    def take_profit_amount(self) -> Optional[Decimal]:
        """
        Calculates the take profit amount to use when the target price is reached, expressed as a number
        of asset units. This function now returns one percent of the position amount if that value is greater than 0.001;
        otherwise, it returns 0.001.

        For example:
          - If position amount = 10, then 1% of 10 is 0.1, which is greater than 0.001, so it returns 0.1.
          - If position amount = 0.05, then 1% of 0.05 is 0.0005, which is less than 0.001, so it returns 0.001.

        :return: The take profit amount as a Decimal representing the quantity of asset units.
        """
        self.logger().debug(
            "Calculating take profit amount based on 1%% of the position amount, with a minimum of 0.001 asset units.")

        # Retrieve the position amount from the configuration.
        amount = self.config.amount
        self.logger().debug("Position amount: %s", amount)

        # Calculate one percent of the position amount.
        one_percent = amount * Decimal("0.01")
        self.logger().debug("One percent of position amount: %s", one_percent)

        # Return one_percent if it is higher than 0.001; otherwise, return 0.001.
        if one_percent > Decimal("0.001"):
            self.logger().debug("1%% of position amount (%s) is greater than 0.001, returning %s", one_percent,
                               one_percent)
            return one_percent
        else:
            self.logger().debug("1%% of position amount (%s) is less than or equal to 0.001, returning 0.None",
                               one_percent)
            return None
    ...
    def control_open_order(self):
        ...
        if not self._open_order:
            if self._is_within_activation_bounds(self.config.triple_barrier_config.open_order_price, self.config.side,
            ...
            if self._open_order.order and not self._open_order.is_filled and \
                    not self._is_within_activation_bounds(self.config.triple_barrier_config.open_order_price,
                                                          self.config.side,
                                                          self.config.triple_barrier_config.open_order_type):
    ...
    def _is_within_activation_bounds(self, order_price: Decimal, side: TradeType, order_type: OrderType) -> bool:
        ...
        """
        size = self.take_profit_amount()
        if size is None:
            self.config.profit_target_action=None
    ...
    def control_barriers(self):
        ...
        :return: None
        """
        self.logger().debug("Control barriers triggered.")

        if self._open_order and self._open_order.is_filled:
            self.logger().debug("Open order is filled.")
            self.logger().debug("Open filled amount: %s (min order size: %s)", self.open_filled_amount,
                               self.trading_rules.min_order_size)
            self.logger().debug("Open filled amount in quote: %s (min notional size: %s)", self.open_filled_amount_quote,
            ...
            if self.open_filled_amount >= self.trading_rules.min_order_size and \
                ...
                self.logger().debug("Trading rules conditions met for barrier control.")
                if not self.profit:
                    self.logger().debug("Profit control not yet triggered. Initiating take profit control.")
                    self.control_take_profit()
                    self.profit = True
                    self.logger().debug("Profit control triggered and profit flag set to True.")
                else:
                    self.logger().debug("Profit control already triggered; no further action.")
            else:
                self.logger().debug(
                    "Trading rules conditions NOT met: open_filled_amount: %s, open_filled_amount_quote: %s",
                    self.open_filled_amount, self.open_filled_amount_quote)
        else:
            self.logger().debug("No open order exists or open order is not filled; skipping barrier control.")

        # Uncomment the following line to enable time limit control as well.
    ...
    def control_stop_loss(self):
        ...
        :return: None
        """
        if not self.config.triple_barrier_config.stop_loss_in_open_order and self.config.triple_barrier_config.stop_loss_price:
            if self.net_pnl_pct <= -self.config.triple_barrier_config.stop_loss_price:
    ...
    def control_take_profit(self):
        ...
        :return: None
        """
        self.logger().info("Control take profit triggered. Net PnL percentage: %.5f", self.net_pnl_pct)

        if self.config.profit_target_action:
            if self.config.triple_barrier_config.take_profit_order_type.is_limit_type():
                tp_price = self.config.profit_target_action.tp_price
                self.logger().info("Take profit order type is LIMIT. Target TP price: %s", tp_price)

                is_within_activation_bounds = self._is_within_activation_bounds(
                    self.config.profit_target_action.tp_price,
                    self.close_order_side,
                    self.config.triple_barrier_config.take_profit_order_type
                )
                self.logger().debug("Activation bounds check for TP: %s", is_within_activation_bounds)

                if not self._take_profit_limit_order:
                    if is_within_activation_bounds:
                        self.logger().info(
                            "No existing take profit order and within activation bounds. Placing take profit limit order.")
                        self.place_take_profit_limit_order()
                    else:
                        self.logger().info(
                            "No existing take profit order and NOT within activation bounds. Not placing order.")
                else:
                    if self._take_profit_limit_order.is_open and not self._take_profit_limit_order.is_filled:
                        if not is_within_activation_bounds:
                            self.logger().info(
                                "Existing take profit order is open and not filled, but activation bounds not met. Cancelling take profit order.")
                            self.cancel_take_profit()
                        else:
                            self.logger().info(
                                "Existing take profit order is open and within activation bounds. No action needed.")
                    else:
                        self.logger().info("Existing take profit order is not open or already filled.")
            elif self.net_pnl_pct >= self.config.triple_barrier_config.take_profit_price:
                self.logger().info(
                    "Net PnL percentage (%.5f) exceeds the take profit threshold (%.5f). Placing close order.",
                    self.net_pnl_pct, self.config.triple_barrier_config.take_profit_price)
    ...
    def place_take_profit_limit_order(self):
        ...
        order_id = self.place_order(
            ...
            trading_pair=self.config.trading_pair,
            amount=size,
            price=self.config.profit_target_action.tp_price,
            order_type=self.config.triple_barrier_config.take_profit_order_type,
            position_action=PositionAction.TP,
            side=self.close_order_side,
            stop_loss_price=0
        )
...
| PNL (%): {self.net_pnl_pct * 100:.2f}% | PNL (abs): {self.net_pnl_quote:.6f} {quote_asset} | Close Type: {self.close_type}
        ...
        if self.is_trading:
            ...
            if self.config.triple_barrier_config.time_limit:
                ...
                time_progress = (
                                        self.config.triple_barrier_config.time_limit - seconds_remaining) / self.config.triple_barrier_config.time_limit
            ...
            if self.config.triple_barrier_config.take_profit and self.config.triple_barrier_config.stop_loss:
                ...
                stop_loss_price = self.entry_price * (
                        1 - self.config.triple_barrier_config.stop_loss) if self.config.side == TradeType.BUY \
                    else self.entry_price * (1 + self.config.triple_barrier_config.stop_loss)
                take_profit_price = self.entry_price * (
                        1 + self.config.triple_barrier_config.take_profit) if self.config.side == TradeType.BUY \
