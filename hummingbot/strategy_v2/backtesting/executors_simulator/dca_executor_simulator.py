...
class DCAExecutorSimulator(ExecutorSimulatorBase):
    ...
    def simulate(self, df: pd.DataFrame, config: DCAExecutorConfig, trade_cost: float) -> ExecutorSimulation:
        ...
        for i in range(len(config.prices)):
            ...
            break_even_price = (
                DCAExecutorSimulator.break_even_price_at_index(config.prices, config.amounts_quote, i)
                if i > 0
                else price
            )

            entry_condition = (
                (df_filtered["close"] <= price) if config.side == TradeType.BUY else (df_filtered["close"] >= price)
            )
            ...
            if trailing_sl_trigger_pct is not None and trailing_sl_delta_pct is not None:
                ...
                if config.side == TradeType.BUY:
                    ...
                    if ts_activated_condition.any():
                        ...
                        returns_df.loc[ts_activated_condition, "ts_trigger_price"] = (
                            returns_df[ts_activated_condition]["close"] * float(1 - trailing_sl_delta_pct)
                        ).cummax()
                    ...
                    if ts_activated_condition.any():
                        ...
                        trailing_stop_condition = returns_df["close"] >= returns_df["ts_trigger_price"]
                trailing_sl_timestamp = (
                    returns_df[trailing_stop_condition]["timestamp"].min()
                    if trailing_stop_condition is not None
                    else None
                )
            ...
            if config.take_profit:
                ...
                take_profit_condition = (
                    returns_df["close"] >= take_profit_price
                    if config.side == TradeType.BUY
                    else returns_df["close"] <= take_profit_price
                )
            ...
            if is_last_order and config.stop_loss:
                ...
                stop_loss_condition = (
                    returns_df["low"] <= stop_loss_price
                    if config.side == TradeType.BUY
                    else returns_df["high"] >= stop_loss_price
                )
                stop_loss_timestamp = returns_df[stop_loss_condition]["timestamp"].min()
            else:
                next_order_condition = (
                    returns_df["close"] <= config.prices[i + 1]
                    if config.side == TradeType.BUY
                    else returns_df["close"] >= config.prices[i + 1]
                )
        ...
        for i, dca_stage in enumerate(potential_dca_stages):
            if dca_stage["close_type"] is None:
                df_filtered.loc[
                    df_filtered["timestamp"] >= dca_stage["entry_timestamp"], f"filled_amount_quote_{i}"
                ] = dca_stage["amount"]
                df_filtered.loc[df_filtered["timestamp"] >= dca_stage["entry_timestamp"], f"net_pnl_quote_{i}"] = (
                    dca_stage["cumulative_returns"] * dca_stage["amount"]
                )
                df_filtered.loc[
                    df_filtered["timestamp"] >= dca_stage["entry_timestamp"], "current_position_average_price"
                ] = dca_stage["break_even_price"]
            else:
                df_filtered.loc[
                    df_filtered["timestamp"] >= dca_stage["entry_timestamp"], f"filled_amount_quote_{i}"
                ] = dca_stage["amount"]
        ...
        df_filtered["filled_amount_quote"] = sum(
            [df_filtered[f"filled_amount_quote_{i}"] for i in range(len(potential_dca_stages))]
        )
        df_filtered["net_pnl_quote"] = sum(
            [df_filtered[f"net_pnl_quote_{i}"] for i in range(len(potential_dca_stages))]
        )
        ...
        simulation = ExecutorSimulation(config=config, executor_simulation=df_filtered, close_type=close_type)
        return simulation
