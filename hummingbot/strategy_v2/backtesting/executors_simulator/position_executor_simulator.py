...
class PositionExecutorSimulator(ExecutorSimulatorBase):
    def simulate(self, df: pd.DataFrame, config: PositionExecutorConfig, trade_cost: float) -> ExecutorSimulation:
        if config.triple_barrier_config.open_order_type.is_limit_type():
            entry_condition = (
                (df["close"] <= config.entry_price)
                if config.side == TradeType.BUY
                else (df["close"] >= config.entry_price)
            )
        ...
        df_filtered.loc[df_filtered["timestamp"] >= start_timestamp, "filled_amount_quote"] = (
            float(config.amount) * entry_price
        )
        ...
        if config.triple_barrier_config.stop_loss:
            ...
            sl_condition = (
                df_filtered["low"] <= sl_price if config.side == TradeType.BUY else df_filtered["high"] >= sl_price
            )
            first_sl_timestamp = df_filtered[sl_condition]["timestamp"].min()
        first_trailing_sl_timestamp = (
            df_filtered[(~df_filtered["ts"].isna()) & (df_filtered["net_pnl_pct"] < df_filtered["ts"])][
                "timestamp"
            ].min()
