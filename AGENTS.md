...
command-line interface** — fully non-interactive and scriptable: every command prints a table by
default, accepts `--json` for machine-readable output, and returns a **stable exit code**. No API
server, no message broker, no MCP — `hbot` itself is the interface you drive over the shell.
...
- **Use `--json` and branch on the exit code**, not on table text
...
- **Don't guess** strategy or field names — confirm with `hbot strategy list` and
  `hbot strategy show <strategy> --json`.
