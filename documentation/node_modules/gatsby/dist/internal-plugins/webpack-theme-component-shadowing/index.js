...
module.exports = (_temp = class GatsbyThemeComponentShadowingResolverPlugin {
  ...
  resolveComponentPath({
    ...
    const themesArray = [path.join(this.projectRoot, `src`, theme)].concat(Array.from(themes).reverse().map(({
      themeDir
    ...
    for (const theme of themesArray) {
      ...
      const isExactPath = existsDir.includes(path.basename(possibleComponentPath) // find if there is an exact path match
      );
      if (isExactPath) return possibleComponentPath; // if no exact path, search for extension

      const matchingExtension = this.extensions.find(ext => existsDir.includes(path.basename(possibleComponentPath) + ext));
