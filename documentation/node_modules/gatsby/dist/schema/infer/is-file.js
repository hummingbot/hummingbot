...
const findAncestorNode = (childNode, predicate) => {
  ...
  do {
    ...
    node = (0, _nodes.getNode)(node.parent);
...
const getFilePath = (fieldPath, relativePath) => {
  ...
  const node = (0, _nodes.getNodesByType)(typeName).find(node => getFirstValueAt(node, selector) === normalizedPath);
...
const isFile = (fieldPath, relativePath) => {
  ...
  const filePathExists = (0, _nodes.getNodesByType)(`File`).some(node => node.absolutePath === filePath);
