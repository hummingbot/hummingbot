...
function createStructuredLoggingDiagnosticsMiddleware(store) {
  ...
  function generateStuckStatusDiagnosticMessage() {
    ...
    return JSON.stringify(Object.values(activities).filter(activity => (0, _utils.isActivityInProgress)(activity.status)).map(activity => {
      ...
      if (stuckStatusWatchdogTimeoutDelay) {
        ...
        if (currentStatus === _constants.ActivityStatuses.InProgress) {
          stuckStatusWatchdogTimer = setTimeout(function fatalStuckStatusHandler() {
            reporter.panic(`Terminating the process (due to GATSBY_WATCHDOG_STUCK_STATUS_TIMEOUT):\n\nGatsby is in "${store.getState().logs.status}" state without any updates for ${(stuckStatusWatchdogTimeoutDelay / 1000).toFixed(3)} seconds. Activities preventing Gatsby from transitioning to idle state:\n\n${generateStuckStatusDiagnosticMessage()}`);
