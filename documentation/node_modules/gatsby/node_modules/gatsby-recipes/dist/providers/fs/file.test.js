...
describe(`file resource`, () => {
  test(`e2e file resource test`, async () => {
    ...
    _fsExtra.default.readFile.mockReturnValue(initialObject.content);
    ...
    const validateResult = _joi.default.validate(createResponse, { ...file.schema,
    ...
    _fsExtra.default.readFile.mockReturnValueOnce(partialUpdate.content);
  ...
  test(`e2e remote file resource test`, async () => {
    ...
    _fsExtra.default.readFile.mockReturnValue(response1);
    ...
    _fsExtra.default.createWriteStream.mockImplementation(() => new PassThrough());
    ...
    _fsExtra.default.readFile.mockReturnValueOnce(``);

    const createPlan = await file.plan(context, initialObject);
    expect(createPlan.currentState).toEqual(``);
    expect(createPlan.newState).toEqual(response1); // Test creating the resource
    ...
    expect(validateResult.error).toBeNull();
    expect(createResponse.content).toEqual(response1); // Test reading the resource
