...
| [Price Band](./price-band) | `price_ceiling` | `Enter the price point above which only sell orders will be placed` | Place only sell orders when market goes above this price. |
| [Price Band](./price-band) | `price_floor` | `Enter the price below which only buy orders will be placed` | Place only buy orders when market falls below this price. |
