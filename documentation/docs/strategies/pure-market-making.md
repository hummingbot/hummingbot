...
| **filled_order_replenish_wait_time** | An amount in seconds, which specifies the delay before placing the next order for single order mode. _Default value: 10 seconds_. <br/> For example if your buy order for gets filled at 1:00:00, your next orders are placed at 1:00:10 if delay is 10 seconds. The sell order is also cancelled and placed at this time to ensure both buy and sell orders use the same reference mid price and are in sync.
