...
| `Would you like to enable inventory skew? (y/n) (Default is no) >>>` <br /><br /> `What is your target base asset inventory percentage? (Enter 0.01 to indicate 1%, default is 0.5 (50%)) >>>` | More information in [Inventory-Based Dynamic Order Sizing](#inventory-based-dynamic-order-sizing) section. |
...
| `Would you like to use an external pricing source for mid-market price? (y/n) (Default is no) >>> ` | More information in [External Pricing Source Configuration](#external-pricing-source-configuration) section. |
