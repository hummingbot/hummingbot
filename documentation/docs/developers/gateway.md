...
Hummingbot Gateway is middleware that allows Hummingbot to connect to decentralized exchanges like Uniswap. Gateway can be found in the `gateway` directory in the Hummingbot repository.
...
2. Stuck and dropped transactions
...
1. Better protection for the SSL private key passphrase on Hummingbot side and gateway side
...
2. Eliminating the need to pass the wallet private key in gateway API calls.
...
* Repurpose `create-gateway.sh` for standalone gateway setup. Since we cannot depend on Hummingbot on creating the SSL certificates, it should now create the SSL certificates, including the certificate authority
* Add `create-gateway-client-cert.sh`, which generates client SSL keys and certificates, and signs the certs from the certificate authority generated from `create-gateway.sh`
* Provision detailed, step-by-step documentations on the preparation steps, setup steps and the configurations involved in standalone gateway setup. Also, add a "Hello World" example for developers to start developing with Gateway API
...
2. Remove global config file, use purpose-specific config files instead.
  ...
  * `ssl.yml` - SSL certificate configurations.
  * `ethereum.yml` - Ethereum chain-wide configurations. e.g. which network to use? Infura node ID?
  * `local.yml` - user overrides.
...
2. Code walkthrough with Uniswap connector
...
3. Things to watch out and test for
...
1. Instructions for running unit test cases, and interfacing with gateway.

  One of the first type of feedback a developer gets on his code, is typically from test cases. This can come in the form of unit test cases, or manual test cases (e.g. invoking curlwith special instructions). Documentations that target the use and creation of test cases would be useful here.
...
2. Test fixtures to simulate common edge cases like dropped transactions and transaction failures.
...
3. Process for submitting and certifying new connectors.
