...
We added network status detection to our market and wallet connectors. If you run a bot like `arbitrage` that trades on two markets, the bot won't place trades if one of the markets is offline. If a market is offline, the `status` command will display a warning.
...
- `history`: displays information about the trades that the bot has executed during the current session
...
- Fixed a bug with the `status` output of the arbitrage strategy
- Fixed a bug in which duplicate orders may appear on DDEX. We have discovered an issue with the DDEX API and are working to isolate and rectify the bug.
