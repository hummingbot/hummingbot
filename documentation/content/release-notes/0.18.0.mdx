...
Hummingbot can now be installed easily on Windows and macOS using a setup/install package available for download and updated with every new version release.
...
- Fixed issues with performance analysis where the asset is initialized with current trading pair but the trades being analyzed can include other trading pair that causes a key error during processing: [#974](https://github.com/CoinAlpha/hummingbot/issues/974)
...
- Fixed unit tests for IDEX market and other tests that do not run in the development branch.
- Strategy config files will now use a base-quote (e.g., ETH-USDT) format. Hummingbot users can continue using the exchange's trading pair syntax.
