...
🚀 Welcome to `hummingbot` version 0.25.0! In this release, we continued working on the Liquidity Mining back-end's stability, scaling/optimizing the infrastructure for quicker updates and increased capacity. There are also major improvements in pure market-making strategy, including inventory skew, status outputs, hanging order mode, and filled order delays.

## 🚰 Updated dependencies to improve stability

To address memory leaks that cause the bot to crash, we upgraded Python to version 3.8 and `prompt-toolkit`, the client interface framework. These fixes should significantly improve bot stability and reduce crashes due to memory leaks.

In addition, we advise users to not `stop` and `start` the bot frequently before running it long-term. This appears to elevate memory usage and cause the bot to crash more often. We continue to monitor this issue.

<Callout
  type="note"
  body="`Updating from source` Due to the updated Python version, users who install Hummingbot from source need to run `./uninstall` and `./clean` before updating."
/>
...
Hanging orders are now tracked as active orders when sending the `status` command. These orders will get canceled after sending the `stop` or `exit` command. Also, users can now specify through `cancel_hanging_order_pct` to cancel hanging orders when their spreads are above a certain value.
...
- Minimum order amount validation now applies to the `order_start_size` prompt in multiple order mode
