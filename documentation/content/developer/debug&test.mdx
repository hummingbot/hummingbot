...
This section will break down some of the ways to debug and test the code. You are not entirely required to use the options during your development process.
...
Below is a list of items required for the Unit Tests:
...
3.  Exchange Class Unit Tests | `test_*_exchange.py`<br/>
    ...
    This test determines if the connector can place and manage orders.<br/>
    All the tests below must pass successfully on both real API calls and mocked API calls mode.<br/>
    The mocked API calls mode facilitates testing where we can run tests as often as we want without incurring costs in transactions and slippage.<br/>
...
- Start a new server for each web socket connection<br/>
...
Make sure to set some delay if the sequence of responses matters. In the above example, data2 is supposed to arrive after data1
...
<Callout
  ...
  body="Ensure that you have enough asset balance before testing. Also, document the #minimum# and #recommended# asset balance to run the tests. 
    This is to aid testing during the PR review process. Please see `test/integration/test_binance_market.py` as an example of how this task is done."
/>
...
it would be easier to test these in the aiopython console. Click [here](https://aioconsole.readthedocs.io/en/latest/) for some documentation on how to use aiopython.
...
Below is just a short example of how to write a short asynchronous function to mimic an API request to place an order and displaying the response received.
...
This option, like in Option 2, is mainly used to test specific functions. This is primarily useful when debugging how various functions/classes interact with one another.

i.e. Initializing a simple WebSocket connection to listen and output all captured messages to examine the user stream message when placing/canceling an order.
...
Below is a sample code for executing a LIMIT-BUY order on Bittrex. This script tests the `BittrexAuth` class and outputs the response from the API server.
