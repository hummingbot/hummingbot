...
from kairos.core.clock_mode import ClockMode
from kairos.core.data_type.common import PositionMode, TradeType
from kairos.core.event.events import OrderType
...
from kairos.strategy_v2.models.executor_actions import CreateExecutorAction
from kairos.strategy_v2.models.executors import CloseType
from kairos.strategy_v2.models.executors_info import ExecutorInfo, PerformanceReport
...
class TestStrategyV2Base(IsolatedAsyncioWrapperTestCase):
    def setUp(self):
        ...
        with patch('asyncio.create_task', return_value=MagicMock()):
            ...
            with patch("kairos.strategy.strategy_v2_base.StrategyV2Base.listen_to_executor_actions", return_value=AsyncMock()):
                with patch('kairos.strategy.strategy_v2_base.ExecutorOrchestrator') as MockExecutorOrchestrator:
                    with patch('kairos.strategy.strategy_v2_base.MarketDataProvider') as MockMarketDataProvider:
    ...
    @patch("kairos.data_feed.market_data_provider.MarketDataProvider.ready", new_callable=PropertyMock)
    @patch("kairos.strategy_v2.executors.executor_orchestrator.ExecutorOrchestrator.execute_action")
...
class StrategyV2BaseBasicTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch('asyncio.create_task', return_value=MagicMock()):
            with patch("kairos.strategy.strategy_v2_base.StrategyV2Base.listen_to_executor_actions", return_value=AsyncMock()):
                with patch('kairos.strategy.strategy_v2_base.ExecutorOrchestrator'):
                    with patch('kairos.strategy.strategy_v2_base.MarketDataProvider'):
