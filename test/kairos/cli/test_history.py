...
from kairos.cli.commands.history import _balances_for_market, history
...
class HistoryCommandTest(unittest.TestCase):
    def _run(self, fills, *, name=None, days=None, balances_status=None, running=False,
        ...
        with patch("kairos.cli.commands._common.resolve_db_for_command",
                   return_value=resolved) as resolve_mock, \
                patch("kairos.cli.data.get_trades", return_value=fills) as get_trades, \
                patch("kairos.client.performance.PerformanceMetrics.create", perf_create), \
                patch.object(bot, "read_status", return_value=balances_status), \
                patch("kairos.cli.commands.history.echo") as echo_mock:
    ...
    def test_running_hint_wording(self):
        fills = [fill()]
        status = {"balances": {}}
        with patch("kairos.cli.commands.status._request_fresh_snapshot"):
    ...
    def test_running_with_no_cached_balances_requests_snapshot(self):
        ...
        with patch("kairos.cli.commands._common.resolve_db_for_command",
                ...
                patch("kairos.cli.data.get_trades", return_value=fills), \
                patch("kairos.client.performance.PerformanceMetrics.create",
