...
from kairos.cli.commands.logs import _resolve_log_file, logs
...
class LogsCommandTest(unittest.TestCase):
    ...
    def test_no_log_file_yet_exits_error(self):
        with patch("kairos.cli.commands.logs._resolve_log_file", return_value=None):
            with self.assertRaises(typer.Exit) as ctx:
    ...
    def test_snapshot_markdown(self):
        with patch("kairos.cli.commands.logs._resolve_log_file", return_value=self.log), \
                patch("kairos.cli.commands.logs.echo") as echo_mock:
    ...
    def test_snapshot_json(self):
        with patch("kairos.cli.commands.logs._resolve_log_file", return_value=self.log), \
                patch("kairos.cli.commands.logs.emit") as emit_mock:
    ...
    def test_follow_streams_new_lines_until_interrupted(self):
        ...
        with patch("kairos.cli.commands.logs._resolve_log_file", return_value=self.log), \
                patch("kairos.cli.commands.logs.time") as time_mock, \
                patch("kairos.cli.commands.logs.echo") as echo_mock:
