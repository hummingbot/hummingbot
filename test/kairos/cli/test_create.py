...
from kairos.cli.commands.create import _collect_values, _resolve_strategy_type, create, create_config
...
class CollectValuesTest(unittest.TestCase):
    ...
    def test_stdin_then_set_pairs_with_set_winning(self):
        with patch("kairos.cli.commands.create.read_json_object_from_stdin",
...
class CreateCommandTest(unittest.TestCase):
    def test_command_renders_the_record(self):
        ...
        out = io.StringIO()
        with patch("kairos.cli.commands.create.create_config", return_value=record) as cc:
