...
from kairos.cli.commands.connect import (
    _collect_key_values,
    _connect_key_fields,
...
class ConnectCommandTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        patch("kairos.cli.commands.connect._connectable_exchanges",
              return_value=["binance", "kraken"]).start()
        self.security = patch("kairos.client.config.security.Security").start()
        self.security.connector_config_file_exists.return_value = False
        self.login = patch("kairos.cli.commands.connect.login").start()
    ...
    def test_no_arg_tests_connected_keys(self):
        ...
        with patch("kairos.cli.commands.connect.resolve_password", return_value="pw"), \
                patch("kairos.client.config.config_crypt.ETHKeyFileSecretManger"), \
                patch("kairos.user.user_balances.UserBalances") as ub_cls:
    ...
    def test_connector_without_api_keys_fails(self):
        with patch("kairos.client.settings.AllConnectorSettings") as acs:
    ...
    def test_rejected_key_value_fails(self):
        ...
        with patch("kairos.client.settings.AllConnectorSettings") as acs, \
                patch("kairos.client.config.config_helpers.ClientConfigAdapter", return_value=cfg), \
                patch("kairos.cli.commands._common.read_json_object_from_stdin",
