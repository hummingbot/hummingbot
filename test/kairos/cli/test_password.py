...
import kairos.cli.password as pw
from kairos.cli.password import resolve_password
...
class PasswordResolveTest(unittest.TestCase):
    ...
    def test_hidden_prompt_used_for_tty(self):
        with patch.dict("os.environ", {}, clear=True), \
                ...
                patch("kairos.cli.password.getpass.getpass", return_value="typed"):
            self.assertEqual(resolve_password(password_stdin=False), "typed")

    def test_empty_prompted_password_fails(self):
        with patch.dict("os.environ", {}, clear=True), \
                patch.object(sys.stdin, "isatty", return_value=True), \
                patch("kairos.cli.password.getpass.getpass", return_value=""):
    ...
    def test_confirm_mismatch_fails(self):
        with patch.dict("os.environ", {}, clear=True), \
                patch.object(sys.stdin, "isatty", return_value=True), \
                patch("kairos.cli.password.getpass.getpass", side_effect=["first", "second"]):
            with self.assertRaises(typer.Exit) as ctx:
    ...
    def test_confirm_match_succeeds(self):
        with patch.dict("os.environ", {}, clear=True), \
                patch.object(sys.stdin, "isatty", return_value=True), \
                patch("kairos.cli.password.getpass.getpass", side_effect=["same", "same"]):
...
class LoginFirstRunTest(unittest.TestCase):
    ...
    def _run_login(self, new_password_required: bool):
        with patch.object(pw, "resolve_password", return_value="pw"), \
                patch("kairos.client.config.config_helpers.load_client_config_map_from_file", return_value={}), \
                patch("kairos.client.config.config_crypt.ETHKeyFileSecretManger", return_value=MagicMock()), \
                patch("kairos.client.config.config_crypt.store_password_verification") as store, \
                patch("kairos.client.config.security.Security") as security:
    ...
    def test_unlock_keystore_bad_password_fails_config_error(self):
        with patch("kairos.client.config.config_crypt.ETHKeyFileSecretManger", return_value=MagicMock()), \
                patch("kairos.client.config.config_crypt.store_password_verification") as store, \
                patch("kairos.client.config.security.Security") as security:
            security.new_password_required.return_value = False
            security.login.return_value = False
            with self.assertRaises(typer.Exit) as ctx:
                pw.unlock_keystore("wrong")
