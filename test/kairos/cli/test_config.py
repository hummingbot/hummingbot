...
from kairos.cli.commands.config import _item_for, _leaf_items, _navigate, config
...
class ConfigRunTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.save_to_yml = patch("kairos.client.config.config_helpers.save_to_yml").start()
        self.running = patch("kairos.cli.bot.running", return_value=False).start()
        self.read_meta = patch("kairos.cli.bot.read_meta", return_value=None).start()
        self.read_loaded = patch("kairos.cli.bot.read_loaded", return_value=None).start()
    ...
    def _strategy(self, content: str, file="conf_x.yml", stype="v2-script", running=False) -> Path:
        ...
        dict_patch = patch.dict("kairos.cli.strategy_configs.TYPE_DIRS", {stype: Path(d.name)})
        dict_patch.start()
    ...
    def test_set_strategy_edit_key_error_fails(self):
        self._strategy("market: binance\n")
        with patch("kairos.cli.strategy_configs.edit_config", side_effect=KeyError("market")):
    ...
    def test_set_running_controller_updatable_field_applies_live(self):
        ...
        with patch("kairos.cli.strategy_configs.edit_config", return_value=(42, {"amount"})):
