...
class TestValidateAndNormalizeConnector(unittest.TestCase):
    ...
    @patch(
        "hummingbot.strategy_v2.executors.gateway_utils.GATEWAY_DEXS",
        [
            "jupiter/router",
            "meteora/clmm",
            "orca/clmm",
    ...
    def test_connector_not_found_with_suffix(self):
        ...
        result, success = validate_and_normalize_connector("nonexistent/router", "router", self.capture_error)
        self.assertFalse(success)
        self.assertIsNone(result)
        self.assertEqual(len(self.errors), 1)
    ...
    def test_base_name_not_found(self):
        ...
        result, success = validate_and_normalize_connector("nonexistent", "router", self.capture_error)
        self.assertFalse(success)
        self.assertIsNone(result)
        self.assertEqual(len(self.errors), 1)
        self.assertIn("not found in Gateway", self.errors[0])
