...
class TWAPUnitTest(unittest.TestCase):
    ...
    def test_with_insufficient_balance(self):
        ...
        self.assertEqual(0, len(self.limit_buy_strategy.active_bids))
        market_buy_events: List[BuyOrderCompletedEvent] = [
            t for t in self.buy_order_completed_logger.event_log if isinstance(t, BuyOrderCompletedEvent)
        ]
        ...
        self.clock.backtest_til(end_ts)
        self.assertEqual(0, len(self.limit_sell_strategy.active_asks))
        market_sell_events: List[SellOrderCompletedEvent] = [
            t for t in self.sell_order_completed_logger.event_log if isinstance(t, SellOrderCompletedEvent)
        ]
    ...
    def test_remaining_quantity_updated_after_cancel_order_event(self):
        ...
        self.market.trigger_event(
            MarketEvent.OrderCancelled, OrderCancelledEvent(self.market.current_timestamp, bid_order.client_order_id)
        )
        ...
        self.assertTrue(
            self._is_logged("INFO", f"Updating status after order cancel (id: {bid_order.client_order_id})")
        )
    ...
    def test_remaining_quantity_updated_after_failed_order_event(self):
        ...
        self.market.trigger_event(
            ...
            MarketOrderFailureEvent(self.market.current_timestamp, bid_order.client_order_id, OrderType.LIMIT),
        )

        self.assertEqual(0, len(self.limit_buy_strategy.active_bids))
        self.assertEqual(self.limit_buy_strategy._quantity_remaining, 2)

        self.assertTrue(self._is_logged("INFO", f"Updating status after order fail (id: {bid_order.client_order_id})"))
    ...
    def test_remaining_quantity_updated_after_expired_order_event(self):
        ...
        # Simulate order cancel
        self.market.trigger_event(
            MarketEvent.OrderExpired, OrderExpiredEvent(self.market.current_timestamp, bid_order.client_order_id)
        )
        ...
        self.assertTrue(
            self._is_logged("INFO", f"Updating status after order expire (id: {bid_order.client_order_id})")
        )
