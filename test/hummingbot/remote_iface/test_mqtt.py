...
class RemoteIfaceMQTTTests(LocalClassEventLoopWrapperTestCase, TestLoggerMixin):
    ...
    def setUpClass(cls):
        ...
        cls.hbapp.ev_loop = cls.local_event_loop
    ...
    def setUp(self) -> None:
        super().setUp()
        # self.run_async_with_timeout(read_system_configs_from_yml())
    ...
    def test_mqtt_command_balance_limit(self):
        ...
        self.local_event_loop.run_until_complete(self.wait_for_rcv(notify_topic, notify_msg))
    ...
    def test_mqtt_command_balance_limit_failure(
        ...
        self.local_event_loop.run_until_complete(self.wait_for_rcv(topic, msg, msg_key='data'))
    ...
    def test_mqtt_command_command_shortcuts(self):
        ...
        self.local_event_loop.run_until_complete(self.wait_for_rcv(reply_topic, reply_data, msg_key='data'))
    ...
    def test_mqtt_command_history(
        ...
        self.local_event_loop.run_until_complete(self.wait_for_rcv(history_topic, history_msg, msg_key='data'))
    ...
    def test_mqtt_command_import(
        ...
        self.local_event_loop.run_until_complete(self.wait_for_rcv(notify_topic, start_msg))
    ...
    def test_mqtt_command_start_sync(
        ...
        self.local_event_loop.run_until_complete(self.wait_for_rcv(
            notify_topic, '\nEnter "start" to start market making.'))
    ...
    def test_mqtt_command_status_async(
        ...
        self.local_event_loop.run_until_complete(asyncio.sleep(0.2))
    ...
    def test_mqtt_command_stop_sync(self):
        ...
        self.local_event_loop.run_until_complete(self.wait_for_rcv(notify_topic, wind_down_msg))
        self.assertTrue(self.is_msg_received(notify_topic, wind_down_msg))
        self.local_event_loop.run_until_complete(self.wait_for_rcv(notify_topic, canceling_msg))
        self.assertTrue(self.is_msg_received(notify_topic, canceling_msg))
        self.local_event_loop.run_until_complete(self.wait_for_rcv(notify_topic, stop_msg))
    ...
    def test_mqtt_event_buy_order_created(self):
        ...
        self.local_event_loop.run_until_complete(self.wait_for_rcv(events_topic, evt_type, msg_key = 'type'))
    ...
    def test_mqtt_gateway_check_health_restarts(
        ...
        self.start_mqtt()
        self.local_event_loop.run_until_complete(self.wait_for_logged("DEBUG", f"Started Heartbeat Publisher <hbot/{self.instance_id}/hb>"))
        self.local_event_loop.run_until_complete(self.wait_for_rcv(status_topic, 'online'))
        self.local_event_loop.run_until_complete(self.wait_for_logged("DEBUG", "Monitoring MQTT Gateway health for disconnections."))
        self.log_records.clear()
        health_mock.return_value = False
        self.restart_interval_mock.return_value = None
        self.local_event_loop.run_until_complete(self.wait_for_logged("WARNING", "MQTT Gateway is disconnected, attempting to reconnect."))
        fake_err = "'<=' not supported between instances of 'NoneType' and 'int'"
        self.local_event_loop.run_until_complete(self.wait_for_logged("ERROR", f"MQTT Gateway failed to reconnect: {fake_err}. Sleeping 10 seconds before retry."))
        self.assertFalse(
            self.is_logged(
                "WARNING",
        ...
        self.local_event_loop.run_until_complete(self.wait_for_logged("WARNING", "MQTT Gateway is disconnected, attempting to reconnect."))
        health_mock.return_value = True
        self.local_event_loop.run_until_complete(self.wait_for_logged("WARNING", "MQTT Gateway successfully reconnected."))
        self.assertTrue(
            self.is_logged(
                "WARNING",
                "MQTT Gateway successfully reconnected.",
