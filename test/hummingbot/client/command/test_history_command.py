...
class HistoryCommandTest(unittest.TestCase):
    ...
    def test_list_trades(self, notify_mock):
        ...
        df_str_expected = (
            f"\n  Recent trades:"
            f"\n    +---------------------+------------+----------+--------------+--------+---------+----------+------------+------------+-------+-------------------------------------------+---------------------+-----------------+"  # noqa: E501
            f"\n    | Timestamp           | Exchange   | Market   | Order_type   | Side   |   Price |   Amount |   Leverage | Position   | Age   | Order ID                                  | Exchange Order ID   | Trade ID        |"  # noqa: E501
            f"\n    |---------------------+------------+----------+--------------+--------+---------+----------+------------+------------+-------+-------------------------------------------+---------------------+-----------------|"  # noqa: E501
            f"\n    | 1970-01-01 07:00:00 | binance    | BTC-USDT | limit        | buy    |       1 |        2 |          1 |            | n/a   | {order_id} |                     | someExchangeId1 |"  # noqa: E501
            f"\n    | 1970-01-01 07:00:00 | binance    | BTC-USDT | limit        | buy    |       2 |        2 |          1 |            | n/a   | {order_id} |                     | someExchangeId2 |"  # noqa: E501
            f"\n    +---------------------+------------+----------+--------------+--------+---------+----------+------------+------------+-------+-------------------------------------------+---------------------+-----------------+"  # noqa: E501
        )
