...
class ConfigCommandTest(unittest.TestCase):
    ...
    def test_list_configs(self, notify_mock, get_strategy_config_map_mock):
        ...
        # Exclude instance_id parameter as it is autogenerated on startup
        # and gets a random UID
        df_str_expected = '\n'.join([line[:-1].rstrip() for line in
                                     df_str_expected.splitlines()
                                     if '| instance_id' not in line])
        captures[1] = '\n'.join([line[:-1].rstrip() for line in
                                 captures[1].splitlines()
                                 if '| instance_id' not in line])
        # Workaround to pass the test because the length of the lines vary.
        # An example is setting long string value for the instance_id parameter
        df_str_expected = '\n'.join([line for line in
                                     df_str_expected.splitlines()
                                     if '+' not in line])
        captures[1] = '\n'.join([line for line in captures[1].splitlines()
                                 if '+' not in line])
