...
class TestArbitrageExecutor(IsolatedAsyncioWrapperTestCase, TestLoggerMixin):
    ...
    def test_net_pnl(self):
        ...
        self.executor._buy_order.order.average_price = Decimal('100')
        self.executor._sell_order.order.average_price = Decimal('200')
        self.executor._buy_order.order.cum_fee_quote = Decimal('1')
        self.executor._sell_order.order.cum_fee_quote = Decimal('1')
