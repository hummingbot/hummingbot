...
class TestLighterSpotCandles(TestCandlesBase):
    ...
    def setUp(self) -> None:
        super().setUp()
        self.data_feed = LighterSpotCandles(
    ...
    def _success_subscription_mock():
        ...
        return {"type": "subscribed", "channel": "trade/100"}
    ...
    def test_name_property(self):
        """name should be lighter_{trading_pair}."""
        self.assertEqual(self.data_feed.name, f"lighter_{self.trading_pair}")
    ...
    async def test_initialize_exchange_data_resolves_spot_market_id(self):
        """initialize_exchange_data picks the spot market with correct symbol format {base}/{quote}."""
        order_books_response = {
            "order_books": [
                {"market_type": "perp", "symbol": "BTC", "market_id": 999},
                {"market_type": "spot", "symbol": "ETH/USDC", "market_id": 101},
                {"market_type": "spot", "symbol": "BTC/USDC", "market_id": self.MARKET_ID},
            ]
        }
    ...
    async def test_initialize_exchange_data_ignores_perp_markets(self):
        """A perp market with the same base symbol must not be selected."""
        order_books_response = {
            "order_books": [
                {"market_type": "perp", "symbol": "BTC/USDC", "market_id": 999},
            ]
        }
        with patch.object(self.data_feed._api_factory, "get_rest_assistant") as mock_get:
            mock_rest = AsyncMock()
    ...
    async def test_initialize_exchange_data_raises_when_market_not_found(self):
        """ValueError is raised when the requested spot symbol is absent."""
        order_books_response = {
            "order_books": [
                {"market_type": "spot", "symbol": "ETH/USDC", "market_id": 101},
            ]
    ...
    async def test_seed_populates_deque_on_ws_connect(
        ...
        # Seed returns data; no further WS messages → loop blocks waiting
    ...
    async def test_same_timestamp_trade_event_replaces_last_candle(
        ...
        # seed empty (deque already populated), trade event → returns updated candle
    ...
    async def test_wrong_channel_message_is_ignored(self, ws_connect_mock, fetch_candles_mock):
        ...
        # Deque still empty — wrong channel was ignored
        self.assertEqual(len(self.data_feed._candles), 0)
        # fetch_candles called exactly once for the seed, not again for the wrong-channel event
    ...
    def test_rest_url_property(self):
        ...
        from hummingbot.data_feed.candles_feed.lighter_spot_candles.lighter_spot_candles import REST_URL
        self.assertEqual(REST_URL, self.data_feed.rest_url)

    def test_parse_websocket_message_returns_none(self):
        """_parse_websocket_message always returns None (covers line 179)."""
        self.assertIsNone(self.data_feed._parse_websocket_message({}))
        self.assertIsNone(self.data_feed._parse_websocket_message({"channel": "trade:100", "data": {}}))
    ...
    async def test_listen_for_subscriptions_handles_connection_error(
            self, ws_connect_mock, mock_time, fetch_candles_mock):
        """ConnectionError during WS session must be caught and logged (covers lines 196, 210)."""
        mock_time.return_value = time.time()
        fetch_candles_mock.return_value = np.array([])

        # First call raises ConnectionError; second call blocks forever so the task can be cancelled
        call_count = 0
        ...
        async def side_effect(*args, **kwargs):
            ...
            # Second iteration: return a mock that never yields messages
    ...
    async def test_process_websocket_messages_ignores_non_dict_data(
        ...
        """Non-dict WS messages must be silently skipped (covers line 222)."""
        ws_connect_mock.return_value = self.mocking_assistant.create_websocket_mock()
        fetch_candles_mock.return_value = np.array([])

        # Send a non-dict raw string message
        self.mocking_assistant.add_websocket_aiohttp_message(
            websocket_mock=ws_connect_mock.return_value,
        ...
        # Candle deque still empty — non-dict message was ignored
        self.assertEqual(len(self.data_feed._candles), 0)
