...
class TestLighterPerpetualCandles(TestCandlesBase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.ex_trading_pair = cls.base_asset  # get_exchange_trading_pair returns "XRP"
        cls.max_records = 150
    ...
    def get_candles_rest_data_mock():
        return {
            "c": [
                {"t": 1748954160000, "o": 1.4000, "h": 1.4100, "l": 1.3900, "c": 1.4050, "v": 1000.0, "V": 1405.0},
                {"t": 1748954220000, "o": 1.4050, "h": 1.4120, "l": 1.4020, "c": 1.4080, "v": 850.0, "V": 1196.8},
                {"t": 1748954280000, "o": 1.4080, "h": 1.4150, "l": 1.4060, "c": 1.4130, "v": 1100.0, "V": 1554.3},
                {"t": 1748954340000, "o": 1.4130, "h": 1.4200, "l": 1.4100, "c": 1.4175, "v": 950.0, "V": 1346.6},
            ]
        }
    ...
    def get_fetch_candles_data_mock(self):
        return [
            ...
            [1748954280.0, 1.4080, 1.4150, 1.4060, 1.4130, 1100.0, 1554.3, 0.0, 0.0, 0.0],
            [1748954340.0, 1.4130, 1.4200, 1.4100, 1.4175, 950.0, 1346.6, 0.0, 0.0, 0.0],
        ]
    ...
    def test_name_property(self):
        self.assertEqual(self.data_feed.name, "lighter_perpetual_XRP-USDC")
    ...
    def test_parse_rest_candles_filters_future(self):
        data = self.get_candles_rest_data_mock()
        # Only candles with ts_seconds <= end_time should pass
    ...
    async def test_process_websocket_messages_duplicated_candle_not_included(
        ...
        self.assertAlmostEqual(float(self.data_feed._candles[-1][2]), 1.4110)
    ...
    async def test_polling_retries_after_exception(self, mock_fetch, mock_sleep):
        # First call raises, second raises CancelledError to stop
