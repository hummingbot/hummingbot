...
class DecibelPerpetualRateSourceTest(IsolatedAsyncioWrapperTestCase):
    ...
    def _get_mock_exchange(self, expected_rate: Decimal):
        ...
        async def mock_get_all_pairs_prices():
            return [
                {"trading_pair": "BTC-USD", "price": str(expected_rate)},
                {"trading_pair": "ETH-USD", "price": "3200.00"},
            ]
        ...
        async def mock_trading_pair_associated_to_exchange_symbol(trading_pair: str):
            symbol_map = {
                "BTC-USD": combine_to_hb_trading_pair("BTC", "USD"),
                "ETH-USD": combine_to_hb_trading_pair("ETH", "USD"),
            }
    ...
    async def test_get_decibel_prices(self):
        expected_rate = Decimal("50120")

        rate_source = DecibelPerpetualRateSource()
    ...
    async def test_get_decibel_prices_with_quote_filter(self):
    ...
    async def test_get_decibel_prices_with_non_matching_quote_filter(self):
        ...
        # Should raise ValueError since quote token is not USD
        with pytest.raises(ValueError, match="Decibel Perpetual only supports USD as quote token."):
            await rate_source.get_prices(quote_token="USDT")  # Not USD

    async def test_get_decibel_prices_exception_handling(self):
    ...
    def test_name_property(self):
        ...
        rate_source = DecibelPerpetualRateSource()
        self.assertEqual("decibel_perpetual", rate_source.name)
