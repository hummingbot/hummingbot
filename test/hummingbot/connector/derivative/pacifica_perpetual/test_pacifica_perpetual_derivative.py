...
class PacificaPerpetualDerivativeUnitTest(IsolatedAsyncioWrapperTestCase):
    ...
    def _mock_builder_code_approvals(self, req_mock, approvals):
        url = web_utils.public_rest_url(CONSTANTS.GET_BUILDER_CODE_APPROVALS_PATH_URL, domain=self.domain)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        req_mock.get(regex_url, payload={"success": True, "data": approvals, "error": None, "code": None})

    def test_builder_omitted_by_default(self):
        self.assertEqual(CONSTANTS.FOUNDATION_BUILDER_CODE, self.exchange._builder_code)
        self.assertFalse(self.exchange._should_inject_builder())

    def test_builder_omitted_when_not_supported(self):
        self.exchange._builder_approved = True
        with patch.object(CONSTANTS, "BUILDER_SUPPORTED", False):
            self.assertFalse(self.exchange._should_inject_builder())

    @aioresponses()
    async def test_initialize_builder_code_approved(self, req_mock):
        self._mock_builder_code_approvals(req_mock, [
            {
                "builder_code": CONSTANTS.FOUNDATION_BUILDER_CODE,
                "description": "Hummingbot Foundation",
                "max_fee_rate": "0.001",
                "updated_at": 1748970123456,
            }
        ])

        await self.exchange._initialize_builder_code()

        self.assertTrue(self.exchange._builder_approved)
        self.assertEqual(Decimal("0.001"), self.exchange._builder_max_fee_rate)
        self.assertTrue(self.exchange._should_inject_builder())

    @aioresponses()
    async def test_initialize_builder_code_not_approved(self, req_mock):
        self._mock_builder_code_approvals(req_mock, [
            {
                "builder_code": "OTHERBUILDER",
                "description": "Some other builder",
                "max_fee_rate": "0.001",
                "updated_at": 1748970123456,
            }
        ])
        self.exchange._builder_approved = True
        self.exchange._builder_max_fee_rate = Decimal("99")

        await self.exchange._initialize_builder_code()

        self.assertFalse(self.exchange._builder_approved)
        self.assertEqual(Decimal("0"), self.exchange._builder_max_fee_rate)
        self.assertFalse(self.exchange._should_inject_builder())

    @aioresponses()
    async def test_initialize_builder_code_fails_safe_to_no_injection(self, req_mock):
        url = web_utils.public_rest_url(CONSTANTS.GET_BUILDER_CODE_APPROVALS_PATH_URL, domain=self.domain)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        req_mock.get(regex_url, exception=Exception("endpoint down"))
        self.exchange._builder_approved = True
        self.exchange._builder_max_fee_rate = Decimal("99")
    ...
    @aioresponses()
    async def test_initialize_builder_code_skipped_when_not_supported(self, req_mock):
        with patch.object(CONSTANTS, "BUILDER_SUPPORTED", False):
            await self.exchange._initialize_builder_code()
        self.assertFalse(self.exchange._builder_approved)
        # No request mocked: a call to the approvals endpoint would have raised.

    @aioresponses()
    async def test_place_order_includes_builder_code_when_approved(self, req_mock):
        await self._simulate_trading_rules_initialized()
        url = web_utils.public_rest_url(CONSTANTS.CREATE_LIMIT_ORDER_PATH_URL, domain=self.domain)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        req_mock.post(regex_url, payload={"success": True, "data": {"order_id": 123456789}})
        self.exchange._builder_approved = True
        ...
        sent_request = next(calls[0] for key, calls in req_mock.requests.items() if key[0] == "POST")
        sent_data = json.loads(sent_request.kwargs["data"])
        self.assertEqual(CONSTANTS.FOUNDATION_BUILDER_CODE, sent_data["builder_code"])

    @aioresponses()
    async def test_place_order_omits_builder_code_when_not_approved(self, req_mock):
        await self._simulate_trading_rules_initialized()
        url = web_utils.public_rest_url(CONSTANTS.CREATE_MARKET_ORDER_PATH_URL, domain=self.domain)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        req_mock.post(regex_url, payload={"success": True, "data": {"order_id": 987654321}})
        ...
        sent_request = next(calls[0] for key, calls in req_mock.requests.items() if key[0] == "POST")
        sent_data = json.loads(sent_request.kwargs["data"])
        self.assertNotIn("builder_code", sent_data)
