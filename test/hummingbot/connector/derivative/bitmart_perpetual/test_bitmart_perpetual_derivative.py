...
class BitmartPerpetualDerivativeUnitTest(IsolatedAsyncioWrapperTestCase):
    ...
    def _get_position_mode_mock_response(position_mode: str = "hedge_mode"):
        position_mode_resp = {
            "code": 1000,
            "message": "Ok",
            "data": {"position_mode": position_mode},
    ...
    async def test_sell_order_fill_event_takes_fee_from_update_event(self):
        ...
        partial_fill = self._get_order_channel_mock_response(
            amount=Decimal("5"), deal_size="2", fill_qty="2", last_trade_id=1234
        )
        ...
        complete_fill = self._get_order_channel_mock_response(
            amount=Decimal("5"), state=4, deal_size="5", fill_qty="3", last_trade_id=1235
        )
    ...
    async def test_order_fill_event_ignored_for_repeated_trade_id(self):
        ...
        partial_fill = self._get_order_channel_mock_response(
            amount=Decimal("5"), state=2, deal_size="2", fill_qty="2", last_trade_id=1234
        )
    ...
    def test_set_position_mode_successful(self, mock_api):
        ...
        url = web_utils.private_rest_url(path_url=CONSTANTS.SET_POSITION_MODE_URL, domain=self.domain)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))

        mock_api.post(regex_url, body=json.dumps(response))

        success, msg = self.async_run_with_timeout(
            self.exchange._trading_pair_position_mode_set(mode=PositionMode.HEDGE, trading_pair=trading_pair)
        )
    ...
    def test_set_position_mode_once(self, mock_api):
        ...
        success, msg = self.async_run_with_timeout(
            self.exchange._trading_pair_position_mode_set(mode=PositionMode.HEDGE, trading_pair=trading_pairs[0])
        )
        self.assertEqual(success, True)
        self.assertEqual(msg, "")

        success, msg = self.async_run_with_timeout(
            self.exchange._trading_pair_position_mode_set(mode=PositionMode.HEDGE, trading_pair=trading_pairs[1])
        )
        self.assertEqual(success, True)
