...
class BackpackPerpetualAPIOrderBookDataSourceUnitTests(IsolatedAsyncioWrapperTestCase):
    ...
    async def test_listen_for_subscriptions_subscribes_to_channels(self, ws_connect_mock):
        ...
        self.mocking_assistant.add_websocket_aiohttp_message(
            websocket_mock=ws_connect_mock.return_value, message=json.dumps(result_subscribe_funding_rates)
        )
        ...
        expected_funding_subscription = {"method": "SUBSCRIBE", "params": [f"markPrice.{self.ex_trading_pair}"]}
    ...
    @aioresponses()
    @patch(
        "hummingbot.connector.derivative.backpack_perpetual.backpack_perpetual_api_order_book_data_source"
    ...
    async def test_subscribe_funding_info_raises_exception_and_logs_error(self):
        ...
        self.assertTrue(
            self._is_logged(
                "ERROR", f"Unexpected error occurred subscribing to funding info for {self.ex_trading_pair}..."
            )
        )
