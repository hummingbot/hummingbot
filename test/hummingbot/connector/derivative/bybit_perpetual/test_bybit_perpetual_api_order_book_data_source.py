...
class BybitPerpetualAPIOrderBookDataSourceTests(IsolatedAsyncioWrapperTestCase):
    ...
    async def test_unsubscribe_from_trading_pair_websocket_not_connected(self):
        ...
        self.assertTrue(
            self._is_logged(
                "WARNING", "Cannot unsubscribe from ETH-USDT: linear (USDT-margined) WebSocket not connected"
            )
        )
    ...
    async def test_unsubscribe_from_trading_pair_raises_exception_and_logs_error(self):
        ...
        self.assertFalse(result)
        self.assertTrue(self._is_logged("ERROR", "Error unsubscribing from ETH-USDT"))
