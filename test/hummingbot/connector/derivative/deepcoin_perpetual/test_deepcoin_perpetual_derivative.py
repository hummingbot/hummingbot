...
class DeepcoinPerpetualDerivativeTests(AbstractPerpetualDerivativeTests.PerpetualDerivativeTests):
    ...
    def configure_erroneous_http_fill_trade_response(
        ...
        url = web_utils.get_rest_url_for_endpoint(endpoint=CONSTANTS.ACCOUNT_TRADE_LIST_URL, domain=CONSTANTS.DEFAULT_DOMAIN)
        regex_url = re.compile(url + r"\?.*")
    ...
    def test_time_synchronizer_related_request_error_detection(self):
        exception = IOError("Error executing request POST https://api.deepcoin.com/deepcoin/trade/order. HTTP status is 401. "
