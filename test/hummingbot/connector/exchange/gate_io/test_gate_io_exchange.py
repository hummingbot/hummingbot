...
class TestGateIoExchange(IsolatedAsyncioWrapperTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.exchange = GateIoExchange(
            gate_io_api_key=self.api_key, gate_io_secret_key=self.api_secret, trading_pairs=[self.trading_pair]
        )
    ...
    def test_update_trading_rules_ignores_rule_with_error(self, mock_api):
        ...
        self.assertEqual(0, len(self.exchange.trading_rules))
        self.assertTrue(self._is_logged("ERROR", f"Error parsing the trading pair rule {resp[0]}. Skipping."))
    ...
    def test_execute_cancel(self, mock_api):
        ...
        self.assertTrue(self._is_logged("INFO", f"Successfully canceled order {client_order_id}."))
    ...
    def test_update_order_status_when_filled(self, mock_api):
        ...
        mock_api.get(regex_order_trade_updates_url, body=json.dumps(order_trade_updates_resp))
        ...
        order_status_resp = self.get_order_create_response_mock(
            cancelled=False, exchange_order_id=order.exchange_order_id
        )
    ...
    def test_update_order_status_processes_trade_fill(self, mock_api):
        ...
        self.assertEqual(
            [TokenAmount(order_trade_updates_resp[0]["fee_currency"], Decimal(order_trade_updates_resp[0]["fee"]))],
    ...
    def test_user_stream_update_for_order_partial_fill(self):
        ...
        self.assertEqual(0.0, fill_event.trade_fee.percent)
        self.assertEqual(
            [TokenAmount(event_message["result"][0]["fee_currency"], Decimal(event_message["result"][0]["fee"]))],
            fill_event.trade_fee.flat_fees,
        )
    ...
    def test_user_stream_update_for_order_fill(self):
        ...
        self.assertEqual(
            [
                TokenAmount(
                    filled_event_message["result"][0]["fee_currency"], Decimal(filled_event_message["result"][0]["fee"])
                )
            ],
            fill_event.trade_fee.flat_fees,
        )
        ...
        self.assertEqual(
            order.amount * Decimal(filled_event_message["result"][0]["price"]), buy_event.quote_asset_amount
        )
        self.assertEqual(order.order_type, buy_event.order_type)
        self.assertEqual(order.exchange_order_id, buy_event.exchange_order_id)
        self.assertNotIn(order.client_order_id, self.exchange.in_flight_orders)
