...
class OpenwareOrderBookMessageTests(TestCase):
    ...
    def test_equality_based_on_type_and_timestamp(self):
        message = OpenwareOrderBookMessage(message_type=OrderBookMessageType.SNAPSHOT, content={}, timestamp=10000000)
        equal_message = OpenwareOrderBookMessage(message_type=OrderBookMessageType.SNAPSHOT, content={}, timestamp=10000000)
        message_with_different_type = OpenwareOrderBookMessage(message_type=OrderBookMessageType.DIFF, content={}, timestamp=10000000)
        message_with_different_timestamp = OpenwareOrderBookMessage(message_type=OrderBookMessageType.SNAPSHOT, content={}, timestamp=90000000)
    ...
    def test_equal_messages_have_equal_hash(self):
        ...
        message = OpenwareOrderBookMessage(message_type=OrderBookMessageType.SNAPSHOT, content=content, timestamp=10000000)
        equal_message = OpenwareOrderBookMessage(message_type=OrderBookMessageType.SNAPSHOT, content=content, timestamp=10000000)

        self.assertEqual(hash(message), hash(equal_message))

    def test_init_error(self):
        with self.assertRaises(ValueError) as context:
            _ = OpenwareOrderBookMessage(OrderBookMessageType.SNAPSHOT, {})
        self.assertEqual('timestamp must not be None when initializing snapshot messages.', str(context.exception))

    def test_instance_creation(self):
        content = self._snapshot_example()
        message = OpenwareOrderBookMessage(message_type=OrderBookMessageType.SNAPSHOT, content=content, timestamp=content["timestamp"])
        bids = message.bids
