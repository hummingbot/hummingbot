...
from hummingbot.connector.exchange.btc_markets import btc_markets_constants as CONSTANTS, btc_markets_utils
...
class BtcMarketsAPIOrderBookDataSourceUnitTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.ex_trading_pair = btc_markets_utils.convert_to_exchange_trading_pair(cls.trading_pair)
    ...
    def setUp(self) -> None:
        ...
        self.data_source = BtcMarketsAPIOrderBookDataSource(trading_pairs=[self.trading_pair])
    ...
    def test_get_last_trade_prices(self, mock_api):
        url = btc_markets_utils.get_rest_url(path_url=CONSTANTS.MARKETS_URL)
    ...
    def test_create_websocket_connection_logs_exception(self, ws_connect_mock):
        ...
        self.assertTrue(self._is_logged(
            "NETWORK", "Unexpected error occured connecting to btc_markets WebSocket API. (TEST ERROR)"
        ))
    ...
    @patch("aiohttp.ClientSession.ws_connect", new_callable=AsyncMock)
    @patch("hummingbot.connector.exchange.btc_markets.btc_markets_websocket.BtcMarketsWebsocket._sleep")
    ...
    def test_listen_for_subscriptions_enqueues_diff_and_trade_messages(self, _, ws_connect_mock):
        diffs_queue = self.data_source._message_queue[BtcMarketsWebsocket.DIFF_CHANNEL_ID]
        trade_queue = self.data_source._message_queue[BtcMarketsWebsocket.TRADE_CHANNEL_ID]
    ...
    @patch("hummingbot.connector.exchange.btc_markets.btc_markets_websocket.BtcMarketsWebsocket._sleep")
    @patch(
        "hummingbot.connector.exchange.btc_markets.btc_markets_api_order_book_data_source.BtcMarketsAPIOrderBookDataSource._sleep"
    )
    ...
    def test_listen_for_trades_successful(self, _, ws_connect_mock):
        ...
        trades_queue = self.data_source._message_queue[BtcMarketsWebsocket.TRADE_CHANNEL_ID]
        self.listening_task = self.ev_loop.create_task(self.data_source.listen_for_subscriptions())
