...
from hummingbot.connector.connector_base import OrderFilledEvent, OrderType, TradeType
...
from hummingbot.core.event.events import BuyOrderCompletedEvent, MarketOrderFailureEvent
...
class FtxExchangeTests(AbstractExchangeConnectorTests.ExchangeConnectorTests):
    ...
    def test_update_order_status_when_order_has_not_changed_and_one_partial_fill(self, mock_api):
        """
        Important: This method is duplicated because FTX Spot and FTX Perpetuals share the same endpoints
        to update order status, trades, place orders, etc. That's why when the method _all_executed_requests
        is called from perpetuals connector, the request is stored in the same instance of the aioresponses
        mock leading to a failure on this test. By implementing the test here, we know for sure that this
        instance of the mock will be only used by this test
        """
