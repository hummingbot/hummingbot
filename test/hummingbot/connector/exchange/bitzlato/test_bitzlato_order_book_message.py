...
class BitzlatoOrderBookMessageTests(TestCase):
    ...
    def test_equality_based_on_type_and_timestamp(self):
        message = BitzlatoOrderBookMessage(message_type=OrderBookMessageType.SNAPSHOT,
                                           content={},
                                           timestamp=10000000)
        equal_message = BitzlatoOrderBookMessage(message_type=OrderBookMessageType.SNAPSHOT,
                                                 content={},
                                                 timestamp=10000000)
        message_with_different_type = BitzlatoOrderBookMessage(message_type=OrderBookMessageType.DIFF,
                                                               content={},
                                                               timestamp=10000000)
        message_with_different_timestamp = BitzlatoOrderBookMessage(message_type=OrderBookMessageType.SNAPSHOT,
                                                                    content={},
                                                                    timestamp=90000000)
    ...
    def test_init_error(self):
        with self.assertRaises(ValueError) as context:
            _ = BitzlatoOrderBookMessage(OrderBookMessageType.SNAPSHOT, {})
