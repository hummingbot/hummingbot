...
class NdaxOrderBookMessageTests(TestCase):
    def test_equality_based_on_type_and_timestamp(self):
        message = NdaxOrderBookMessage(
            message_type=OrderBookMessageType.SNAPSHOT, content={"data": []}, timestamp=10000000
        )
        equal_message = NdaxOrderBookMessage(
            message_type=OrderBookMessageType.SNAPSHOT, content={"data": []}, timestamp=10000000
        )
        message_with_different_type = NdaxOrderBookMessage(
            message_type=OrderBookMessageType.DIFF, content={"data": []}, timestamp=10000000
        )
        message_with_different_timestamp = NdaxOrderBookMessage(
            message_type=OrderBookMessageType.SNAPSHOT, content={"data": []}, timestamp=90000000
        )
    ...
    def test_delete_buy_order_book_entry_always_has_zero_amount(self):
        ...
        message = NdaxOrderBookMessage(message_type=OrderBookMessageType.DIFF, content=content, timestamp=time.time())
        bids = message.bids
