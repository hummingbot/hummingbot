...
class TestXRPLAMMFunctions(IsolatedAsyncioWrapperTestCase):
    ...
    def setUp(self) -> None:
        ...
        # Mock _query_xrpl (new architecture) - returns Response objects
        self.connector._query_xrpl = AsyncMock()
    ...
    async def test_amm_get_pool_info(self, mock_convert_string_to_hex):
        # Setup mock response for _query_xrpl
        resp = Response(
        ...
        self.connector._query_xrpl.return_value = resp
        ...
        # Verify the method called _query_xrpl
        self.connector._query_xrpl.assert_called_once()
        ...
        self.connector._query_xrpl.reset_mock()
    ...
    async def test_amm_add_liquidity(self, mock_xrp_to_drops, mock_convert_string_to_hex):
        ...
        # Mock the quote add liquidity response
        mock_quote = QuoteLiquidityResponse(
            base_limited=True,
            base_token_amount=Decimal("10"),
            quote_token_amount=Decimal("20"),
            base_token_amount_max=Decimal("10.1"),
            quote_token_amount_max=Decimal("20.2"),
        )
        self.connector.amm_quote_add_liquidity = AsyncMock(return_value=mock_quote)

        # Mock transaction submission response - NEW FORMAT (dict, not Response)
        # The _submit_transaction now returns a dict with 'response' key
        submit_result_response = Response(
            status=ResponseStatus.SUCCESS,
            result={
        ...
        # _submit_transaction now returns a dict with 'response' key
        submit_result = {
            "signed_tx": MagicMock(),
        ...
        self.connector._submit_transaction = AsyncMock(return_value=submit_result)
    ...
    async def test_amm_remove_liquidity(self, mock_convert_string_to_hex):
        ...
        # Mock account objects (LP tokens) response - for _query_xrpl
        ...
        self.connector._query_xrpl = AsyncMock(return_value=account_objects_response)

        # Mock transaction submission response with metadata
        submit_result_response = Response(
            status=ResponseStatus.SUCCESS,
            result={
                "engine_result": "tesSUCCESS",
    ...
    async def test_amm_get_balance(self):
        ...
        # Mock account lines response with LP tokens - for _query_xrpl
        resp = Response(
            status=ResponseStatus.SUCCESS,
            result={
                "lines": [
        ...
        self.connector._query_xrpl.reset_mock()
        resp_no_lines = Response(
            status=ResponseStatus.SUCCESS,
            result={"lines": []},
        )
        self.connector._query_xrpl.return_value = resp_no_lines
