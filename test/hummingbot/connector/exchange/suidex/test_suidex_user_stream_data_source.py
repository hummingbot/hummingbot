...
class SUIdexUserStreamDataSourceUnitTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.auth = SUIdexAuth(api_key="TEST_API_KEY", secret_key="TEST_SECRET", time_provider=self.mock_time_provider)
        ...
        self.data_source = SUIdexAPIUserStreamDataSource(
            auth=self.auth,
    ...
    def test_get_listen_key_log_exception(self, mock_api):
        url = web_utils.private_rest_url(path_url=CONSTANTS.SUIDEX_USER_STREAM_PATH_URL, domain=self.domain)
    ...
    @patch("hummingbot.connector.exchange.suidex.suidex_api_user_stream_data_source.SUIdexAPIUserStreamDataSource"
           "._ping_listen_key",
