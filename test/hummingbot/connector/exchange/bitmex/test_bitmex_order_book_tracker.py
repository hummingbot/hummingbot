...
class BitmexOrderBookTrackerUnitTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        super().setUp()
        self.tracker: BitmexOrderBookTracker = BitmexOrderBookTracker(trading_pairs=[self.trading_pair])
        self.tracking_task: Optional[asyncio.Task] = None

        # Simulate start()
        self.tracker._order_books[self.trading_pair] = BitmexOrderBook()
    ...
    def test_exchange_name(self):
        self.assertEqual("bitmex", self.tracker.exchange_name)
    ...
    def test_track_single_book_snapshot_message_with_past_diffs(self):
        past_diff_msg: OrderBookMessage = BitmexOrderBook.diff_message_from_exchange(
            msg={
                "lastUpdateId": 1,
                "data_dict": {
    ...
    def test_track_single_book_diff_message(self):
        diff_msg: OrderBookMessage = BitmexOrderBook.diff_message_from_exchange(
            msg={
                "lastUpdateId": 1,
                "data_dict": {
                    "symbol": "COINALPHA-USD",
