...
class LambdaplexAPIOrderBookDataSourceUnitTests(IsolatedAsyncioWrapperTestCase):
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.connector = LambdaplexExchange(
            lambdaplex_api_key="",
            lambdaplex_private_key="",
            trading_pairs=[],
            trading_required=False,
        )
        self.data_source = LambdaplexAPIOrderBookDataSource(
    ...
    async def test_listen_for_subscriptions_subscribes_to_trades_and_order_diffs(self, ws_connect_mock):
        ...
        expected_trade_subscription = {
            "method": "subscribe",
            "params": [f"{self.ex_trading_pair}@trade"],
            "id": 1,
        }
    ...
    async def test_listen_for_trades_cancelled_when_listening(self):
        ...
        with self.assertRaises(asyncio.CancelledError):
            await asyncio.wait_for(self.data_source.listen_for_trades(self.local_event_loop, msg_queue), timeout=1)
    ...
    async def test_listen_for_order_book_diffs_cancelled(self):
        ...
        with self.assertRaises(asyncio.CancelledError):
            await asyncio.wait_for(
                self.data_source.listen_for_order_book_diffs(self.local_event_loop, msg_queue), timeout=1
            )
    ...
    @aioresponses()
    @patch(
        "hummingbot.connector.exchange.lambdaplex.lambdaplex_api_order_book_data_source"
        ".LambdaplexAPIOrderBookDataSource._sleep"
    )
