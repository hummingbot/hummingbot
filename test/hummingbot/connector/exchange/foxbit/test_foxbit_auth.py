...
class FoxbitAuthTests(TestCase):
    ...
    def test_rest_authenticate(self):
        ...
        auth = FoxbitAuth(
            api_key=self._api_key, secret_key=self._secret, user_id=self._user_id, time_provider=mock_time_provider
        )
        url = web_utils.private_rest_url(CONSTANTS.ORDER_PATH_URL)
        endpoint_url = web_utils.rest_endpoint_url(url)
        request = RESTRequest(
            url=url, endpoint_url=endpoint_url, method=RESTMethod.GET, data=params, is_auth_required=True
        )
        configured_request = self.async_run_with_timeout(auth.rest_authenticate(request))

        timestamp = configured_request.headers["X-FB-ACCESS-TIMESTAMP"]
        payload = "{}{}{}{}".format(timestamp, request.method, request.endpoint_url, params)
        expected_signature = (
            hmac.new(self._secret.encode("utf8"), payload.encode("utf8"), hashlib.sha256).digest().hex()
        )
