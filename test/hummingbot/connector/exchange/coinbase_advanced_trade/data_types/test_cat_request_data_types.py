...
from hummingbot.connector.exchange.coinbase_advanced_trade.cat_data_types.cat_api_request_data_types import (
    ...
    try:
        ...
        for header_element, section_element in zip(header_elements, section_elements):
            ...
            for element in parameter_elements:
                ...
                is_required = await page.evaluate("(element) => element.textContent", required_element) if required_element else None
            ...
            parameter_elements = await section_element.querySelectorAll(".listItem_mkJa")  # Select the list items instead
            ...
            for element in parameter_elements:
                ...
                name = await page.evaluate("(element) => element.textContent", name_element)
                parameter_type = await page.evaluate("(element) => element.textContent", type_element)
                is_required = await page.evaluate("(element) => element.textContent", required_element)

                # Append parameter details to the list along with the associated header value
                parameters.append({
                    "name": name.strip(),
                    "type": parameter_type.strip(),
                    "required": is_required.strip() == "required",
                    "header": header_value.strip()
                })
...
class TestDocumentation(unittest.TestCase):
    def verify_classes(self, web_definition: Dict[str, Any], cls: Type) -> None:
        ...
        class_params = set(f.name for f in fields(cls) if not f.metadata.get("path_param"))
        ...
        class_params = set(f.name for f in fields(cls) if f.metadata.get("path_param"))
        web_params_set = {name for name, param in web_params.items()}
    ...
    def test_list_accounts(self):
        ...
        web_params = asyncio.get_event_loop().run_until_complete(get_page_content(doc_url))
