...
class CoinbaseAdvancedTradeExchangeTests(AbstractExchangeConnectorTests.ExchangeConnectorTests):
    ...
    def all_symbols_url(self):
        return web_utils.public_rest_url(path_url=CONSTANTS.CURRENCIES_EP, domain=CONSTANTS.DEFAULT_DOMAIN)

    @property
    def latest_prices_url(self):
        url = web_utils.public_rest_url(path_url=CONSTANTS.EXCHANGE_RATES_QUOTE_EP, domain=CONSTANTS.DEFAULT_DOMAIN)
        url = f"{url}?currency={self.exchange_symbol_for_tokens(self.base_asset, self.quote_asset).split('-')[1]}"
        return url
    ...
    def trading_rules_url(self):
        url = web_utils.private_rest_url(CONSTANTS.EXCHANGE_INFO_PATH_URL, domain=CONSTANTS.DEFAULT_DOMAIN)
        return url
    ...
    def balance_url(self):
        url = web_utils.private_rest_url(CONSTANTS.ACCOUNT_EP, domain=CONSTANTS.DEFAULT_DOMAIN)
        return url

    @property
    def all_symbols_request_mock_response(self):
        return {"data": {"currency": f"{self.quote_asset}", "rates": {"AED": "98964.65564239121587667827000592460626",
                                                                      "AFN": "2367918.7697619204164525033290542128136",
                                                                      "ALL": "2739799.83888158920485664121091600080001",
                                                                      f"{self.base_asset}": "10431168.12746029738983156279627292702557"
                                                                      }
                         }
                }
