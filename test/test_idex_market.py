...
class IDEXMarketUnitTest(unittest.TestCase):
    ...
    def test_market_buy(self):
        ...
        # buy_amount: float = 4411
        # buy_price = 0.00000001
        # buy_order_id: str = self.market.buy(symbol, buy_amount, OrderType.MARKET, buy_price)
        # self.run_parallel(asyncio.sleep(30))
        buy_amount: float = 4999
        buy_price = 0.00000001
        # buy_order_id: str = self.market.buy(symbol, buy_amount, OrderType.MARKET, buy_price)
        self.run_parallel(asyncio.sleep(30))
    ...
    def test_market_sell(self):
        ...
        # buy_order_id: str = self.market.buy(symbol, buy_amount, OrderType.MARKET, buy_price)
        # self.run_parallel(asyncio.sleep(30))
        buy_amount: float = 4020
        buy_price = 0.00000001
        # buy_order_id: str = self.market.sell(symbol, buy_amount, OrderType.MARKET, buy_price)
        self.run_parallel(asyncio.sleep(30))

    # def test_place_limit_buy_and_sell(self):
    #     # # Try to buy 2000 HOT from the exchange, and watch for completion event.
    #     symbol = TEST_SYMBOL_A
    #     # self.run_parallel(asyncio.sleep(10))

    #     # a = self.market.active_order_trackers[symbol].get_best_limit_orders(True, 1000)
    #     # print("&&&&&&&& A", [b for b in a])
    #     # # bid_price: float = self.market.get_price(symbol, True)
    #     buy_amount: float = 16000000
    #     buy_price = 0.00000001
    #     buy_order_id: str = self.market.buy(symbol, buy_amount, OrderType.LIMIT, buy_price)
    #     # sell_amount: float = 99
    #     # sell_price: float = 1
    #     # # sell_order_id: str = self.market.sell(symbol, sell_amount, OrderType.LIMIT, sell_price)
    #     self.run_parallel(asyncio.sleep(20))
    #     cancel = self.market.cancel(symbol, buy_order_id)
    #     self.run_parallel(asyncio.sleep(1000))
