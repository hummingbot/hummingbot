...
class DDEXMarketUnitTest(unittest.TestCase):
    ...
    def test_calculate_fees(self):
        weth_trade_fees: List[TradeFee] = self.market.calculate_fees("ZRX-WETH", 10000, 1, OrderType.LIMIT, TradeType.BUY)
        self.assertGreater(weth_trade_fees[0].amount, 0)
        dai_trade_fees: List[TradeFee] = self.market.calculate_fees("WETH-DAI", 10000, 1, OrderType.MARKET, TradeType.BUY)
        self.assertGreater(dai_trade_fees[0].amount, 0)
