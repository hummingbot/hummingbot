...
class HoubiOrderBookLoaderUnitTest(unittest.TestCase):
    start_time: float = pd.Timestamp("2018-12-14", tz="UTC").timestamp()
    end_time: float = pd.Timestamp("2018-12-15", tz="UTC").timestamp()
    snapshot_time: float = pd.Timestamp("2018-12-14 01:00:00.444000", tz="UTC").timestamp()

    def setUp(self):
        self.clock: Clock = Clock(ClockMode.BACKTEST, start_time=self.start_time, end_time=self.end_time)
        self.order_book_loader: HuobiOrderBookLoader = HuobiOrderBookLoader("btcusdt", "btc", "usdt")
    ...
    def test_order_book_snapshots(self):
        ...
        self.assertLess(total - matching, 600)
    ...
    def test_order_book_diffs(self):
        ...
        def _test_order_book_diffs(start: int, end: int):
            ...
            or the compare snapshot shows that it is removed.
    ...
    def test_order_book_trades(self):
        ...
        end: int = int(self.snapshot_time) + 300
        self.clock.backtest_til(start)
