...
class BinanceMarketUnitTest(unittest.TestCase):
    ...
    def test_calculate_fees(self):
        [maker_buy_trade_fee] = self.market.calculate_fees("BTCUSDT", 1, 4000, OrderType.LIMIT, TradeType.BUY)
        self.assertGreater(maker_buy_trade_fee.amount, 0)
        self.assertEqual(maker_buy_trade_fee.type, FeeType.SUB_BASE)
        [taker_buy_trade_fee] = self.market.calculate_fees("BTCUSDT", 1, 4000, OrderType.MARKET, TradeType.BUY)
        self.assertGreater(taker_buy_trade_fee.amount, 0)
        self.assertEqual(taker_buy_trade_fee.type, FeeType.SUB_BASE)
        [sell_trade_fee] = self.market.calculate_fees("BTCUSDT", 1, 4000, OrderType.LIMIT, TradeType.SELL)
        self.assertGreater(sell_trade_fee.amount, 0)
