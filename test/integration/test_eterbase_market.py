...
class EterbaseMarketUnitTest(unittest.TestCase):
    ...
    def test_get_fee(self):
        limit_fee: TradeFee = self.market.get_fee("ETH", "EUR", OrderType.LIMIT, TradeType.BUY, 1, 1)
        self.assertGreater(limit_fee.percent, 0)
        self.assertEqual(len(limit_fee.flat_fees), 0)
        market_fee: TradeFee = self.market.get_fee("ETH", "EUR", OrderType.MARKET, TradeType.BUY, 1)
    ...
    def test_market_buy(self):
        ...
        order_id = self.market.buy(trading_pair, quantized_amount, OrderType.MARKET, quantize_price)
        [order_completed_event] = self.run_parallel(self.market_logger.wait_for(BuyOrderCompletedEvent))
        order_completed_event: BuyOrderCompletedEvent = order_completed_event
        trade_events: List[OrderFilledEvent] = [t for t in self.market_logger.event_log
                                                if isinstance(t, OrderFilledEvent)]
        ...
        self.assertEqual(order_id, order_completed_event.order_id)
        #not possible to exactly defined, as reuest is in COSTS (amount*price), fullfiled order can be little less then COSTS
        self.assertGreaterEqual(quantized_amount*quantize_price, order_completed_event.quote_asset_amount,)

        sig_dig = abs((quantized_amount*quantize_price).as_tuple().exponent)-1
        str_quant_cost = ("{0:."+str(sig_dig)+"g}").format(quantized_amount*quantize_price)
    ...
    def test_withdraw(self):
        ...
        # Withdraw ZRX from Eterbase to test wallet.
    ...
    def test_order_fill_record(self):
        ...
        try:
            ...
            order_id = self.market.buy(trading_pair, amount, OrderType.MARKET, quantize_price)
