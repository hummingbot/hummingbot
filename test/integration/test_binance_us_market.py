...
from hummingbot.connector.exchange.binance_us.binance_us_exchange import (
    BinanceUsExchange,
    BinanceUsTime,
    binance_us_client_module
)
from hummingbot.connector.exchange.binance_us.binance_us_utils import convert_to_exchange_trading_pair
...
from test.integration.assets.mock_data.fixture_binance_us import FixtureBinanceUs
...
API_KEY = "XXX" if API_MOCK_ENABLED else conf.binance_us_api_key
API_SECRET = "YYY" if API_MOCK_ENABLED else conf.binance_us_api_secret


class BinanceUsExchangeUnitTest(unittest.TestCase):
    ...
    market: BinanceUsExchange
    market_logger: EventLogger
    stack: contextlib.ExitStack
    base_api_url = "api.binance.us"
    ...
    def setUpClass(cls):
        ...
        if API_MOCK_ENABLED:
            ...
            cls.web_app.update_response("get", cls.base_api_url, "/api/v1/depth",
                                        FixtureBinanceUs.LINKETH_SNAP, params={'symbol': 'LINKETH'})
            cls.web_app.update_response("get", cls.base_api_url, "/api/v1/depth",
                                        FixtureBinanceUs.ZRXETH_SNAP, params={'symbol': 'ZRXETH'})
            ws_base_url = "wss://stream.binance.us:9443/ws"
            cls._ws_user_url = f"{ws_base_url}/{FixtureBinanceUs.LISTEN_KEY['listenKey']}"
            ...
            cls._t_nonce_patcher = unittest.mock.patch(
                "hummingbot.connector.exchange.binance_us.binance_us_exchange.get_tracking_nonce")
            cls._t_nonce_mock = cls._t_nonce_patcher.start()
        cls.clock: Clock = Clock(ClockMode.REALTIME)
        cls.market: BinanceUsExchange = BinanceUsExchange(API_KEY, API_SECRET, ["LINK-ETH", "ZRX-ETH"], True)
        print("Initializing Binance.US market... this will take about a minute.")
    ...
    def setUp(self):
        self.db_path: str = realpath(join(__file__, "../binance_us_test.sqlite"))
        try:
    ...
    def test_fee_overrides_config(self):
        fee_overrides_config_map["binance_us_taker_fee"].value = None
        ...
        fee_overrides_config_map["binance_us_taker_fee"].value = Decimal('0.2')
        taker_fee: TradeFee = self.market.get_fee("LINK", "ETH", OrderType.LIMIT, TradeType.BUY, Decimal(1),
                                                  Decimal('0.1'))
        self.assertAlmostEqual(Decimal("0.002"), taker_fee.percent)
        fee_overrides_config_map["binance_us_maker_fee"].value = None
        ...
        fee_overrides_config_map["binance_us_maker_fee"].value = Decimal('0.5')
        maker_fee: TradeFee = self.market.get_fee("LINK", "ETH", OrderType.LIMIT_MAKER, TradeType.BUY, Decimal(1),
                                                  Decimal('0.1'))
    ...
    def test_buy_and_sell(self):
        ...
        order_id = self.place_order(True, "LINK-ETH", amount, OrderType.LIMIT, bid_price, 10001, FixtureBinanceUs.BUY_MARKET_ORDER,
                                    FixtureBinanceUs.WS_AFTER_BUY_1, FixtureBinanceUs.WS_AFTER_BUY_2)
        ...
        order_id = self.place_order(False, "LINK-ETH", amount, OrderType.LIMIT, ask_price, 10002, FixtureBinanceUs.SELL_MARKET_ORDER,
                                    FixtureBinanceUs.WS_AFTER_SELL_1, FixtureBinanceUs.WS_AFTER_SELL_2)
    ...
    def test_limit_maker_rejections(self):
        ...
        order_id = self.place_order(True, "LINK-ETH", amount, OrderType.LIMIT_MAKER,
                                    price, 10001,
                                    FixtureBinanceUs.LIMIT_MAKER_ERROR)
    ...
    def test_limit_makers_unfilled(self):
        ...
        order_id = self.place_order(True, "LINK-ETH", amount, OrderType.LIMIT_MAKER,
                                    price, 10001,
                                    FixtureBinanceUs.OPEN_BUY_ORDER)
        ...
        order_id = self.place_order(False, "LINK-ETH", amount, OrderType.LIMIT_MAKER,
                                    price, 10002,
                                    FixtureBinanceUs.OPEN_SELL_ORDER)
    ...
    def test_cancel_all(self):
        ...
        buy_id = self.place_order(True, "LINK-ETH", quantized_amount, OrderType.LIMIT, quantize_bid_price, 10001,
                                  FixtureBinanceUs.OPEN_BUY_ORDER, FixtureBinanceUs.WS_AFTER_BUY_1,
                                  FixtureBinanceUs.WS_AFTER_BUY_2)

        sell_id = self.place_order(False, "LINK-ETH", quantized_amount, OrderType.LIMIT, quantize_ask_price, 10002,
                                   FixtureBinanceUs.OPEN_SELL_ORDER, FixtureBinanceUs.WS_AFTER_SELL_1,
                                   FixtureBinanceUs.WS_AFTER_SELL_2)

        self.run_parallel(asyncio.sleep(1))
        if API_MOCK_ENABLED:
            resp = self.fixture(FixtureBinanceUs.CANCEL_ORDER, origClientOrderId=buy_id, side="BUY")
            self.web_app.update_response("delete", self.base_api_url, "/api/v3/order", resp,
                                         params={'origClientOrderId': buy_id})
            resp = self.fixture(FixtureBinanceUs.CANCEL_ORDER, origClientOrderId=sell_id, side="SELL")
            self.web_app.update_response("delete", self.base_api_url, "/api/v3/order", resp,
    ...
    def test_order_price_precision(self):
        ...
        binance_us_client = self.market.binance_us_client
        ...
        if API_MOCK_ENABLED:
            resp = self.order_response(FixtureBinanceUs.ORDER_BUY_PRECISION, 1000001, "buy", "LINK-ETH")
            self.web_app.update_response("post", self.base_api_url, "/api/v3/order", resp)
        # Test bid order
        ...
        if API_MOCK_ENABLED:
            resp = FixtureBinanceUs.ORDER_BUY_PRECISION_GET
        ...
        order_data: Dict[str, any] = binance_us_client.get_order(
            symbol=trading_pair,
        ...
        # Test ask order
        if API_MOCK_ENABLED:
            resp = self.order_response(FixtureBinanceUs.ORDER_SELL_PRECISION, 1000002, "sell", "LINK-ETH")
            self.web_app.update_response("post", self.base_api_url, "/api/v3/order", resp)
        ...
        if API_MOCK_ENABLED:
            resp = FixtureBinanceUs.ORDER_SELL_PRECISION_GET
        ...
        order_data = binance_us_client.get_order(
            symbol=trading_pair,
        ...
        if API_MOCK_ENABLED:
            resp = self.fixture(FixtureBinanceUs.CANCEL_ORDER, origClientOrderId=bid_order_id, side="BUY")
            self.web_app.update_response("delete", self.base_api_url, "/api/v3/order", resp,
                                         params={'origClientOrderId': bid_order_id})
            resp = self.fixture(FixtureBinanceUs.CANCEL_ORDER, origClientOrderId=ask_order_id, side="SELL")
            self.web_app.update_response("delete", self.base_api_url, "/api/v3/order", resp,
    ...
    def test_server_time_offset(self):
        time_obj: BinanceUsTime = binance_us_client_module.time
        ...
        try:
            ...
            with patch("hummingbot.connector.exchange.binance_us.binance_us_time.time") as market_time:
                def delayed_time():
                    return time.perf_counter() - 30.0
                market_time.perf_counter = delayed_time
                self.run_parallel(asyncio.sleep(3.0))
                raw_time_offset = BinanceUsTime.get_instance().time_offset_ms
    ...
    def test_orders_saving_and_restoration(self):
        ...
        try:
            ...
            if API_MOCK_ENABLED:
                resp = self.order_response(FixtureBinanceUs.OPEN_BUY_ORDER, 1000001, "buy", "LINK-ETH")
                self.web_app.update_response("post", self.base_api_url, "/api/v3/order", resp)
            ...
            for event_tag in self.events:
                self.market.remove_listener(event_tag, self.market_logger)
            self.market: BinanceUsExchange = BinanceUsExchange(API_KEY, API_SECRET, ["LINK-ETH", "ZRX-ETH"], True)
            for event_tag in self.events:
                self.market.add_listener(event_tag, self.market_logger)
            recorder.stop()
            ...
            if API_MOCK_ENABLED:
                resp = self.fixture(FixtureBinanceUs.CANCEL_ORDER, origClientOrderId=order_id, side="BUY")
                self.web_app.update_response("delete", self.base_api_url, "/api/v3/order", resp,
    ...
    def test_order_fill_record(self):
        ...
        try:
            ...
            order_id = self.place_order(True, "LINK-ETH", amount, OrderType.LIMIT, bid_price, 10001,
                                        FixtureBinanceUs.BUY_LIMIT_ORDER, FixtureBinanceUs.WS_AFTER_BUY_1,
                                        FixtureBinanceUs.WS_AFTER_BUY_2)
            ...
            order_id = self.place_order(False, "LINK-ETH", amount, OrderType.LIMIT, ask_price, 10002,
                                        FixtureBinanceUs.SELL_LIMIT_ORDER, FixtureBinanceUs.WS_AFTER_SELL_1,
                                        FixtureBinanceUs.WS_AFTER_SELL_2)
