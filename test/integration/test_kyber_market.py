...
class KyberMarketUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.wallet = Web3Wallet(private_key=conf.kyber_test_wallet_private_key,
                                backend_urls=conf.test_web3_provider_list,
                                erc20_token_addresses=[conf.kyber_test_erc20_token_address_1],
                                chain=EthereumChain.ROPSTEN)
        cls.market: KyberMarket = KyberMarket(
            wallet=cls.wallet,
            ethereum_rpc_url=conf.test_web3_provider_list[0],
            order_book_tracker_data_source_type=OrderBookTrackerDataSourceType.EXCHANGE_API,
            trading_pairs=[ETH_KNC]
        )
        print("Initializing Kyber market... ")
    ...
    def setUp(self):
        self.db_path: str = realpath(join(__file__, "../kyber_test.sqlite"))
        try:
    ...
    def test_quantize_order_amount(self):
        amount = self.market.quantize_order_amount("ETH_KNC", Decimal(1E-16))
        self.assertEqual(0, amount)
        amount = self.market.quantize_order_amount("ETH_KNC", Decimal(1))
        self.assertEqual(1, amount)

    def test_cancel_all_failed_cancel(self):
        trading_pair = ETH_KNC
        ...
        self.assertEqual(ETH_KNC, buy_order_created_event.trading_pair)
        ...
        self.assertEqual(ETH_KNC, sell_order_created_event.trading_pair)
    ...
    def test_orders_saving_and_restoration(self):
        ...
        try:
            ...
            for event_tag in self.market_events:
                self.market.remove_listener(event_tag, self.market_logger)
            self.market: KyberMarket = KyberMarket(
                wallet=self.wallet,
                ethereum_rpc_url=conf.test_web3_provider_list[0],
                order_book_tracker_data_source_type=OrderBookTrackerDataSourceType.EXCHANGE_API,
                trading_pairs=[ETH_KNC]
