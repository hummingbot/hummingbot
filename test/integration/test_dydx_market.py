...
from test.integration.assets.mock_data.fixture_dydx import FixtureDydx
# from hummingbot.connector.exchange.dydx.dydx_auth import DydxAuth
...
PRIVATE_KEY = "a3f18ad5927fa7cf218f1afa8eaeef8f794ad4f324f4a030f7469aa297fb8ea9" if API_MOCK_ENABLED else conf.dydx_private_key
NODE_ADDRESS = "https://mainnet.infura.io/v3/ba0cd0f4191245f18d04c934b220d227" if API_MOCK_ENABLED else conf.dydx_node_address
if API_MOCK_ENABLED:
    WALLET_ADDRESS = "0x7E1431664d05212774704Cff9d7949DfBe2d5d25"
    ACCOUNT_NUMBER = "78249916358380492593314239409032173911741268194868200833150293576330928686520"
...
class DydxExchangeUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if API_MOCK_ENABLED:
            ...
            cls.web_app.update_response("get", cls.base_api_url, f"/v1/accounts/{WALLET_ADDRESS}",
                                       FixtureDydx.BALANCES, params={'number': f'{ACCOUNT_NUMBER}'})

            cls.web_app.update_response("get", cls.base_api_url, "/v2/markets",
                                        FixtureDydx.MARKETS)
            cls.web_app.update_response("get", cls.base_api_url, "/v1/orderbook/WETH-USDC",
                                        FixtureDydx.WETHUSDC_SNAP)
            cls._buy_order_exchange_id = "0xb0751a113c759779ff5fd6a53b37b26211a9f8845d443323b9f877f32d9aafd9"
            cls._sell_order_exchange_id = "0x03dfd18edc2f26fc9298edcd28ca6cad4971bd1f44d40253d5154b0d1f217680"
            cls.web_app.update_response("delete", cls.base_api_url, f"/v2/orders/{cls._buy_order_exchange_id}",
                                        FixtureDydx.CANCEL_ORDER_BUY)
            cls.web_app.update_response("delete", cls.base_api_url, f"/v2/orders/{cls._sell_order_exchange_id}",
            ...
            cls._t_nonce_patcher = unittest.mock.patch(
                "hummingbot.connector.exchange.dydx.dydx_exchange.get_tracking_nonce")
    ...
    def place_order(self, is_buy, trading_pair, amount, order_type, price, nonce, fixture_resp,
                    fixture_ws_1 = None, fixture_ws_2 = None, fixture_ws_3 = None):
        order_id = None
        if API_MOCK_ENABLED:
            resp = self.order_response(fixture_resp, nonce, 'buy' if is_buy else 'sell', trading_pair)
            self.web_app.update_response("post", self.base_api_url, "/v2/orders", resp)
        if is_buy:
        ...
        if API_MOCK_ENABLED and fixture_ws_1 is not None:
            self.web_app.update_response("get", self.base_api_url, "/v2/fills",
                                        FixtureDydx.FILLS, params={"orderId": order_id, "limit": 100})
            ...
            if fixture_ws_3 is not None:
                HummingWsServerFactory.send_json_threadsafe(self._ws_user_url, fixture_ws_3, delay=0.1)
        return order_id

    def test_get_fee(self):
        limit_trade_fee: TradeFee = self.market.get_fee("WETH", "USDC", OrderType.LIMIT_MAKER, TradeType.SELL, 10000, 1)
    ...
    def test_limit_buy(self):
        ...
        buy_order_id: str = self.place_order(True, "WETH-USDC", amount, OrderType.LIMIT, ask_price * Decimal('1.5'), 10001, FixtureDydx.BUY_LIMIT_ORDER,
                                    FixtureDydx.WS_AFTER_BUY_1, FixtureDydx.WS_AFTER_BUY_2, FixtureDydx.WS_AFTER_BUY_3)
    ...
    def test_limit_sell(self):
        ...
        sell_order_id: str = self.place_order(False, "WETH-USDC", amount, OrderType.LIMIT, bid_price * Decimal('0.5'), 10001, FixtureDydx.SELL_LIMIT_ORDER,
                                    FixtureDydx.WS_AFTER_SELL_1, FixtureDydx.WS_AFTER_SELL_2, FixtureDydx.WS_AFTER_SELL_3)
    ...
    def test_limit_maker_rejections(self):
        ...
        amount: Decimal = Decimal("40.0")
        bid_price: Decimal = self.market.get_price(trading_pair, True)
        sell_order_id: str = self.place_order(False, "WETH-USDC", amount, OrderType.LIMIT_MAKER, bid_price * Decimal('0.5'), 10001, FixtureDydx.SELL_LIMIT_MAKER_ORDER,
                                    FixtureDydx.WS_AFTER_SELL_1, FixtureDydx.LIMIT_MAKER_SELL_ERROR)
    ...
    def test_limit_makers_unfilled(self):
        ...
        amount: Decimal = Decimal("40.0")
        bid_price: Decimal = self.market.get_price(trading_pair, True)
        buy_order_id: str = self.place_order(True, "WETH-USDC", amount, OrderType.LIMIT_MAKER, bid_price * Decimal('0.5'), 10001, FixtureDydx.BUY_LIMIT_MAKER_ORDER, #LIMIT_MAKER
                                    FixtureDydx.WS_AFTER_BUY_1)
        self.run_parallel(asyncio.sleep(6.0))
        self.market.cancel(trading_pair, buy_order_id)

        if API_MOCK_ENABLED:
            HummingWsServerFactory.send_json_threadsafe(self._ws_user_url, FixtureDydx.WS_AFTER_CANCEL_BUY, delay=0.1)
        [order_cancelled_event] = self.run_parallel(self.market_logger.wait_for(OrderCancelledEvent))
    ...
    def test_cancel_order(self):
        ...
        client_order_id = self.place_order(True, "WETH-USDC", amount, OrderType.LIMIT_MAKER, bid_price * Decimal('0.5'), 10001, FixtureDydx.BUY_LIMIT_ORDER,
                                    FixtureDydx.WS_AFTER_BUY_1)
    ...
    def test_cancel_all(self):
        ...
        # Intentionally setting price far away from best ask
        client_order_id = self.place_order(True, "WETH-USDC", amount, OrderType.LIMIT, bid_price * Decimal('0.5'), 10001, FixtureDydx.BUY_LIMIT_ORDER,
                                    FixtureDydx.WS_AFTER_BUY_1)
        self.run_parallel(asyncio.sleep(1.0))
    ...
    def test_limit_orders(self):
        ...
        amount: Decimal = Decimal("40.0")
        bid_price: Decimal = self.market.get_price(trading_pair, True)
        buy_order_id: str = self.place_order(True, "WETH-USDC", amount, OrderType.LIMIT, bid_price * Decimal('0.5'), 10001, FixtureDydx.BUY_LIMIT_ORDER,
                                    FixtureDydx.WS_AFTER_BUY_1)
    ...
    def test_order_saving_and_restoration(self):
        ...
        try:
            ...
            order_id = self.place_order(True, trading_pair, amount, OrderType.LIMIT, quantize_bid_price, 10001, FixtureDydx.BUY_LIMIT_ORDER,
                                    FixtureDydx.WS_AFTER_BUY_1)            
    ...
    def test_order_fill_record(self):
        ...
        try:
            ...
            order_id = self.place_order(True, "WETH-USDC", amount, OrderType.LIMIT, ask_price * Decimal('1.5'), 1000100010001000, FixtureDydx.BUY_LIMIT_ORDER,
                                    FixtureDydx.WS_AFTER_BUY_1, FixtureDydx.WS_AFTER_BUY_2, FixtureDydx.WS_AFTER_BUY_3)
            [buy_order_completed_event] = self.run_parallel(self.market_logger.wait_for(BuyOrderCompletedEvent))
            # Reset the logs
            ...
            order_id = self.place_order(False, "WETH-USDC", amount, OrderType.LIMIT, ask_price, 1000200010001000,
                                        FixtureDydx.SELL_LIMIT_ORDER, FixtureDydx.WS_AFTER_SELL_1,
                                        FixtureDydx.WS_AFTER_SELL_2, FixtureDydx.WS_AFTER_SELL_3)
            [sell_order_completed_event] = self.run_parallel(self.market_logger.wait_for(SellOrderCompletedEvent))
