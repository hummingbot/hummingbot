...
class DuedexOrderBookTrackerUnitTest(unittest.TestCase):
    order_book_tracker: Optional[DuedexOrderBookTracker] = None
    ...
    def setUpClass(cls):
        ...
        cls.order_book_tracker: DuedexOrderBookTracker = DuedexOrderBookTracker(
    ...
    async def wait_til_tracker_ready(cls):
        while True:
            if len(cls.order_book_tracker.order_books) == len(cls.trading_pairs):
    ...
    def test_tracker_integrity(self):
        ...
        self.assertGreaterEqual(btcusd_book.get_price_for_volume(True, 1000).result_price,
                                btcusd_book.get_price(True))
        self.assertLessEqual(btcusd_book.get_price_for_volume(False, 1000).result_price,
    ...
    def test_api_get_last_traded_prices(self):
        prices = self.ev_loop.run_until_complete(
            DuedexAPIOrderBookDataSource.get_last_traded_prices(["BTC-USDT", "ETH-USDT"]))
        for key, value in prices.items():
            print(f"{key} last_trade_price: {value}")
        self.assertGreater(prices["BTC-USDT"], 1000)
        self.assertGreater(prices["ETH-USDT"], 100)
