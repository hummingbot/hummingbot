...
class BinancePerpetualDerivativeUnitTest(unittest.TestCase):
    ...
    start_timestamp: float = pd.Timestamp("2022-10-26", tz="UTC").timestamp()
    ...
    async def test_set_position_mode_initial_mode_is_none(self, mock_api):
        ...
        url = web_utils.rest_url(path_url= CONSTANTS.CHANGE_POSITION_MODE_URL, domain=self.domain, api_version =CONSTANTS.API_VERSION)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        ...
        task = self.ev_loop.create_task(self.exchange.set_position_mode(PositionMode.HEDGE))
        self.async_run_with_timeout(task)
    ...
    async def test_set_position_mode_diff_initial_mode_change_successful(self, mock_api):
        ...
        self.exchange._position_mode = PositionMode.ONEWAY
        url = web_utils.rest_url(path_url= CONSTANTS.CHANGE_POSITION_MODE_URL, domain=self.domain, api_version =CONSTANTS.API_VERSION_V2)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        get_position_mode_response = {"dualSidePosition": False}  # True: Hedge Mode; False: One-way Mode
        post_position_mode_response = {"code": 200, "msg": "success"}
        ...
        task = self.ev_loop.create_task(perpetual_trading.set_position_mode(value=PositionMode.HEDGE))
        self.async_run_with_timeout(task)
    ...
    def test_format_trading_rules(self):
        ...
        mocked_response = self._get_exchange_info_mock_response(
            trading_pair, min_order_size, min_price_increment, min_base_amount_increment, min_notional_size
        )
        ...
        self.assertEqual(trading_pair, trading_rule.buy_order_collateral_token)
        self.assertEqual(trading_pair, trading_rule.sell_order_collateral_token)

    def test_get_collateral_token(self):
        trading_pair = self.trading_pair
        mocked_response = self._get_exchange_info_mock_response(trading_pair)
        trading_rules = self.exchange._format_trading_rules(mocked_response)
        self.exchange._trading_rules[self.trading_pair] = trading_rules[0]

        self.assertEqual(trading_pair, self.exchange.get_buy_collateral_token(self.trading_pair))
        self.assertEqual(trading_pair, self.exchange.get_sell_collateral_token(self.trading_pair))
    ...
    def test_create_order_successful(self, req_mock):
        ...
        self.async_run_with_timeout(
            self.exchange._place_order(trade_type=TradeType.BUY, order_id="OID1", trading_pair=self.trading_pair, amount=Decimal("10000"), order_type=OrderType.LIMIT, position_action=PositionAction.OPEN, price=Decimal("10000"))
        )
    ...
    def test_create_order_exception(self, req_mock):
        ...
        self.async_run_with_timeout(
            self.exchange._place_order(trade_type=TradeType.BUY, order_id="OID1", trading_pair=self.trading_pair, amount=Decimal("10000"), order_type=OrderType.LIMIT, position_action=PositionAction.OPEN, price=Decimal("1010"))
        )
    ...
    def test_create_order_position_action_failure(self):
        ...
        self.exchange._trading_rules[self.trading_pair] = trading_rules[0]
        self.async_run_with_timeout(
            self.exchange._place_order(trade_type=TradeType.BUY, order_id="OID1", trading_pair=self.trading_pair, amount=Decimal("10000"), order_type=OrderType.LIMIT, position_action="BAD POSITION ACTION", price=Decimal("1010"))
        )
    ...
    def test_create_order_supported_order_type_failure(self):
        ...
        self.exchange._trading_rules[self.trading_pair] = trading_rules[0]
        self.async_run_with_timeout(
            self.exchange._place_order(trade_type=TradeType.BUY, order_id="OID1", trading_pair=self.trading_pair, amount=Decimal("10000"), order_type="STOP LIMIT", position_action=PositionAction.OPEN, price=Decimal("1010"))
        )
    ...
    def test_create_order_min_order_size_failure(self):
        trading_pair = self.trading_pair
        min_order_size = 3
        mocked_response = self._get_exchange_info_mock_response(trading_pair, min_order_size=min_order_size)
        trading_rules = self.exchange._format_trading_rules(mocked_response)
        self.exchange._trading_rules[self.trading_pair] = trading_rules[0]
        ...
        self.async_run_with_timeout(
            self.exchange._place_order(trade_type=trade_type, order_id="OID1", trading_pair=self.trading_pair, amount=amount, order_type=OrderType.LIMIT, position_action=PositionAction.OPEN, price=Decimal("1010"))
        )
    ...
    def test_create_order_min_notional_size_failure(self):
        ...
        self.async_run_with_timeout(
            self.exchange._place_order(trade_type=trade_type, order_id="OID1", trading_pair=self.trading_pair, amount=amount, order_type=OrderType.LIMIT, position_action=PositionAction.OPEN, price=price)
        )
