...
class CoinflexOrderBookTrackerUnitTest(unittest.TestCase):
    order_book_tracker: Optional[CoinflexOrderBookTracker] = None
    ...
    def setUpClass(cls):
        ...
        cls.order_book_tracker: CoinflexOrderBookTracker = CoinflexOrderBookTracker(trading_pairs=cls.trading_pairs, domain=os.getenv("COINFLEX_DOMAIN", "live"))
    ...
    def test_tracker_integrity(self):
        ...
        self.assertIsNot(test_order_book.last_diff_uid, 0)
        self.assertGreaterEqual(test_order_book.get_price_for_volume(True, 1).result_price,
                                test_order_book.get_price(True))
        self.assertLessEqual(test_order_book.get_price_for_volume(False, 1).result_price,
                             test_order_book.get_price(False))

    def test_api_get_last_traded_prices(self):
        prices = self.ev_loop.run_until_complete(
            CoinflexAPIOrderBookDataSource.get_last_traded_prices(["BTC-USD", "FLEX-USD"]))
        print("\n")
        for key, value in prices.items():
            print(f"{key} last_trade_price: {value}")
        self.assertGreater(prices["BTC-USD"], 1000)
        self.assertLess(prices["FLEX-USD"], 10)
