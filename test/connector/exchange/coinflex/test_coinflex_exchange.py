...
class CoinflexExchangeUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.connector: CoinflexExchange = CoinflexExchange(
            domain=DOMAIN,
            coinflex_api_key=API_KEY,
            coinflex_api_secret=API_SECRET,
            trading_pairs=[cls.trading_pair],
            trading_required=True
        )
        print("Initializing Coinflex market... this will take about a minute.")
    ...
    async def _async_run_with_timeout(self, coro, seconds):
        async with timeout(seconds):
            return await coro

    def _run_with_timeout(self, coro, seconds=20):
        return self.ev_loop.run_until_complete(self._async_run_with_timeout(coro, seconds))
    ...
    def test_estimate_fee(self):
        ...
        self.assertAlmostEqual(maker_fee, Decimal("0.000"))
        taker_fee = self.connector.estimate_fee_pct(False)
        self.assertAlmostEqual(taker_fee, Decimal("0.0008"))
    ...
    def test_buy_and_sell(self):
        print(self.connector.get_price(self.trading_pair, True))
        price = self.connector.get_mid_price(self.trading_pair) * Decimal("1.05")
        ...
        order_completed_event = self._run_with_timeout(self.event_logger.wait_for(BuyOrderCompletedEvent))
        ...
        expected_quote_bal_self = quote_bal - order_completed_event.fee_amount
        self.ev_loop.run_until_complete(self.connector._update_balances())
        try:
            self.assertAlmostEqual(expected_quote_bal, self.connector.get_available_balance(self.quote_token), 0)
        except Exception:
            self.assertAlmostEqual(expected_quote_bal_self, self.connector.get_available_balance(self.quote_token), 0)
        ...
        order_completed_event = self._run_with_timeout(self.event_logger.wait_for(SellOrderCompletedEvent))
        self.ev_loop.run_until_complete(asyncio.sleep(5))
        trade_events = [t for t in self.event_logger.event_log if isinstance(t, OrderFilledEvent)]
        base_amount_traded = sum(t.amount for t in trade_events)
        quote_amount_traded = sum(t.amount * t.price for t in trade_events)
    ...
    def test_limit_makers_unfilled(self):
        ...
        order_created_event = self._run_with_timeout(self.event_logger.wait_for(BuyOrderCreatedEvent))
    ...
    def test_order_quantized_values(self):
        ...
        self.assertGreater(self.connector.get_balance("USD"), Decimal("40"))
        ...
        self._run_with_timeout(self.event_logger.wait_for(BuyOrderCreatedEvent))
    ...
    def test_filled_orders_recorded(self):
        ...
        try:
            ...
            price = self.connector.get_price(self.trading_pair, True) * Decimal("1.1")
            price = self.connector.quantize_order_price(self.trading_pair, price)
            amount = self.connector.quantize_order_amount(self.trading_pair, Decimal("0.001"))

            order_id = self._place_order(True, amount, OrderType.LIMIT, price, 1)
            self._run_with_timeout(self.event_logger.wait_for(BuyOrderCompletedEvent))
            self.ev_loop.run_until_complete(asyncio.sleep(1))
            ...
            # Try to sell back the same amount to the exchange, and watch for completion event.
            price = self.connector.get_price(self.trading_pair, True) * Decimal("0.9")
            price = self.connector.quantize_order_price(self.trading_pair, price)
            amount = self.connector.quantize_order_amount(self.trading_pair, Decimal("0.001"))
            order_id = self._place_order(False, amount, OrderType.LIMIT, price, 2)
            self._run_with_timeout(self.event_logger.wait_for(SellOrderCompletedEvent))
            self.ev_loop.run_until_complete(asyncio.sleep(1))
    ...
    def test_orders_saving_and_restoration(self):
        ...
        try:
            self.connector._order_tracker._in_flight_orders.clear()
            self.assertEqual(0, len(self.connector.tracking_states))

            # Try to put limit buy order for 0.001 BTC, and watch for order creation event.
            current_bid_price: Decimal = self.connector.get_price(self.trading_pair, True)
            price: Decimal = current_bid_price * Decimal("0.9")
            price = self.connector.quantize_order_price(self.trading_pair, price)
            ...
            with recorder._sql_manager.get_new_session() as session:
                saved_market_states: MarketState = recorder.get_market_states(config_path, self.connector, session=session)
            ...
            self.event_logger.clear()
            new_connector = CoinflexExchange(
                domain=DOMAIN,
                coinflex_api_key=API_KEY,
                coinflex_api_secret=API_SECRET,
            ...
            recorder.start()
            with recorder._sql_manager.get_new_session() as session:
                saved_market_states = recorder.get_market_states(config_path, new_connector, session=session)
            self.clock.add_iterator(new_connector)
            self._run_with_timeout(self.wait_til_ready(new_connector), seconds=60)
            ...
            with recorder._sql_manager.get_new_session() as session:
                ...
                self.assertEqual(0, len(new_connector.limit_orders))
                self.assertEqual(0, len(new_connector.tracking_states))
                new_saved_market_states = recorder.get_market_states(config_path, new_connector, session=session)
            self.assertEqual(0, len(new_saved_market_states.saved_state))
        finally:
            if order_id is not None:
