...
from hummingbot.connector.exchange.k2.k2_exchange import K2Exchange
...
class K2ExchangeUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.connector: K2Exchange = K2Exchange(
            k2_api_key=API_KEY,
    ...
    def test_limit_buy_and_sell(self):
        ...
        order_id = self.connector.buy(trading_pair=self.trading_pair,
                                      amount=amount,
        ...
        price = self.connector.quantize_order_price(self.trading_pair, price)
        amount = self.connector.quantize_order_amount(self.trading_pair, Decimal("0.0001"))
        order_id = self.connector.sell(trading_pair=self.trading_pair,
                                       amount=amount,
                                       order_type=OrderType.LIMIT,
                                       price=price,
                                       )
    ...
    def test_order_price_precision(self):
        ...
        cl_order_id_1 = self.connector.buy(trading_pair=self.trading_pair,
                                           amount=amount,
                                           order_type=OrderType.LIMIT,
                                           price=bid_price,
                                           )
        ...
        # Test ask order
        cl_order_id_2 = self.connector.sell(trading_pair=self.trading_pair,
                                            amount=amount,
                                            order_type=OrderType.LIMIT,
                                            price=bid_price,
                                            )
    ...
    def test_orders_saving_and_restoration(self):
        ...
        try:
            ...
            for event_tag in self.events:
                self.connector.remove_listener(event_tag, self.event_logger)
            new_connector = K2Exchange(API_KEY, API_SECRET, [self.trading_pair], True)
            for event_tag in self.events:
