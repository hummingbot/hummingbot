...
TEST_PRIVATE_KEY = "3771ddf5dd1d38ff72334b9763dc3cbc6fc3196f23e651f391fe65e31e466e3d"  # noqa: documentation
...
class TestOrderDataManager(aiounittest.AsyncTestCase):
    ...
    async def test_initalize_orderbook(self, get_orderbook_mock: AsyncMock, connect_mock: AsyncMock):
        bids = orders([(5, 2), (6, 7)])
        asks = orders([(7, 4), (8, 4)])
        get_orderbook_mock.return_value = api.GetOrderbookResponse(
            market="SOLUSDC",
    ...
    async def test_apply_orderbook_update_with_empty_side(
        ...
        bids = orders([])
        asks = orders([])
        orderbook_mock.return_value = api.GetOrderbookResponse(
            market="SOLUSDC",
            market_address="SOL_USDC_Market",
            bids=bids,
            asks=asks,
    ...
    async def test_apply_order_status_update(
        ...
        def side_effect_function(**kwargs):
            ...
            if market == "SOLUSDC":
                return async_generator_order_status_stream(
                    [("SOL/USDC", 123, 10, api.OrderStatus.OS_FILLED, api.Side.S_ASK, 0.3, 0.2)]
                )
            elif market == "BTCUSDC":
                return async_generator_order_status_stream(
                    [
                        ("BTC-USDC", 456, 50, api.OrderStatus.OS_PARTIAL_FILL, api.Side.S_BID, 0.4, 0.2),
                    ]
                )
        ...
        self.assertEqual([OrderStatusInfo(
            ...
            fill_price=0.0,
            order_price=10,
            order_status=api.OrderStatus.OS_FILLED,
        ...
        self.assertEqual(os2, [OrderStatusInfo(
            ...
            fill_price=0.0,
            order_price=50,
            order_status=api.OrderStatus.OS_PARTIAL_FILL,
    ...
    async def test_apply_order_status_update_edge_cases(
        ...
        def side_effect_function(**kwargs):
            ...
            if market == "SOLUSDC":
                return async_generator_order_status_stream([
                    ("SOL/USDC", 123, 10, api.OrderStatus.OS_OPEN, api.Side.S_ASK, 0, 0.3),
                    ("SOL/USDC", 123, 10, api.OrderStatus.OS_OPEN, api.Side.S_ASK, 0, 0.3),
                    ("SOL/USDC", 123, 10, api.OrderStatus.OS_FILLED, api.Side.S_ASK, 0.3, 0),
                ])
        ...
        self.assertListEqual(os, [
            OrderStatusInfo(
                ...
                fill_price=0.0,
                order_price=10,
                order_status=api.OrderStatus.OS_OPEN,
                quantity_released=0,
    ...
    async def test_apply_order_status_update_on_existing_order(
        ...
        order_status_stream_mock.return_value = async_generator_order_status_stream(
            [
                ("SOL/USDC", 123, 11, api.OrderStatus.OS_PARTIAL_FILL, api.Side.S_ASK, 0.1, 0.1),
                ("SOL/USDC", 123, 10, api.OrderStatus.OS_FILLED, api.Side.S_ASK, 0.1, 0),
            ]
        )
        time_mock.return_value = 1
    ...
    async def test_apply_order_status_update_on_order_in_existing_market(
        ...
        order_status_stream_mock.return_value = async_generator_order_status_stream(
            [
                ("SOL/USDC", 123, 11, api.OrderStatus.OS_PARTIAL_FILL, api.Side.S_ASK, 0.3, 0.2),
                ("SOL/USDC", 456, 10, api.OrderStatus.OS_FILLED, api.Side.S_ASK, 0.4, 0.2),
            ]
        )
        time_mock.return_value = 1
...
def orders(price_and_sizes: List[Tuple[int, int]]) -> List[api.OrderbookItem]:
    orderbook_items = []
    for price, size in price_and_sizes:
        orderbook_items.append(api.OrderbookItem(price=price, size=size))

    return orderbook_items


async def async_generator_orderbook_stream(market, bids, asks) -> AsyncGenerator:
    yield api.GetOrderbooksStreamResponse(slot=1,
                                          orderbook=api.GetOrderbookResponse(market=market, bids=bids, asks=asks))


async def async_generator_order_status_stream(
        order_status_updates: List[Tuple[str, int, int, api.OrderStatus, api.Side, float, float]]
) -> AsyncGenerator:
    for market, client_order_id, order_price, order_status, side, q_rel, q_rem in order_status_updates:
        yield api.GetOrderStatusStreamResponse(
            slot=1,
            order_info=api.GetOrderStatusResponse(
                market=market, client_order_id=client_order_id, order_price=order_price, order_status=order_status,
                side=side,
...
async def start_os_stream(provider: Provider, market: str, owner_address: str, queue: asyncio.Queue):
    await provider.connect()
    os_stream = provider.get_order_status_stream(
        api.GetOrderStatusStreamRequest(market=market, owner_address=owner_address,
