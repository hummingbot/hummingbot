...
class TestSouthXchangeExchange(unittest.TestCase):  
    ...
    def test_orders_saving_and_restoration(self):
        ...
        base_token: str = "LTC2"
        price = self.market.get_price(trading_pair, True) * Decimal("1.05")
        price = self.market.quantize_order_price(trading_pair, price)
        amount = self.market.quantize_order_amount(trading_pair, Decimal("0.0002"))
        quote_bal = self.market.get_available_balance(quote_token)
        base_bal = self.market.get_available_balance(base_token)

        order_buy, exch_order_id = self.place_order(True, trading_pair, amount, OrderType.LIMIT, price)
        resp_LIST_ORDER_BUY = Fixturesouthxchange.OPEN_ORDERS_BUY.copy()
        resp_LIST_ORDER_BUY["Code"] = str(exch_order_id)
        [order_created_event] = self.run_parallel(self.market_logger.wait_for(BuyOrderCompletedEvent))
        ...
        self.assertTrue([evt.order_type == OrderType.LIMIT for evt in trade_events])
        self.assertEqual(order_buy, order_created_event.order_id)
        self.assertEqual(amount, order_created_event.base_asset_amount)
        self.assertEqual("LTC2", order_created_event.base_asset)
        self.assertEqual("USD2", order_created_event.quote_asset)
        self.assertAlmostEqual(base_amount_traded, order_created_event.base_asset_amount)
        self.assertAlmostEqual(quote_amount_traded, order_created_event.quote_asset_amount)
        self.assertGreater(order_created_event.fee_amount, Decimal(0))
        self.assertTrue(any([isinstance(event, BuyOrderCreatedEvent) and event.order_id == order_buy
                             for event in self.market_logger.event_log]))
        ...
        # self.ev_loop.run_until_complete(asyncio.sleep(1))
        self.ev_loop.run_until_complete(self.market._update_balances())
        self.assertAlmostEqual(expected_quote_bal, self.market.get_available_balance(quote_token), 1)

        # Reset the logs
        self.event_logger.clear()

        # Try to sell back the same amount to the exchange, and watch for completion event.
        price = self.market.get_price(trading_pair, True) * Decimal("0.95")
        price = self.market.quantize_order_price(trading_pair, price)
        amount = self.market.quantize_order_amount(trading_pair, Decimal("0.0002"))
        order_id, exch_order_id = self.place_order(False, trading_pair, amount, OrderType.LIMIT, price)
        order_completed_event = self.ev_loop.run_until_complete(self.market_logger.wait_for(SellOrderCompletedEvent))
        trade_events = [t for t in self.market_logger.event_log if isinstance(t, OrderFilledEvent)]
        base_amount_traded = sum(t.amount for t in trade_events)
        quote_amount_traded = sum(t.amount * t.price for t in trade_events)
        ...
        self.assertEqual("LTC2", order_completed_event.base_asset)
        self.assertEqual("USD2", order_completed_event.quote_asset)
        ...
        # self.ev_loop.run_until_complete(asyncio.sleep(1))
        self.ev_loop.run_until_complete(self.market._update_balances())
        self.assertAlmostEqual(expected_base_bal, self.market.get_available_balance(base_token), 5)