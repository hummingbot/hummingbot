...
API_SECRET = conf.btc_markets_secret_key
BASE_API_URL = "api.btcmarkets.net"


class BtcMarketsExchangeUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.connector: BtcMarketsExchange = BtcMarketsExchange(
            btc_markets_api_key=API_KEY,
            btc_markets_secret_key=API_SECRET,
            trading_pairs=[cls.trading_pair],
            trading_required=True
        )
        print("Initializing BtcMarket market... this will take about a minute.")
    ...
    def test_estimate_fee(self):
        ...
        self.assertAlmostEqual(maker_fee, Decimal("0.0085"))
        taker_fee = self.connector.estimate_fee_pct(False)
        self.assertAlmostEqual(taker_fee, Decimal("0.0085"))
    ...
    def test_buy_and_sell(self):
        ...
        trade_fee = sum(t.trade_fee.flat_fees[0].amount for t in trade_events)
        ...
        self.assertEqual("AUD", order_completed_event.quote_asset)
        ...
        expected_quote_bal = quote_bal - quote_amount_traded - trade_fee
    ...
    def test_order_price_precision(self):
        ...
        self.assertGreater(self.connector.get_balance("XRP"), Decimal("0.001"))
        self.assertGreater(self.connector.get_balance("AUD"), Decimal("10"))
    ...
    def test_orders_saving_and_restoration(self):
        ...
        try:
            ...
            # Try to put limit buy order for 0.001 AUD worth of XRP, and watch for order creation event.
            ...
            for event_tag in self.events:
                self.connector.remove_listener(event_tag, self.event_logger)
            new_connector = BtcMarketsExchange(API_KEY, API_SECRET, [self.trading_pair], True)
            for event_tag in self.events:
