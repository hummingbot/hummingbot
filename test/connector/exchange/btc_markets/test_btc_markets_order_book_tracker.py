...
class BtcMarketsOrderBookTrackerUnitTest(unittest.TestCase):
    order_book_tracker: Optional[BtcMarketsOrderBookTracker] = None
    ...
    def setUpClass(cls):
        ...
        cls.order_book_tracker: BtcMarketsOrderBookTracker = BtcMarketsOrderBookTracker(cls.trading_pairs)
    ...
    def test_tracker_integrity(self):
        # Wait 15 seconds to process some diffs.
        self.ev_loop.run_until_complete(asyncio.sleep(60.0))
        order_books: Dict[str, OrderBook] = self.order_book_tracker.order_books
        btc_aud: OrderBook = order_books["BTC-AUD"]
        self.assertIsNot(btc_aud.last_diff_uid, 0)
        self.assertGreaterEqual(btc_aud.get_price_for_volume(True, 10).result_price,
                                btc_aud.get_price(True))
        self.assertLessEqual(btc_aud.get_price_for_volume(False, 10).result_price,
                             btc_aud.get_price(False))

    def test_api_get_last_traded_prices(self):
        prices = self.ev_loop.run_until_complete(
            BtcMarketsAPIOrderBookDataSource.get_last_traded_prices(["BTC-AUD", "USDT-AUD"]))
        for key, value in prices.items():
            print(f"{key} last_trade_price: {value}")
        self.assertGreater(prices["BTC-AUD"], 1000)
        self.assertLess(prices["USDT-AUD"], 2)
