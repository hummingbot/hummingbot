...
class PeatioOrderBookTrackerUnitTest(unittest.TestCase):
    ...
    def test_tracker_integrity(self):
        ...
        btcusdt_book: OrderBook = order_books["btc_usdterc20"]
        ethusdt_book: OrderBook = order_books["eth_usdterc20"]
    ...
    def test_api_get_last_traded_prices(self):
        prices = self.ev_loop.run_until_complete(
            PeatioAPIOrderBookDataSource.get_last_traded_prices(["btc_usdterc20", "eth_usdterc20"]))
        for key, value in prices.items():
            print(f"{key} last_trade_price: {value}")
        self.assertGreater(prices["btc_usdterc20"], 1000)
        self.assertLess(prices["eth_usdterc20"], 1000)
