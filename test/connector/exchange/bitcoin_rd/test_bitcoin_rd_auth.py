...
from hummingbot.connector.exchange.bitcoin_rd.bitcoin_rd_constants import REST_URL
from hummingbot.connector.exchange.bitcoin_rd.bitcoin_rd_utils import get_ws_url_private

sys.path.insert(0, realpath(join(__file__, "../../../../../")))
logging.basicConfig(level=METRICS_LOG_LEVEL)


class TestBitcoinRDAuth(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.ev_loop: asyncio.BaseEventLoop = asyncio.get_event_loop()
        api_key = conf.bitcoin_rd_api_key
        secret_key = conf.bitcoin_rd_secret_key
        cls.auth = BitcoinRDAuth(api_key, secret_key)
