...
class RadarRelayMarketUnitTest(unittest.TestCase):
    ...
    def test_calculate_fees(self):
        [maker_buy_trade_fee] = self.market.calculate_fees("ZRX-WETH", 20, 0.01, OrderType.LIMIT, TradeType.BUY)
        self.assertEqual(maker_buy_trade_fee.amount, 0)
        [taker_buy_trade_fee] = self.market.calculate_fees("ZRX-WETH", 20, 0.01, OrderType.MARKET, TradeType.BUY)
        self.assertGreater(taker_buy_trade_fee.amount, 0)
        self.assertEqual(taker_buy_trade_fee.type, FeeType.ADD_OTHER)
        self.assertEqual(taker_buy_trade_fee.other_symbol, "ETH")
